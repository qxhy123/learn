...
下图展示了相量的核心直觉：左边是一个在复平面上以角速度 $\omega$ 旋转的向量（模长为振幅 $A$，辐角为 $\omega t+\varphi$），它在实轴上的投影随时间画出右边的正弦信号 $A\cos(\omega t+\varphi)$。

![相量：复平面上旋转向量在实轴的投影画出正弦波 A cos(ωt+φ)](../figures/svg/trig-p8-22-1.svg)
