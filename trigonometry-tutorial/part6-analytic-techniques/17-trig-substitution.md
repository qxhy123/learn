...
更直观的看法是画一个直角三角形：取角 $\theta$，让**斜边**为 $a$、**对边**为 $x$，则 $\sin\theta=\dfrac{x}{a}$ 正好对应代换 $x=a\sin\theta$；由勾股定理，**邻边**就是 $\sqrt{a^2-x^2}=a\cos\theta$。根式不是被「消去」，而是变成了三角形的一条边。

![直角三角形解释 x=a sinθ：斜边 a、对边 x、邻边 √(a²−x²)](../figures/svg/trig-p6-17-1.svg)
