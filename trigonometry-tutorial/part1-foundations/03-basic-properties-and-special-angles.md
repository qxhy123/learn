...
参考角法的几何本质，是把同一个参考角 $\alpha$ 沿坐标轴"对称复制"到四个象限：$\theta=\alpha$ 在第一象限，关于 $y$ 轴对称得到 $\pi-\alpha$（第二象限），关于原点对称得到 $\pi+\alpha$（第三象限），关于 $x$ 轴对称得到 $-\alpha$（即 $2\pi-\alpha$，第四象限）。这四点的 $\sin,\cos$ **大小完全相同**，只是符号随象限改变——这正是诱导公式"符号看象限"的来源。

![参考角 α 在四象限的对称分布：π−α、π+α、−α 与各自的坐标符号](../figures/svg/trig-p1-03-1.svg)
