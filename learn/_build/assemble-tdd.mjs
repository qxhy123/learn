// 组装「TDD 闯关」课程：_build/tdd/uN.json（agent 产出的关卡数组）
//   → public/courses/tdd/{course.json, units/uN.json}
// 用法：node _build/assemble-tdd.mjs
// 只组装存在源文件的单元。
...
const SRC = path.join(ROOT, '_build/tdd')
const OUT = path.join(ROOT, 'public/courses/tdd')

// 5 单元 = 教程 5 个 part
const UNITS = [
  { id: 'u1', title: '基础：红绿重构',   color: '#58cc02', icon: '🌱', blurb: 'TDD 是什么 · unittest 基础 · 第一个完整 TDD 循环' },
  { id: 'u2', title: '核心概念',         color: '#1cb0f6', icon: '🧩', blurb: '测试组织命名 · 断言深析 · 夹具与生命周期 · 套件发现' },
  { id: 'u3', title: '中级技巧',         color: '#ce82ff', icon: '🔧', blurb: 'Mock 与打桩 · 异常测试 · 参数化 · IO 与数据库' },
  { id: 'u4', title: '高阶实践',         color: '#ff9600', icon: '🚀', blurb: '测试替身模式 · 异步测试 · 覆盖率与度量 · 最佳实践' },
  { id: 'u5', title: '深入',             color: '#ff4b4b', icon: '🧠', blurb: '基于性质 · 六边形架构 · 遗留代码 · Mock 内部 · 设计模式' },
]
...
async function main() {
  ...
  for (const u of UNITS) {
    ...
    manifestUnits.push({ id: u.id, title: u.title, color: u.color, icon: u.icon, blurb: u.blurb, file: `courses/tdd/units/${u.id}.json` })
  }

  const manifest = {
    id: 'tdd',
    title: 'TDD 闯关',
    subtitle: '像闯关一样学测试驱动开发',
    brandMark: '✅',
    brandName: 'TDD Quest',
    color: '#25a162',
    source: '来自《Python TDD 完整教程：从零到高阶》',
    figureBase: 'courses/tdd/figures/',
    inputHint: '可填术语、断言/方法名或数值；空格大小写不敏感',
