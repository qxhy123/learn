// 组装「DDD 闯关」课程：_build/ddd/uN.json → public/courses/ddd/{course.json, units/uN.json}
// 用法：node _build/assemble-ddd.mjs   只组装存在源文件的单元。
...
const SRC = path.join(ROOT, '_build/ddd')
const OUT = path.join(ROOT, 'public/courses/ddd')

// 5 单元 = 教程 5 个 part
const UNITS = [
  { id: 'u1', title: '基础认知',   color: '#58cc02', icon: '🧭', blurb: 'DDD 是什么 · 核心概念 · 通用语言 · 战略 vs 战术' },
  { id: 'u2', title: '战略设计',   color: '#1cb0f6', icon: '🗺️', blurb: '限界上下文 · 上下文映射 · 子域类型 · 事件风暴' },
  { id: 'u3', title: '战术设计',   color: '#ce82ff', icon: '🧱', blurb: '实体与值对象 · 聚合 · 领域事件 · 仓储/服务/工厂' },
  { id: 'u4', title: '架构模式',   color: '#ff9600', icon: '🏛️', blurb: '分层 · 六边形 · CQRS · 事件溯源' },
  { id: 'u5', title: '高阶实践',   color: '#ff4b4b', icon: '🚀', blurb: '微服务 · Saga · 防腐层 · 测试策略 · 案例研究' },
]
...
async function main() {
  ...
  for (const u of UNITS) {
    ...
    manifestUnits.push({ id: u.id, title: u.title, color: u.color, icon: u.icon, blurb: u.blurb, file: `courses/ddd/units/${u.id}.json` })
  }

  const manifest = {
    id: 'ddd',
    title: 'DDD 闯关',
    subtitle: '像闯关一样学领域驱动设计',
    brandMark: 'DDD',
    brandName: 'DDD Quest',
    color: '#8b5cf6',
    source: '来自《领域驱动设计（DDD）从零到高阶教程》',
    figureBase: 'courses/ddd/figures/',
    inputHint: '可填术语、模式名或概念（中英皆可）；空格大小写不敏感',
