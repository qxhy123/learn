// 组装「AI Infra」课程：_build/aiinfra/uNcK.json → public/courses/ai-infra/{course.json,units/uN.json}
// 用法：node _build/assemble-aiinfra.mjs   （agent 直接产出 JSON）
...
const SRC = path.join(ROOT, '_build/aiinfra')
const OUT = path.join(ROOT, 'public/courses/ai-infra')

// 9 单元元信息（每 part 一个单元）
const UNITS = [
  { id: 'u1', n: 27, title: '系统基础',       color: '#64748b', icon: '🧮', blurb: 'CPU微架构·虚存与IO·文件系统·网络栈' },
  { id: 'u2', n: 3,  title: 'AI Infra 总览',  color: '#58cc02', icon: '🗺️', blurb: '定义边界·算力存储网络·从模型到生产' },
  { id: 'u3', n: 15, title: '系统栈：GPU与IO', color: '#1cb0f6', icon: '🖥️', blurb: 'GPU加速器·内存互联·CUDA运行时与内核' },
  { id: 'u4', n: 7,  title: '训练基础设施',   color: '#ff9600', icon: '🏋️', blurb: '单机·数据并行·模型流水并行·检查点恢复' },
  { id: 'u5', n: 18, title: '数据与存储',     color: '#ce82ff', icon: '🗄️', blurb: '数据管道·制品检查点·特征向量与缓存' },
  { id: 'u6', n: 8,  title: '推理服务',       color: '#e11d48', icon: '⚡', blurb: '在线推理·批处理KV缓存·量化引擎·多租户' },
  { id: 'u7', n: 15, title: '平台与编排',     color: '#0d9488', icon: '☸️', blurb: '容器运行时·K8s for AI·队列配额弹性' },
  { id: 'u8', n: 3,  title: '可靠性与安全',   color: '#9333ea', icon: '🛡️', blurb: '可观测容量·评测发布事件·安全隔离治理' },
  { id: 'u9', n: 4,  title: '进阶与实战',     color: '#db2777', icon: '🚀', blurb: '搭建平台·Agent与推理时计算·面试题' },
]
...
async function main() {
  ...
  for (const u of UNITS) {
    ...
    manifestUnits.push({ id: u.id, title: u.title, color: u.color, icon: u.icon, blurb: u.blurb, file: `courses/ai-infra/units/${u.id}.json` })
  }

  const manifest = {
    id: 'ai-infra',
    title: 'AI Infra 闯关',
    subtitle: '像闯关一样学 AI 基础设施',
    brandMark: '🛠',
    brandName: 'AI Infra Quest',
    color: '#0ea5e9',
    source: '来自《AI Infra 从系统到平台完全教程》',
    figureBase: 'courses/ai-infra/figures/',
    inputHint: "可填英文术语、缩写或数值；不区分大小写",
