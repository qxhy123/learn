...
import { useCourse, lessonOrder } from '../course'
...
export function SkillTree({ onStart, onHome }: Props) {
  const { progress } = useProgress()
  const course = useCourse()
  const UNITS = course.units
  const LESSON_ORDER = lessonOrder(course)

  const isCompleted = (id: string) => Boolean(progress.completed[id])
  const isUnlocked = (id: string) => {
    if (course.config.unlockAll) return true
  ...
  return (
    <div className="tree">
      <div className="tree-head">
        ...
        <h1>
          <RichText text={course.title} />
        </h1>
        <p className="tree-sub">
          {course.subtitle} · 已完成 {doneLessons}/{totalLessons} 课
        </p>
      ...
      <footer className="tree-foot">{course.source}</footer>
    </div>
  )
}
