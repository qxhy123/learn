...
看成纯代数技巧。但从动力系统角度看，它真正做的是：

- 找到一组更适合描述系统演化的坐标轴
- 让原本彼此耦合的变化，在新坐标下尽量拆开
- 把一个矩阵问题降成多个标量指数问题

所以“对角化”并不只是为了方便算，而是在寻找系统最自然的观察方式。
...
$$
\mathbf x_0=\begin{pmatrix}x_0\\y_0\end{pmatrix}
$$

则

$$
\mathbf x(t)=\begin{pmatrix}e^{2t}x_0\\e^{-t}y_0\end{pmatrix}
$$

这个例子很简单，却非常关键，因为它告诉我们：

- 一个特征方向可以按 $e^{2t}$ 快速增长
- 另一个特征方向可以按 $e^{-t}$ 衰减
- 同一个系统里，不同方向可以同时呈现完全不同的指数行为

也就是说，矩阵指数不是给“一个增长率”，而是在给整个状态空间的方向结构分配不同的演化方式。

### 17.5.3 旋转矩阵：为什么会连到三角函数
...
![线性系统 $\mathbf x'=A\mathbf x$ 的相平面轨迹：当 $A$ 有共轭复特征值 $\lambda=-0.35\pm i$（实部为负、虚部非零）时，多条轨迹螺旋收敛到原点平衡点，体现旋转振荡与指数衰减的叠加](../figures/svg/exp-p5-17-1.svg)
