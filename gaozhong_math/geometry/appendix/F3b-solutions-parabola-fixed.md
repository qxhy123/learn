...
$AQ$ 方程：$y=\dfrac{2k}{3}(x+2)$，即截距 $y|_{x=0}=\dfrac{4k}{3}$，不为定值。但令 $\dfrac{y}{x+2}=\dfrac{2k}{3}$，若令 $x_Q=-2$？不过直线 $AQ$ 过 $A(-2,0)$ 本身。观察：$\dfrac{y_Q}{x_Q-x_A}=\dfrac{2k}{3}$ 与 $k$ 有关，但 $\dfrac{k_{AQ}}{k_{AP}}=\dfrac{2k/3}{k}=\dfrac{2}{3}$ 为定值。改求定点：设 $AQ$ 过点 $(t,0)$，则 $0=\dfrac{2k}{3}(t+2)\Rightarrow t=-2$（即 $A$ 点，非真正意义的"另一定点"）。

再检查：$Q$ 在直线 $y=\dfrac{2k}{3}(x+2)$ 上，此直线过 $(-2,0)=A$，斜率为 $\dfrac{2k}{3}$（随 $k$ 变化），所以 $AQ$ 始终过 $A$（这是已知的），但不过其他固定点。

检验定点候选 $(-\tfrac{2}{3},0)$：代入 $y=\dfrac{2k}{3}(x+2)$，$0=\dfrac{2k}{3}\cdot\dfrac{4}{3}\neq0$。改验 $(0,\tfrac{4k}{3})$：随 $k$ 变化。**结论：直线 $AQ$ 过定点 $A(-2,0)$ 本身（题设要求重新理解）**。题意应为判断 $AQ$（即连接 $A$ 与 $Q$）的延长线是否过某个与 $k$ 无关的定点——$AQ$ 斜率 $=\dfrac{2k}{3}$，$AP$ 斜率 $=k$，故 $k_{AQ}=\dfrac{2}{3}k_{AP}$，直线 $AQ$ 斜率随 $k$ 而变，不过固定点（除 $A$ 外）。
...
直线方程 $x=my+3$，无论 $m$ 如何变化，令 $y=0$ 得 $x=3$……但 $A=(3,0)$ 正是题设点，若 $M,N$ 均在过 $A$ 的直线上则显然过 $A$。再令 $x=0$：$0=my+3\Rightarrow y=-\tfrac{3}{m}$ 随 $m$ 变化，故不过 $y$ 轴定点。

重新审题：$A(3,0)$ 不在抛物线上（$0\neq4\cdot3$），$M,N$ 在抛物线上，直线 $MN$ 即过 $A$ 的直线，$\overrightarrow{AM}=\lambda\overrightarrow{AN}$ 给出额外约束 $y_1y_2=-12$（非焦点弦的 $-4$）。则直线 $MN$ 满足截距条件 $n=3$，即 $x=my+3$，此直线过 $(3,0)=A$，所以定点就是 $A(3,0)$。

**注意**：题意中的"直线 $MN$ 过某定点"——若条件 $\overrightarrow{AM}=\lambda\overrightarrow{AN}$ 变为"$\overrightarrow{OM}=\lambda\overrightarrow{ON}$"（原点而非 $A$），则约束不同。按题意 $y_1y_2=-12$ 使直线方程过 $A(3,0)$，定点即 $\boxed{(3,0)}$ 即 $A$ 本身，或题目考查过焦点 $F(1,0)$ 的定点（取 $A=1$时 $y_1y_2=-4$，此时 $n=1$，过 $F$）。
