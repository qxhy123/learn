...
|------|------|
| $e^{O} = I$ | 零矩阵的指数是单位矩阵 |
| $e^{(s+t)A} = e^{sA}e^{tA}$ | 指数法则（同矩阵） |
| $(e^A)^{-1} = e^{-A}$ | 矩阵指数总可逆 |
| $\det(e^A) = e^{\text{tr}(A)}$ | Jacobi 公式 |
| $e^{PAP^{-1}} = Pe^AP^{-1}$ | 相似不变性 |

**注意**：一般情况下 $e^{A+B} \neq e^A e^B$，除非 $AB = BA$（$A, B$ 可交换）。

**不可对角化时的矩阵指数**：利用 Jordan 标准形 $A = PJP^{-1}$，可以计算 $e^{Jt}$。对 Jordan 块 $J_k(\lambda)$：

$$e^{J_k(\lambda)t} = e^{\lambda t}\begin{pmatrix} 1 & t & t^2/2! & \cdots \\ 0 & 1 & t & \cdots \\ \vdots & & \ddots & \\ 0 & \cdots & 0 & 1 \end{pmatrix}$$

### Schur 分解

对于不可对角化的矩阵，还有另一个重要的标准形——**Schur 分解**：

**定理（Schur）**：对任意方阵 $A \in \mathbb{C}^{n \times n}$，存在酉矩阵 $U$（$U^*U = I$）使得：

$$A = UTU^*$$

其中 $T$ 是**上三角矩阵**，对角线上的元素是 $A$ 的特征值。

对于**实矩阵**，如果所有特征值都是实数，则存在**实正交矩阵** $Q$ 使得 $A = QTQ^T$。

**Schur 分解 vs Jordan 标准形**：

| | Schur 分解 | Jordan 标准形 |
|--|-----------|--------------|
| 存在性 | **所有**方阵 | **所有**方阵 |
| 变换矩阵 | 酉/正交矩阵（数值稳定） | 一般可逆矩阵（可能病态） |
| 结果形式 | 上三角 | 块对角（Jordan块） |
| 数值计算 | 首选（QR算法） | 数值不稳定，很少直接计算 |
...
![$P^{-1}$ 转特征基 → $\Lambda$ 缩放 → $P$ 还原](../figures/svg/la-p7-20-1.svg)
