...
> **难度**：★★★☆☆
> **前置知识**：第4–5章（线性方程组、高斯消元）
> **本文件**：融合"原版严格推导 + 速记 / 套路 / 自测"。保留原版完整正文 + 在最前置一例速记 / 思维路径 + 最后追加方法总结与自测。

> **一例速记**：
> **定义**：$AB=BA=I$ 则 $B=A^{-1}$；逆矩阵只对**方阵**存在，且**唯一**。
> **可逆条件**：$\det(A)\ne 0$（等价：满秩、行列无关、$A\mathbf{x}=\mathbf{0}$ 只有零解……）。
> **2×2 公式**：$\begin{pmatrix}a&b\\c&d\end{pmatrix}^{-1}=\dfrac{1}{ad-bc}\begin{pmatrix}d&-b\\-c&a\end{pmatrix}$（对角交换，副对角变号）。
> **行变换法**（通用）：$[A\mid I]\xrightarrow{\text{RREF}}[I\mid A^{-1}]$；若左边出现全零行则不可逆。
> **乘积逆**：$(AB)^{-1}=B^{-1}A^{-1}$（顺序反转，"脱衣服定律"）。
> **AI 关联**：实际中**不直接求逆**解 $A\mathbf{x}=\mathbf{b}$——`solve(A,b)` 用 LU 分解，更快更稳定；逆矩阵 $O(n^3)$ 常数更大、精度更低、破坏稀疏性。

---

## 引入：为什么 $2\times 2$ 逆矩阵公式那么"神奇"？

> **题目**：直接验证以下公式：$A=\begin{pmatrix}3&1\\5&2\end{pmatrix}$，$A^{-1}=\begin{pmatrix}2&-1\\-5&3\end{pmatrix}$，并说明公式的来源。

直觉上"逆矩阵"需要解一个大方程组，但 $2\times 2$ 情形有闭合公式——这不是巧合，而是行列式与代数余子式的结构保证的。下面把直觉还原。

---

## 思维路径还原（解题者的内心独白）

> "看到 $2\times 2$ 逆矩阵，先算行列式：$\det(A)=3\times 2-1\times 5=6-5=1$。不为零，可逆。
>
> **套公式**：$A^{-1}=\dfrac{1}{1}\begin{pmatrix}2&-1\\-5&3\end{pmatrix}$。规律：主对角调换位置（$a\leftrightarrow d$），副对角变符号（$b\to -b,c\to -c$）。
>
> **验证**：$AA^{-1}=\begin{pmatrix}3&1\\5&2\end{pmatrix}\begin{pmatrix}2&-1\\-5&3\end{pmatrix}=\begin{pmatrix}6-5&-3+3\\10-10&-5+6\end{pmatrix}=\begin{pmatrix}1&0\\0&1\end{pmatrix}=I$。✓
>
> **公式来源**：设 $B=\begin{pmatrix}d&-b\\-c&a\end{pmatrix}$（即伴随矩阵），则 $AB=\begin{pmatrix}ad-bc&0\\0&ad-bc\end{pmatrix}=(ad-bc)I$。因此 $A\cdot\dfrac{B}{ad-bc}=I$，即 $A^{-1}=\dfrac{B}{ad-bc}$。
>
> **推广到 $3\times 3$**：不用记公式，改用行变换法 $[A\mid I]\to[I\mid A^{-1}]$，更通用且不易出错。
>
> **工程直觉**：知道 $A^{-1}$ 是为了写出形式解 $\mathbf{x}=A^{-1}\mathbf{b}$；实际运行时 `numpy.linalg.solve(A, b)` 比 `inv(A) @ b` 更快、精度更高——这是数值线性代数的铁律。"
...
![A 变换与 $A^{-1}$ 反变换之间的关系](../figures/svg/la-p2-06-1.svg)
...
| 场景 | 公式 / 方法 | 复杂度 |
|---|---|---|
| **2×2 逆** | $\frac{1}{ad-bc}\begin{pmatrix}d&-b\\-c&a\end{pmatrix}$ | $O(1)$ |
| **一般方阵逆** | $[A\mid I]\xrightarrow{\text{RREF}}[I\mid A^{-1}]$ | $O(n^3)$ |
| **伴随矩阵法** | $A^{-1}=\frac{1}{\det(A)}\text{adj}(A)$ | $O(n!)$，仅理论用 |
| **乘积逆** | $(AB)^{-1}=B^{-1}A^{-1}$ | 顺序反转 |
| **转置逆** | $(A^T)^{-1}=(A^{-1})^T$ | 两者可交换 |
| **可逆性检验** | $\det(A)\ne 0$，或化 REF 无全零行 | — |
| **解方程组（推荐）** | `solve(A, b)`，底层 LU 分解 | $O(n^3)$ 一次 + $O(n^2)$ |

### 判断可逆性标准 3 步

1. **计算行列式**：$\det(A)\ne 0$ → 可逆（$2\times 2$ 直接算，大矩阵用三角化）
2. **行变换法**：$[A\mid I]$ 消元，左边化 RREF 是否为 $I$；出现全零行则不可逆
3. **秩判断**：$\text{rank}(A)=n$ → 可逆（$n$ 为阶数）

### 求逆矩阵标准步骤（行变换法）

1. 构造 $[A\mid I_n]$（$n\times 2n$ 增广矩阵）
2. 对整体施初等行变换，目标：左半部变为 $I_n$
3. 右半部自动变为 $A^{-1}$
4. 若左半部无法化为 $I_n$（出现全零行）→ $A$ 不可逆，停止

---

## 方法变形

### 变形 1：用 $A^{-1}$ 验证可逆矩阵等价条件

$A$ 可逆 $\Leftrightarrow$ $A$ 各行（列）线性无关 $\Leftrightarrow$ $\text{rank}(A)=n$ $\Leftrightarrow$ $A\mathbf{x}=\mathbf{0}$ 只有零解 $\Leftrightarrow$ 对任意 $\mathbf{b}$，$A\mathbf{x}=\mathbf{b}$ 有唯一解。这 9 个等价条件构成"可逆矩阵定理"，做题时灵活选用最方便的那个。

### 变形 2：$(AB)^{-1}$ 的方向

记忆：先穿袜子后穿鞋（$AB$），脱的时候先脱鞋后脱袜子（$B^{-1}A^{-1}$）。对 $k$ 个矩阵：$(A_1A_2\cdots A_k)^{-1}=A_k^{-1}\cdots A_2^{-1}A_1^{-1}$。

### 变形 3：Sherman-Morrison 秩 1 更新

已知 $A^{-1}$，矩阵变为 $A+\mathbf{u}\mathbf{v}^T$（秩 1 更新）：
$$( A+\mathbf{u}\mathbf{v}^T)^{-1}=A^{-1}-\dfrac{A^{-1}\mathbf{u}\mathbf{v}^T A^{-1}}{1+\mathbf{v}^T A^{-1}\mathbf{u}}$$
更新代价从 $O(n^3)$ 降为 $O(n^2)$——在线学习 / Kalman 滤波的核心技巧。

### 变形 4：为何工程中避免显式求逆

- **同等复杂度但常数更大**：求 $A^{-1}$ 相当于对 $n$ 个右端向量分别消元，常数因子约是直接 `solve` 的 $n$ 倍
- **数值误差更大**：两次矩阵乘法的舍入误差叠加
- **稀疏矩阵变稠密**：图神经网络的拉普拉斯矩阵是稀疏的，其逆通常稠密，存储从 $O(n)$ 变 $O(n^2)$

---

## 思考路标（条件反射）

1. 看到"求逆矩阵" → 先判断 $\det(A)\ne 0$；$2\times 2$ 用公式，大矩阵用行变换法
2. 看到 $[A\mid I]$ 行变换后左边出现全零行 → $A$ 不可逆，**停止**
3. 看到"$(AB)^{-1}$" → 反转顺序 $B^{-1}A^{-1}$，不能直接写 $A^{-1}B^{-1}$
4. 看到"$(A^T)^{-1}$" → 等于 $(A^{-1})^T$，两者可以自由交换
5. 看到"解 $A\mathbf{x}=\mathbf{b}$" → 不要求 $A^{-1}$，用 `solve(A, b)`；教材里写 $A^{-1}\mathbf{b}$ 是记号，不是算法
6. 看到"可逆" → 等价于 $\det\ne 0$ = 满秩 = 各列线性无关 = 零空间平凡
7. 看到"奇异矩阵" → 行列式为零，某方向信息丢失，无法还原（不可逆）
8. 看到"矩阵的秩 1 更新" → 想到 Sherman-Morrison 公式，$O(n^2)$ 更新逆
9. 看到"$(cA)^{-1}$" → 等于 $\frac{1}{c}A^{-1}$，标量提出取倒数
10. 看到"伴随矩阵" → 记住 $A^{-1}=\text{adj}(A)/\det(A)$，计算量大，仅用于理论

---

## 易错点

1. **$2\times 2$ 公式副对角号搞反**：$\begin{pmatrix}a&b\\c&d\end{pmatrix}^{-1}=\frac{1}{ad-bc}\begin{pmatrix}d&-b\\-c&a\end{pmatrix}$——主对角元素 $a,d$ 调换，副对角 $b,c$ 取反。常见错误：忘记对 $b$ 或 $c$ 取反，或忘记除以 $\det$。

2. **$(AB)^{-1}=A^{-1}B^{-1}$（顺序未反转）**：矩阵乘法不满足交换律，$(AB)^{-1}=B^{-1}A^{-1}$，顺序必须反转；类比脱衣服定律记忆。

3. **把 $\det(A)=0$ 当成"$A$ 的逆为 $\infty$"**：行列式为零意味着 $A$ 根本没有逆矩阵，不是"逆是无穷大"——是结构性无法恢复，而非数值问题。

4. **行变换法中两侧操作不同步**：$[A\mid I]$ 化简时必须对整行（含右半部）施行同样的变换，只变左半部而忘了右半部是最常见失误，会导致右边不是 $A^{-1}$。

5. **用 `inv(A) @ b` 代替 `solve(A, b)`**：理论上等价，数值上前者精度更差（尤其条件数大时）且多一次矩阵乘法。工程上必须用 `solve`；考试解题时写 $A^{-1}\mathbf{b}$ 是可以的，但需知晓数值风险。

---

## 典型应用例题

### 例 1：$2\times 2$ 逆矩阵 + 方程组求解

> **题目**：已知 $A=\begin{pmatrix}4&3\\3&2\end{pmatrix}$，(1) 求 $A^{-1}$；(2) 用 $A^{-1}$ 解方程组 $A\mathbf{x}=\begin{pmatrix}1\\0\end{pmatrix}$。

【思路】$2\times 2$ 直接用公式；解方程 = 矩阵-向量乘。

【解】
(1) $\det(A)=4\times 2-3\times 3=8-9=-1$，故
$$A^{-1}=\frac{1}{-1}\begin{pmatrix}2&-3\\-3&4\end{pmatrix}=\begin{pmatrix}-2&3\\3&-4\end{pmatrix}$$

(2) $\mathbf{x}=A^{-1}\mathbf{b}=\begin{pmatrix}-2&3\\3&-4\end{pmatrix}\begin{pmatrix}1\\0\end{pmatrix}=\begin{pmatrix}-2\\3\end{pmatrix}$。

验证：$A\mathbf{x}=\begin{pmatrix}4\times(-2)+3\times 3\\3\times(-2)+2\times 3\end{pmatrix}=\begin{pmatrix}1\\0\end{pmatrix}$。✓

【答案】$A^{-1}=\begin{pmatrix}-2&3\\3&-4\end{pmatrix}$，$\mathbf{x}=(-2,3)^T$。

### 例 2：行变换法求 $3\times 3$ 逆矩阵

> **题目**：求 $A=\begin{pmatrix}2&1&0\\1&3&1\\0&1&2\end{pmatrix}$ 的逆（即练习 6.2）。

【思路】构造 $[A\mid I]$，化 RREF，右边即 $A^{-1}$。

【解】（详细步骤见 6.2 节练习 6.2 答案）最终：
$$A^{-1}=\frac{1}{8}\begin{pmatrix}5&-2&1\\-2&4&-2\\1&-2&5\end{pmatrix}$$

【关键节点检查】每步后验证左边行阶梯形进展正确；最终乘回验证 $AA^{-1}=I$。

### 例 3：逆矩阵性质与矩阵方程

> **题目**：已知 $A$ 可逆，化简 $(A^T A)^{-1} A^T$。

【思路】分拆逆、用性质 $(AB)^{-1}=B^{-1}A^{-1}$ 和 $(A^T)^{-1}=(A^{-1})^T$。

【解】$(A^T A)^{-1} A^T = A^{-1}(A^T)^{-1} A^T = A^{-1}(A^{-1})^T A^T$。

注意 $(A^T)^{-1}A^T=I$，故 $(A^T A)^{-1}A^T=A^{-1}$。

更简洁：$(A^T A)^{-1}A^T$ 是 $A$ 的**左伪逆**（当 $A$ 是方阵且可逆时退化为 $A^{-1}$）：
$$[(A^T A)^{-1}A^T]\cdot A=A^{-1}(A^T)^{-1}A^T A=A^{-1}I A=A^{-1}A=I$$

【答案】$(A^T A)^{-1}A^T=A^{-1}$（$A$ 可逆时）；这也正是最小二乘解的正规方程解 $\hat{\mathbf{w}}=(X^TX)^{-1}X^T\mathbf{y}$ 在 $X$ 满列秩时的意义。

---

## 自测题

**自测 1**　$A=\begin{pmatrix}1&2\\3&7\end{pmatrix}$，求 $A^{-1}$，并用它解 $A\mathbf{x}=\begin{pmatrix}0\\1\end{pmatrix}$。

> 💡 提示：$\det(A)=7-6=1$；$A^{-1}=\begin{pmatrix}7&-2\\-3&1\end{pmatrix}$；$\mathbf{x}=(-2,1)^T$。验证：$1(-2)+2(1)=0,3(-2)+7(1)=1$。✓

**自测 2**　设 $A,B$ 可逆，化简 $(B^{-1}A)^{-1}(B^{-1}AB)$。

> 💡 提示：$(B^{-1}A)^{-1}=A^{-1}(B^{-1})^{-1}=A^{-1}B$；$(A^{-1}B)(B^{-1}AB)=A^{-1}(BB^{-1})AB=A^{-1}AB=B$。答案为 $B$。

**自测 3**　$A$ 是可逆对称矩阵（$A^T=A$），证明 $A^{-1}$ 也是对称矩阵。

> 💡 提示：$(A^{-1})^T=(A^T)^{-1}=A^{-1}$（用 $(A^T)^{-1}=(A^{-1})^T$ 和 $A^T=A$）。因此 $A^{-1}$ 也对称。

**自测 4**　为什么对于稀疏矩阵（如图拉普拉斯矩阵），工程中绝对不用 `inv(A)` 求逆？

> 💡 提示：稀疏矩阵 $A$（$O(n)$ 个非零元素）存储高效；但 $A^{-1}$ 通常是**稠密矩阵**（$O(n^2)$ 个非零元素），存储从 $O(n)$ 变 $O(n^2)$，完全失去稀疏优势。应直接用稀疏求解器 `spsolve(A, b)`。

**自测 5**　利用 Sherman-Morrison：若 $A=I$，$\mathbf{u}=\mathbf{v}=(1,0)^T$，计算 $(I+\mathbf{u}\mathbf{v}^T)^{-1}$，并直接验证。

> 💡 提示：$\mathbf{u}\mathbf{v}^T=\begin{pmatrix}1&0\\0&0\end{pmatrix}$，$A+\mathbf{u}\mathbf{v}^T=\begin{pmatrix}2&0\\0&1\end{pmatrix}$，逆为 $\begin{pmatrix}1/2&0\\0&1\end{pmatrix}$。用 S-M 公式：$I^{-1}-\frac{I^{-1}\mathbf{u}\mathbf{v}^TI^{-1}}{1+\mathbf{v}^TI^{-1}\mathbf{u}}=I-\frac{\mathbf{u}\mathbf{v}^T}{1+1}=I-\frac{1}{2}\begin{pmatrix}1&0\\0&0\end{pmatrix}=\begin{pmatrix}1/2&0\\0&1\end{pmatrix}$。✓

---

**回头看一眼"一例速记"**：

> 逆矩阵 $AB=BA=I$，只对方阵存在，$\det\ne 0$ 是可逆充要条件。
> $2\times 2$ 公式：对角换、副对角变号、除以 $\det$。
> 大矩阵：$[A\mid I]\to[I\mid A^{-1}]$；乘积逆反转顺序。
> 工程：解 $A\mathbf{x}=\mathbf{b}$ 用 `solve`，不要显式求逆。
