...
> **一例速记**：
> **二次型**：$Q(\mathbf{x}) = \mathbf{x}^T A \mathbf{x}$，$A$ 为实对称矩阵；对角元 = 纯二次项系数，$a_{ij} = a_{ji}$ = 交叉项系数的一半。
> **标准化**：正交变换 $\mathbf{x} = Q\mathbf{y}$（$Q$ 为特征向量矩阵）→ 标准形 $\sum \lambda_i y_i^2$；配方法也可得标准形（系数未必是特征值，但惯性指数不变）。
> **正定判定**：特征值全正 $\Leftrightarrow$ 顺序主子式全正 $\Leftrightarrow$ Cholesky 分解存在；$H \succ 0$ 时临界点是极小值，$H$ 不定时是鞍点。
> **Rayleigh 商**：$\min_{\|\mathbf{x}\|=1} \mathbf{x}^TA\mathbf{x} = \lambda_{\min}$，$\max = \lambda_{\max}$，最优点是对应特征向量。
> **AI 关联**：Hessian 矩阵是损失函数的二次型矩阵；符号差 $(p,q)$ 决定临界点类型；条件数 $\kappa = \lambda_{\max}/\lambda_{\min}$ 决定梯度下降收敛速度。

---

## 引入：一道判断损失面形状的题

> **题目**：设某两参数模型的损失函数在临界点 $\boldsymbol{\theta}^*$ 附近的 Hessian 矩阵为 $H = \begin{pmatrix}5 & 2 \\ 2 & 2\end{pmatrix}$。判断 $\boldsymbol{\theta}^*$ 是极小值、极大值还是鞍点。并估算以 $\boldsymbol{\theta}^*$ 为出发点时，梯度下降的最优学习率。

请先停下来想一想：要判断临界点类型，只需看 Hessian 的**正定性**。三种常见方法：(1) 算特征值；(2) 算顺序主子式；(3) 配方。选最快的做。

---

## 思维路径还原（解题者的内心独白）

> "看到 $2\times 2$ Hessian，最快的方法是顺序主子式：$\Delta_1 = 5 > 0$，$\Delta_2 = 5\times 2 - 4 = 6 > 0$，两个顺序主子式全正 → $H \succ 0$ → 正定。
>
> **结论**：$\boldsymbol{\theta}^*$ 是**局部极小值**（损失面在该点各方向均上弯，形成碗形）。
>
> **求特征值（为了学习率）**：$\text{tr}(H) = 7 = \lambda_1 + \lambda_2$，$\det(H) = 6 = \lambda_1\lambda_2$，特征方程 $\lambda^2 - 7\lambda + 6 = 0$，根为 $\lambda_1 = 1, \lambda_2 = 6$。
>
> **最优学习率**：梯度下降稳定要求 $\eta < 2/\lambda_{\max} = 2/6 = 1/3$；最优学习率（最快收敛）$\eta^* = 2/(\lambda_1 + \lambda_2) = 2/7 \approx 0.286$。
>
> **条件数**：$\kappa = \lambda_{\max}/\lambda_{\min} = 6/1 = 6$，不算太大，梯度下降应能较快收敛（理论收敛率 $\rho^* = (6-1)/(6+1) = 5/7 \approx 0.71$）。
>
> **延伸思考**：若 $H = \begin{pmatrix}5 & 3 \\ 3 & 2\end{pmatrix}$，$\Delta_2 = 10 - 9 = 1 > 0$，仍正定；若 $H = \begin{pmatrix}5 & 4 \\ 4 & 2\end{pmatrix}$，$\Delta_2 = 10 - 16 = -6 < 0$，不定——$\boldsymbol{\theta}^*$ 是鞍点，需要 SGD 噪声逃脱。"
...
![正定 / 鞍点 / 半正定三种情形对比](../figures/svg/la-p8-23-1.svg)

### 图 23-2：Hessian 与凹凸

![凸函数 / 鞍点 / 凹函数与 Hessian 关系](../figures/svg/la-p8-23-2.svg)
...
| 名称 | 公式 / 方法 | 关键注意 |
|---|---|---|
| **矩阵表示** | $Q(\mathbf{x}) = \mathbf{x}^T A \mathbf{x}$，$A = A^T$ | $a_{ij} = $ 交叉项系数的 $1/2$ |
| **正交标准形** | $\mathbf{x} = Q\mathbf{y}$ → $\sum \lambda_i y_i^2$ | 系数正好是特征值 |
| **配方标准形** | 逐步消去交叉项 | 系数未必是特征值，但正/负个数不变 |
| **正定判定** | $\Delta_1 > 0,\ldots,\Delta_n > 0$ | 最快：顺序主子式（Sylvester 准则）|
| **负定判定** | $\Delta_1 < 0, \Delta_2 > 0, \Delta_3 < 0,\ldots$（交替变号）| 注意符号规律 |
| **Rayleigh 商** | $R_A(\mathbf{x}) = \mathbf{x}^T A \mathbf{x}/\|\mathbf{x}\|^2$ | 极值 = $\lambda_{\min}$（或 $\lambda_{\max}$），在特征向量处取到 |
| **合同不变量** | 符号差 $(p,q)$ | 正/负惯性指数，与线性变换无关 |

### 正定性快速判断 3 步流程

1. **写矩阵**：读取系数，对角元直接用，交叉项系数除以 $2$
2. **算顺序主子式**：$\Delta_1 = a_{11}$，$\Delta_2 = \det\begin{pmatrix}a_{11}&a_{12}\\a_{21}&a_{22}\end{pmatrix}$，…，$\Delta_n = \det(A)$
3. **判断**：全正 → 正定；全 $\geq 0$（含零）→ 半正定；交替变号 → 负定；其他 → 不定

### 标准化两条路

**路径 A（正交变换）**：① 求 $A$ 的特征值 $\lambda_i$；② 求对应标准正交特征向量 $\mathbf{q}_i$；③ $Q = [\mathbf{q}_1 \vert \cdots \vert \mathbf{q}_n]$，变换 $\mathbf{x} = Q\mathbf{y}$；④ 标准形 $\sum \lambda_i y_i^2$。

**路径 B（配方法）**：从含 $x_1$ 的项开始配平方，消去与 $x_1$ 的交叉项，递推处理 $x_2, x_3,\ldots$。结果的系数正负个数与路径 A 一致（惯性定理）。
...
要证明正定：优先用**顺序主子式**（免求特征值）；对 $2\times 2$ 矩阵用 $\text{tr} > 0$ 且 $\det > 0$（等价于特征值全正）。

要证明半正定：用 $A = B^TB$ 的分解形式（$A$ 是某矩阵的 Gram 矩阵），特别适合对协方差矩阵的半正定证明。

### 变形 2：不等式与 Rayleigh 商

许多二次型不等式本质是 Rayleigh 商的放缩：对任意 $\mathbf{x} \neq \mathbf{0}$，
$$\lambda_{\min} \|\mathbf{x}\|^2 \leq \mathbf{x}^T A \mathbf{x} \leq \lambda_{\max} \|\mathbf{x}\|^2$$
用于估计二次型的上下界或给出梯度下降的收敛保证（强凸 / Lipschitz 梯度）。

### 变形 3：合同标准形与秩

两个二次型合同（$A \simeq B$，即 $\exists$ 非退化 $C$ 使 $B = C^TAC$）$\Leftrightarrow$ 它们有相同的**秩**和**正惯性指数**。验证合同只需比较 $(p, q)$ 而无需显式求变换矩阵。

### 变形 4：混合形式 $\mathbf{x}^T A \mathbf{x} + \mathbf{b}^T\mathbf{x} + c$

通过配方转化为"纯二次型 + 常数"：令 $\boldsymbol{\mu} = -A^{-1}\mathbf{b}/2$（$A \succ 0$ 时的极小值点），则

$$\mathbf{x}^T A \mathbf{x} + \mathbf{b}^T\mathbf{x} + c = (\mathbf{x} - \boldsymbol{\mu})^T A (\mathbf{x} - \boldsymbol{\mu}) + c - \frac{1}{4}\mathbf{b}^T A^{-1}\mathbf{b}$$

这是"完全配方"，直接给出极小值位置和极小值。
...
| 概念 | 关键内容 |
|---|---|
| **二次型定义** | $Q(\mathbf{x}) = \mathbf{x}^T A\mathbf{x}$，$A = A^T$ 实对称矩阵 |
| **正交标准化** | 正交变换 $\mathbf{x} = Q\mathbf{y}$ → $\sum \lambda_i y_i^2$，消去所有交叉项 |
| **惯性定理** | 正惯性指数 $p$（正系数数）、负惯性指数 $q$ 是不变量，与变换选择无关 |
| **正定等价条件** | 特征值全正 = 顺序主子式全正 = Cholesky 分解存在 = $A = B^TB$（$B$ 列满秩）|
| **Rayleigh 商** | $\min_{\vert\mathbf{x}\vert =1}\mathbf{x}^TA\mathbf{x} = \lambda_{\min}$；$\max = \lambda_{\max}$，在特征向量处取到 |
| **损失函数应用** | Hessian $H \succ 0$ → 极小值；$H$ 不定 → 鞍点；$\kappa(H)$ → 收敛速度 |

**核心要点**：二次型把矩阵的"方向—大小"信息完整编码；正定性是凸优化的数学基础；Rayleigh 商把约束优化与特征值统一起来。

---

## 思考路标（条件反射）

1. 看到"二次型" → 写成 $\mathbf{x}^T A \mathbf{x}$，$A$ 实对称，交叉项系数除以 $2$
2. 看到"判断正定" → 顺序主子式（最快）：$\Delta_1 > 0, \ldots, \Delta_n > 0$
3. 看到"化标准形" → 正交变换法（保距离，系数是特征值）或配方法（任意非退化变换）
4. 看到"惯性指数" → 正特征值个数 $p$，负特征值个数 $q$；合同不变
5. 看到"Rayleigh 商 $\mathbf{x}^TA\mathbf{x}/\|\mathbf{x}\|^2$" → 极值在特征向量处取到，值是特征值
6. 看到"Hessian 矩阵" → 二次型矩阵，正定 → 极小，不定 → 鞍点，负定 → 极大
7. 看到"学习率上界" → $\eta < 2/\lambda_{\max}(H)$（不能超过最大曲率的倒数的两倍）
8. 看到"梯度下降慢" → 条件数 $\kappa = \lambda_{\max}/\lambda_{\min}$ 大；考虑预条件化或 Adam
9. 看到"椭圆主轴" → $A$ 的特征向量方向，半轴长 $1/\sqrt{\lambda_i}$
10. 看到"$A \simeq B$（合同）" → 比较秩和正惯性指数 $(p, q)$，两者相等则合同

---

## 易错点

1. **交叉项系数要除以 $2$**：$Q = 3x_1^2 + 4x_1x_2 - x_2^2$ 的矩阵中 $a_{12} = a_{21} = 2$（不是 $4$）。写错为 $a_{12} = 4$ 是最常见低级错误，导致后续一切计算错误。

2. **正定不等于行列式大于零**：$\det(A) > 0$ 只是正定的必要条件，不充分。$A = \begin{pmatrix}-1&0\\0&-1\end{pmatrix}$ 的 $\det = 1 > 0$ 但 $A \prec 0$（负定）。必须**所有顺序主子式**均为正。

3. **标准形系数不等于特征值（用配方时）**：配方法得到的标准形系数与所选变换有关，可以是任意正实数（不一定是 $A$ 的特征值）。但**正系数的个数**（正惯性指数）是不变的，这才是惯性定理的内容。

4. **Rayleigh 商只在单位球面上取极值**：$\mathbf{x}^TA\mathbf{x}$ 本身可在 $\mathbf{x}\to\infty$ 时趋于无穷，没有全局最大值；$\lambda_{\max}$ 是**约束在 $\|\mathbf{x}\|=1$ 时**的最大值。不要忘记归一化约束。

5. **鞍点 $\neq$ 不可导点**：鞍点是梯度为零但不是极值的临界点，Hessian 不定（有正有负特征值）。高维网络中几乎所有"坏临界点"都是鞍点，而非局部极大值（后者要求 Hessian 全部负定，概率指数小）。
...
> **题目**：$Q = 3x_1^2 + 2x_1x_2 + x_2^2$，判断正定性，并用配方法写出标准形。

【思路】先写矩阵，用顺序主子式判断，再配方。

【解】
矩阵：$A = \begin{pmatrix}3&1\\1&1\end{pmatrix}$（$a_{12} = 2/2 = 1$）。

顺序主子式：$\Delta_1 = 3 > 0$，$\Delta_2 = 3-1 = 2 > 0$，**正定**。

配方：$Q = 3(x_1 + \frac{1}{3}x_2)^2 + (1 - \frac{1}{3})x_2^2 = 3(x_1+\frac{1}{3}x_2)^2 + \frac{2}{3}x_2^2$。

令 $y_1 = x_1 + x_2/3$，$y_2 = x_2$，则标准形 $Q = 3y_1^2 + \frac{2}{3}y_2^2$（两正系数，正定确认）。

【答案】$\boxed{Q = 3y_1^2 + \frac{2}{3}y_2^2}$（正定，符号差 $(2,0)$）。

【注】配方系数 $3, 2/3$ 与特征值 $\lambda = 2\pm\sqrt{2}$ 不同，但正系数个数（均为 $2$）相同——惯性定理的体现。

### 例 2：Rayleigh 商求约束极值

> **题目**：$A = \begin{pmatrix}4 & 1 \\ 1 & 2\end{pmatrix}$，求 $\max_{\|\mathbf{x}\|=1}\mathbf{x}^TA\mathbf{x}$ 的值和最优解方向。

【思路】Rayleigh 商极值定理：最大值 = $\lambda_{\max}$，最优方向 = 对应特征向量。

【解】
特征方程：$(4-\lambda)(2-\lambda) - 1 = \lambda^2 - 6\lambda + 7 = 0$，$\lambda = 3 \pm \sqrt{2}$。

$\lambda_{\max} = 3 + \sqrt{2} \approx 4.414$。

对应特征向量：$(A - \lambda_{\max}I)\mathbf{q} = 0$：$(4 - 3 - \sqrt{2})q_1 + q_2 = 0$，$q_2 = (\sqrt{2}-1)q_1$。归一化后 $\mathbf{q}_{\max} = \dfrac{1}{\sqrt{4-2\sqrt{2}}}(1, \sqrt{2}-1)^T$。

【答案】最大值 $\boxed{3+\sqrt{2}}$，在 $\mathbf{x} = \mathbf{q}_{\max}$ 方向取到（椭圆的短轴方向曲率最大）。

### 例 3：Hessian 分析与梯度下降

> **题目**：损失函数 $\mathcal{L}(\theta_1,\theta_2) = 5\theta_1^2 + 6\theta_1\theta_2 + 5\theta_2^2 - 4\theta_1 - 2\theta_2$。(1) 求极小值点；(2) 分析 Hessian，求最优学习率和条件数；(3) 判断梯度下降是否会出现"锯齿"现象。

【思路】完整流程：梯度为零求极小值 → Hessian 正定验证 → 特征值分析学习率。

【解】
(1) $\nabla\mathcal{L} = \begin{pmatrix}10\theta_1+6\theta_2-4 \\ 6\theta_1+10\theta_2-2\end{pmatrix} = \mathbf{0}$，解得 $\theta_1^* = \frac{7}{16}$，$\theta_2^* = -\frac{1}{16}$。

(2) Hessian $H = \begin{pmatrix}10&6\\6&10\end{pmatrix}$。顺序主子式：$\Delta_1 = 10 > 0$，$\Delta_2 = 100-36=64 > 0$，$H \succ 0$，极小值确认。

特征值：$\lambda = 10 \pm 6$，即 $\lambda_1 = 4, \lambda_2 = 16$。

最优学习率：$\eta^* = 2/(\lambda_1+\lambda_2) = 2/20 = 0.1$。

条件数：$\kappa = 16/4 = 4$。

(3) 收敛率 $\rho^* = (4-1)/(4+1) = 3/5 = 0.6$，条件数为 $4$（中等）。梯度下降会有轻微锯齿（椭圆略扁，长轴约是短轴的 $4$ 倍），但不严重。

【答案】极小值点 $\boldsymbol{\theta}^* = (7/16, -1/16)^T$；最优学习率 $0.1$，条件数 $\kappa = 4$，轻微锯齿。

---

## 自测题

**自测 1**　$Q = x_1^2 + 4x_1x_2 + 5x_2^2$。(1) 写出矩阵 $A$；(2) 用顺序主子式判断正定性；(3) 求特征值，与 (2) 结论一致吗？

> 提示：$A = \begin{pmatrix}1&2\\2&5\end{pmatrix}$。$\Delta_1 = 1 > 0$，$\Delta_2 = 5-4=1>0$，正定。特征值 $\lambda = 3\pm\sqrt{5} > 0$，一致。

**自测 2**　$Q = x_1^2 - 2x_1x_2 - x_2^2$。判断类型，并找出一个使 $Q > 0$ 和一个使 $Q < 0$ 的具体向量。

> 提示：$A = \begin{pmatrix}1&-1\\-1&-1\end{pmatrix}$，$\Delta_2 = -1-1=-2<0$，不定。$\mathbf{x}=(1,0)^T$：$Q=1>0$；$\mathbf{x}=(0,1)^T$：$Q=-1<0$。

**自测 3**　两矩阵 $A = \begin{pmatrix}2&0\\0&-3\end{pmatrix}$ 与 $B = \begin{pmatrix}5&4\\4&-1\end{pmatrix}$ 是否合同？

> 提示：$A$ 的符号差：$(p,q)=(1,1)$，秩 $2$。$B$ 的 $\det = -5-16=-21<0$，$\Delta_1=5>0$，故 $B$ 有正有负特征值，$(p,q)=(1,1)$，秩 $2$。两者符号差相同，**合同**。

**自测 4**　Hessian $H = \begin{pmatrix}2&1\\1&2\end{pmatrix}$，特征值为 $1$ 和 $3$。(1) 判断临界点类型；(2) 学习率上界是多少？(3) 最优学习率 $\eta^*$ 是多少？

> 提示：(1) $H \succ 0$（特征值全正），局部极小值。(2) 稳定要求 $\eta < 2/\lambda_{\max} = 2/3$。(3) $\eta^* = 2/(\lambda_1+\lambda_2) = 2/4 = 0.5$。条件数 $\kappa = 3$，收敛率 $\rho^* = 1/2$。

**自测 5**　用 Rayleigh 商证明：对正定矩阵 $A \succ 0$ 和任意非零向量 $\mathbf{x}$，有 $\dfrac{1}{\lambda_{\max}} \leq \dfrac{\|\mathbf{x}\|^2}{\mathbf{x}^TA\mathbf{x}} \leq \dfrac{1}{\lambda_{\min}}$。

> 提示：由 Rayleigh 商不等式 $\lambda_{\min}\|\mathbf{x}\|^2 \leq \mathbf{x}^TA\mathbf{x} \leq \lambda_{\max}\|\mathbf{x}\|^2$，各项除以 $\mathbf{x}^TA\mathbf{x}\|\mathbf{x}\|^2 > 0$ 即得。这是分析预条件梯度下降收敛性的基础不等式。

---

**回头看一眼"一例速记"**：

> 二次型 $\mathbf{x}^TA\mathbf{x}$，$A$ 实对称；交叉项系数除以 $2$。
> 正定：顺序主子式全正；不定：Hessian 有鞍点方向。
> Rayleigh 商极值 = 特征值，在特征向量处取到。

如果现在不看笔记，能独立完成例 1 + 例 3 + 自测 1 + 自测 4——本章，你拿下了。
...
| 学习目标 + 23.1–23.5 严格正文 | 原版 | 完整定义与推导 |
| 本章小结 | 融合 | 公式速查 |
