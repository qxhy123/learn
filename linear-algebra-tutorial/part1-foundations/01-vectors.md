...
> **一例速记**：
> **向量加减**：对应分量加减；维度必须相同。$\mathbf{u}+\mathbf{v}$：首尾相接；$\mathbf{u}-\mathbf{v}$：从 $\mathbf{v}$ 终点指向 $\mathbf{u}$ 终点。
> **范数**：$\|\mathbf{v}\|_2 = \sqrt{\sum_i v_i^2}$；非负、齐次、三角不等式。**归一化**：$\hat{\mathbf{v}} = \mathbf{v}/\|\mathbf{v}\|$，长度为 1 只保留方向。
> **内积**：$\mathbf{u}\cdot\mathbf{v} = \sum_i u_i v_i = \|\mathbf{u}\|\|\mathbf{v}\|\cos\theta$，结果为**标量**。正交：内积为 0。
> **余弦相似度**：$\cos\theta = \dfrac{\mathbf{u}\cdot\mathbf{v}}{\|\mathbf{u}\|\|\mathbf{v}\|}$，范围 $[-1,1]$，深度学习中衡量词向量 / 特征相似性的核心工具。
> **AI 关联**：词嵌入 = 高维实数向量；余弦相似度 = 语义距离；全连接层输出 = 一批内积。

---

## 引入：词向量的余弦相似度

> **题目**：设"猫"和"狗"的词向量分别为 $\mathbf{u} = (1, 2, 0)^T$ 和 $\mathbf{v} = (0, 2, 1)^T$（简化示意）。求它们的余弦相似度，并判断语义是否相近。

请先停下来想一想：要比较两个词的"方向相似性"，只看数值大小不够——两个长度不同的向量，绝对内积会被长度放大。**归一化后计算内积**（即余弦相似度）才能剔除长度干扰，只保留方向信息。下面还原完整解题思路。

---

## 思维路径还原（解题者的内心独白）

> "题目给了两个向量，要求余弦相似度。我的公式是 $\cos\theta = \dfrac{\mathbf{u}\cdot\mathbf{v}}{\|\mathbf{u}\|\|\mathbf{v}\|}$，三步走：内积、范数、相除。
>
> **第一步：算内积**。$\mathbf{u}\cdot\mathbf{v} = 1\times 0 + 2\times 2 + 0\times 1 = 4$。
>
> **第二步：算范数**。$\|\mathbf{u}\| = \sqrt{1^2+2^2+0^2} = \sqrt{5}$；$\|\mathbf{v}\| = \sqrt{0^2+2^2+1^2} = \sqrt{5}$。
>
> **第三步：相除**。$\cos\theta = \dfrac{4}{\sqrt{5}\cdot\sqrt{5}} = \dfrac{4}{5} = 0.8$。
>
> $0.8$ 接近 1，说明两向量夹角约 $37°$，**方向高度相似**——在词嵌入空间里，"猫"和"狗"确实是近义的动物类词汇。
>
> **延伸思考**：如果题目改为计算"猫"与"汽车"的相似度，预期余弦值会接近 0（无关词）甚至负数（对立词）。余弦相似度不受向量缩放影响：把 $\mathbf{u}$ 乘以 100，分子分母同乘 100，结果不变——这正是归一化的意义。"
...
![平行四边形法则与三角形法则示意](../figures/svg/la-p1-01-1.svg)

### 图 1-2：内积的投影几何

![向量内积与投影几何示意，含夹角和公式](../figures/svg/la-p1-01-2.svg)

### 图 1-3：$\ell_1/\ell_2/\ell_\infty$ 单位球

![三种范数下的单位球对比（菱形 / 圆 / 正方形）](../figures/svg/la-p1-01-3.svg)
...
| 名称 | 公式 | 关键性质 |
|---|---|---|
| **向量加法** | $(\mathbf{u}+\mathbf{v})_i = u_i + v_i$ | 维度相同才能加 |
| **标量乘法** | $(c\mathbf{v})_i = cv_i$ | $c<0$ 方向翻转 |
| **$\ell_2$ 范数** | $\|\mathbf{v}\| = \sqrt{\sum_i v_i^2}$ | 非负、齐次、三角不等式 |
| **归一化** | $\hat{\mathbf{v}} = \mathbf{v}/\|\mathbf{v}\|$ | 结果长度为 1，只保留方向 |
| **内积** | $\mathbf{u}\cdot\mathbf{v} = \sum_i u_i v_i$ | 结果为标量；$\mathbf{v}\cdot\mathbf{v}=\|\mathbf{v}\|^2$ |
| **夹角公式** | $\cos\theta = \dfrac{\mathbf{u}\cdot\mathbf{v}}{\|\mathbf{u}\|\|\mathbf{v}\|}$ | $\theta\in[0,\pi]$；正交时 $=0$ |
| **正交条件** | $\mathbf{u}\cdot\mathbf{v}=0$ | $\Leftrightarrow$ 方向独立（垂直） |

### 求余弦相似度 3 步

1. **算内积**：$\mathbf{u}\cdot\mathbf{v} = \sum_i u_i v_i$
2. **算范数**：$\|\mathbf{u}\|=\sqrt{\sum u_i^2}$，$\|\mathbf{v}\|=\sqrt{\sum v_i^2}$
3. **相除**：$\cos\theta = \dfrac{\mathbf{u}\cdot\mathbf{v}}{\|\mathbf{u}\|\|\mathbf{v}\|}$

### 判断正交 2 步

1. 计算内积 $\mathbf{u}\cdot\mathbf{v}$
2. 等于 0 则正交，不等于 0 则不正交
...
公式完全相同，只是求和项更多。$\mathbb{R}^{768}$ 维词向量与二维向量用同一套公式。

### 变形 2：$\ell_p$ 范数

除 $\ell_2$ 外还有 $\ell_1$ 范数 $\|\mathbf{v}\|_1 = \sum_i \vert v_i\vert$（正则化常用）和 $\ell_\infty$ 范数 $\|\mathbf{v}\|_\infty = \max_i \vert v_i\vert$。$\ell_2$ 是"默认范数"，其余需显式标注下标。

### 变形 3：归一化再内积 = 余弦相似度

$\hat{\mathbf{u}}\cdot\hat{\mathbf{v}} = \dfrac{\mathbf{u}\cdot\mathbf{v}}{\|\mathbf{u}\|\|\mathbf{v}\|} = \cos\theta$。因此**先归一化，再算内积**与直接用余弦相似度公式等价。PyTorch 的 `F.cosine_similarity` 内部正是这样实现的。

### 变形 4：向量投影

$\mathbf{u}$ 在 $\mathbf{v}$ 方向上的投影长度为 $\dfrac{\mathbf{u}\cdot\mathbf{v}}{\|\mathbf{v}\|}$，投影向量为 $\dfrac{\mathbf{u}\cdot\mathbf{v}}{\|\mathbf{v}\|^2}\mathbf{v}$。这是 Gram-Schmidt 正交化和注意力机制的基础操作。

---

## 思考路标（条件反射）

1. 看到"向量相加" → 检查维度是否相同，否则无定义
2. 看到"向量长度 / 模" → 用 $\ell_2$ 范数 $\sqrt{\sum v_i^2}$
3. 看到"只保留方向" → 归一化 $\hat{\mathbf{v}} = \mathbf{v}/\|\mathbf{v}\|$
4. 看到"内积 = 0" → 两向量正交（垂直），信息无重叠
5. 看到"求夹角" → 三步法：内积 → 两范数 → 相除取 $\arccos$
6. 看到"余弦相似度" → 等价于归一化后内积，范围 $[-1, 1]$
7. 看到"词向量相似" → 余弦相似度（不用欧氏距离，因向量长度不可比）
8. 看到"全连接层输出" → $\mathbf{y} = W\mathbf{x}+\mathbf{b}$，每行是一次内积
9. 看到 $\mathbf{v}\cdot\mathbf{v}$ → 等于 $\|\mathbf{v}\|^2$，不要再开根号
10. 看到"单位向量" → 长度为 1，归一化的结果；用 $\hat{}$ 符号标记

---

## 易错点

1. **维度不匹配就相加**：$\mathbb{R}^2$ 中的向量不能与 $\mathbb{R}^3$ 中的向量相加；PyTorch 会报 `RuntimeError: size mismatch`。检查维度是防止此错的唯一手段。

2. **内积结果是标量，不是向量**：$\mathbf{u}\cdot\mathbf{v}\in\mathbb{R}$，写成向量是严重错误。$\mathbf{u}\cdot\mathbf{v}$ 与 $\mathbf{u}\otimes\mathbf{v}$（外积）有本质区别，后者是矩阵。

3. **把 $\|\mathbf{v}\|$ 写成 $\sqrt{\mathbf{v}\cdot\mathbf{v}}$ 忘了开根号**：常见粗心——$\mathbf{v}\cdot\mathbf{v}=\|\mathbf{v}\|^2$，是长度的**平方**；要得长度还需开根。

4. **余弦相似度与欧氏距离混用**：两词向量欧氏距离小 $\neq$ 余弦相似度大（比如一个向量是另一个的 100 倍，欧氏距离很大但余弦相似度 = 1）。NLP 中统一用余弦相似度；几何距离问题才用欧氏距离。

5. **正交 $\neq$ 线性无关**（此处预备概念）：两非零正交向量一定线性无关，但线性无关不要求正交。后续章节中不要把两者混淆。
...
> **题目**：设 $\mathbf{a} = (3, 0, 4)^T$，$\mathbf{b} = (0, 5, 0)^T$。(1) 将 $\mathbf{a}$ 归一化；(2) 判断 $\mathbf{a}$ 与 $\mathbf{b}$ 是否正交。

【思路】归一化 = 除以 $\ell_2$ 范数；正交 = 内积为 0。

【解】
(1) $\|\mathbf{a}\| = \sqrt{9+0+16} = \sqrt{25} = 5$，故 $\hat{\mathbf{a}} = \dfrac{1}{5}(3,0,4)^T = (0.6, 0, 0.8)^T$。

(2) $\mathbf{a}\cdot\mathbf{b} = 3\times 0 + 0\times 5 + 4\times 0 = 0$，内积为 0，$\mathbf{a}\perp\mathbf{b}$，**正交**。

【答案】$\boxed{\hat{\mathbf{a}} = (0.6,\ 0,\ 0.8)^T;\ \mathbf{a}\perp\mathbf{b}}$。

【注】$\mathbf{b}$ 仅有 $y$ 分量，$\mathbf{a}$ 的 $y$ 分量为 0，内积天然为零——正交的几何直觉是两向量分别在不同坐标平面内"各自为政"。

### 例 2：余弦相似度 + 夹角

> **题目**：$\mathbf{p} = (1, 1, 0)^T$，$\mathbf{q} = (1, 0, 1)^T$。求余弦相似度及夹角 $\theta$。

【思路】三步法：内积 → 范数 → 相除。

【解】
内积：$\mathbf{p}\cdot\mathbf{q} = 1+0+0 = 1$。

范数：$\|\mathbf{p}\| = \sqrt{2}$，$\|\mathbf{q}\| = \sqrt{2}$。

余弦：$\cos\theta = \dfrac{1}{\sqrt{2}\cdot\sqrt{2}} = \dfrac{1}{2}$，故 $\theta = 60°$。

【答案】$\boxed{\cos\theta = 0.5,\ \theta = 60°}$。

【注】两向量各含两个非零分量，各自有一个分量独占（$p_3=0,\ q_2=0$），夹角 $60°$ 是合理的"中等相似"。

### 例 3：向量组合 + 范数

> **题目**：$\mathbf{u} = (2, -1, 3)^T$，$\mathbf{v} = (-1, 4, 2)^T$。计算 $\|2\mathbf{u} - \mathbf{v}\|$。

【思路】先算线性组合，再算范数。

【解】
$2\mathbf{u} - \mathbf{v} = (4, -2, 6)^T - (-1, 4, 2)^T = (5, -6, 4)^T$。

$\|2\mathbf{u}-\mathbf{v}\| = \sqrt{25+36+16} = \sqrt{77} \approx 8.775$。

【答案】$\boxed{\sqrt{77}}$。

【注】范数不能"分配"到加法上：$\|\mathbf{u}+\mathbf{v}\| \neq \|\mathbf{u}\| + \|\mathbf{v}\|$（仅三角不等式 $\leq$）。必须先算好向量再取范数。

---

## 自测题

**自测 1**　设 $\mathbf{a} = (1, 2, 2)^T$。求 $\|\mathbf{a}\|$，并写出归一化向量 $\hat{\mathbf{a}}$。

> 💡 提示：$\|\mathbf{a}\| = \sqrt{1+4+4} = 3$；$\hat{\mathbf{a}} = (1/3,\ 2/3,\ 2/3)^T$。验证：$\|\hat{\mathbf{a}}\|^2 = 1/9+4/9+4/9 = 1$。

**自测 2**　$\mathbf{u} = (1, 0, -1)^T$，$\mathbf{v} = (2, k, 1)^T$，若 $\mathbf{u}\perp\mathbf{v}$，求 $k$。

> 💡 提示：$\mathbf{u}\cdot\mathbf{v} = 2 + 0\cdot k + (-1) = 1 \neq 0$……等等，正确算法：$\mathbf{u}\cdot\mathbf{v} = 1\times 2 + 0\times k + (-1)\times 1 = 2-1=1$。为使正交，需要 $1=0$——这不可能！因此对任意 $k$，$\mathbf{u}$ 与 $\mathbf{v}$ 不正交（$k$ 所在项系数为 0，不影响内积）。**结论：不存在满足条件的 $k$**。这是一道"陷阱题"，说明并非任意修改分量都能使向量正交。

**自测 3**　证明：对任意向量 $\mathbf{u}, \mathbf{v}\in\mathbb{R}^n$，$\|\mathbf{u}-\mathbf{v}\|^2 = \|\mathbf{u}\|^2 - 2(\mathbf{u}\cdot\mathbf{v}) + \|\mathbf{v}\|^2$。

> 💡 提示：展开 $(\mathbf{u}-\mathbf{v})\cdot(\mathbf{u}-\mathbf{v})$，利用分配律和 $\mathbf{x}\cdot\mathbf{x}=\|\mathbf{x}\|^2$，中间项用交换律合并。这是余弦定理的向量版。

**自测 4**　词向量 $\mathbf{a} = (1, 1, 0, 0)^T$，$\mathbf{b} = (0, 0, 1, 1)^T$。求余弦相似度，并解释语义含义。

> 💡 提示：$\mathbf{a}\cdot\mathbf{b}=0$，$\cos\theta=0$，夹角 $90°$——两词**语义完全无关**（正交）。这正是词嵌入空间中"语义独立"的数学表达：非零向量内积为零。

**自测 5**　设 $\mathbf{w} = (3, 4)^T$ 是某神经元的权重向量。当输入 $\mathbf{x}$ 的范数固定为 $\|\mathbf{x}\| = 2$ 时，使神经元输出 $\mathbf{w}\cdot\mathbf{x}$ 最大的 $\mathbf{x}$ 方向是什么？最大值是多少？

> 💡 提示：$\mathbf{w}\cdot\mathbf{x} = \|\mathbf{w}\|\|\mathbf{x}\|\cos\theta$，$\cos\theta$ 最大时 $\theta=0$，即 $\mathbf{x}$ 与 $\mathbf{w}$ 同向。最大值 $= \|\mathbf{w}\|\|\mathbf{x}\| = 5\times 2 = 10$。**权重向量 $\mathbf{w}$ 就是该神经元最敏感的"特征方向"**。

---

**回头看一眼"一例速记"**：

> 向量加减逐元素、维度相同才有效；范数 = 模 = $\sqrt{\sum v_i^2}$；归一化只留方向。
> 内积 = 标量 = $\sum u_i v_i = \|\mathbf{u}\|\|\mathbf{v}\|\cos\theta$；内积为零 = 正交。
> 余弦相似度 = 内积除以两范数之积，范围 $[-1,1]$，是词向量相似度的核心工具。
