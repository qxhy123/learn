...
> **难度**：★★★☆☆
> **前置知识**：第9章（向量空间）、第10章（线性相关）
> **本文件**：融合"原版严格推导 + 速记 / 套路 / 自测"。保留原版完整正文（学习目标 / 11.1–11.4 / 深度学习应用 / 练习题）+ 在最前置一例速记 / 思维路径还原 + 最后追加方法总结与自测。

> 基是向量空间的"坐标系骨架"，维数是空间"自由度"的精确度量。掌握这两个概念，是理解线性代数深层结构的关键一步。

> **一例速记**：
> **基**：线性无关 + 张成整个空间（两个条件缺一不可）。
> **维数**：任意一个基的向量个数——不同的基大小相同，这是维数良定义的保证。
> **坐标**：向量在给定基下的唯一线性组合系数；求坐标 = 解线性方程组 $[b_1\mid\cdots\mid b_n]\mathbf{c} = \mathbf{x}$。
> **秩-零化度定理**：$\text{rank}(A) + \text{nullity}(A) = n$（列数），是分析解结构的核心工具。
> **AI 关联**：PCA = 寻找数据方差最大的正交基（主成分）；低维嵌入 = 在 $k$ 维子空间中重新坐标化（$k \ll n$）。

---

## 引入：坐标系的选择影响表达效率

> **题目**：同一个向量 $\mathbf{x} = (7, 11)^T \in \mathbb{R}^2$，在标准基 $\mathcal{E} = \{(1,0)^T, (0,1)^T\}$ 下坐标为 $(7, 11)$。现有另一个基 $\mathcal{B} = \{b_1 = (1,2)^T, b_2 = (3,5)^T\}$。求 $\mathbf{x}$ 在 $\mathcal{B}$ 下的坐标。
>
> 为什么不同坐标系下同一向量的坐标不同？

请先停下来想一想：向量 $\mathbf{x}$ 本身没有变，只是"描述它所用的尺子"换了——这正是**坐标与基的关系**。PCA 就是找到一组特殊的基（主成分方向），让数据的坐标表示"最紧凑"。

---

## 思维路径还原（解题者的内心独白）

> "求 $\mathbf{x}$ 在基 $\mathcal{B}$ 下的坐标，就是求 $c_1 b_1 + c_2 b_2 = \mathbf{x}$，即解线性方程组。
>
> **建立方程**：$c_1(1,2)^T + c_2(3,5)^T = (7,11)^T$
> $$\begin{cases} c_1 + 3c_2 = 7 \\ 2c_1 + 5c_2 = 11 \end{cases}$$
>
> **增广矩阵行化简**：
...
>
> **关键直觉**：坐标 $(-2, 3)$ 是在 $\mathcal{B}$ 这把"尺子"下对 $\mathbf{x}$ 的度量——数字变了，但 $\mathbf{x}$ 本身没变。PCA 做的事情也一样：找到新基（主成分），在新基下大部分信息浓缩在前几个坐标里，后面的坐标接近 0，就可以安全丢弃了。"
...
![同一 2D 平面的两组基（标准基 vs 一般基）都张成相同空间](../figures/svg/la-p4-11-1.svg)
...
|---|---|---|
| **基** | 线性无关 $+$ 张成整个空间 | 两个条件缺一不可 |
| **维数** | 任意基中向量个数 | 不同基大小相同（良定义） |
| **坐标** | $[b_1\vert\cdots\vert b_n]\mathbf{c}=\mathbf{x}$ 的解 $\mathbf{c}$ | 给定基后唯一确定 |
| **列空间基** | 行化简，取**原矩阵**主元列 | 不是行阶梯形的列，是原列 |
| **行空间基** | 行化简后的**非零行** | 行变换不改变行空间 |
| **零空间基** | 解 $A\mathbf{x}=\mathbf{0}$，自由变量参数化 | 自由变量个数 $=$ 零空间维数 |
| **秩-零化度** | $\text{rank}(A)+\text{nullity}(A)=n$ | $n$ 是列数（不是行数） |

### 验证基：标准 2 步

1. **线性无关**：行化简矩阵，秩 $= k$（向量个数）则无关
2. **张成空间**：对于 $n$ 维空间，$n$ 个线性无关向量自动张成（用"维数 $n$，无关向量 $n$ 个 → 基"推论）

**高效快捷方式**：在 $\mathbb{R}^n$ 中，$n$ 个向量构成基 $\Leftrightarrow$ 它们线性无关 $\Leftrightarrow$ 组成的方阵行列式非零 $\Leftrightarrow$ 矩阵可逆。
...
若 $\mathcal{B}_1$ 和 $\mathcal{B}_2$ 是同一空间的两个基，坐标变换矩阵 $P_{\mathcal{B}_1 \to \mathcal{B}_2}$ 由解方程组 $[B_2 \mid B_1]$ 行化简得到。深度学习中的投影层本质上是坐标变换。

### 变形 2：扩充线性无关集为基

给定 $k$ 个线性无关向量，将其与标准基向量合并成矩阵，行化简，选出 $n$ 个主元列，即为一个基。

### 变形 3：秩-零化度定理的推论

- 若 $A$ 是 $m \times n$ 矩阵：
  - $\text{rank}(A) = n$（列满秩）$\Leftrightarrow$ 零空间仅含零向量 $\Leftrightarrow$ $A\mathbf{x}=\mathbf{0}$ 只有零解
  - $\text{rank}(A) = m$（行满秩）$\Leftrightarrow$ $A\mathbf{x}=\mathbf{b}$ 对所有 $\mathbf{b}$ 有解

### 变形 4：子空间维数的估计

若 $W$ 由 $k$ 个向量张成，则 $\dim(W) \leq k$，等号当且仅当 $k$ 个向量线性无关。PCA 中，数据的"有效维数"就是协方差矩阵非零特征值的个数——通常远小于特征数 $n$。

---

## 思考路标（条件反射）

1. 看到"验证 $S$ 是否为基" → 两步：线性无关 + 张成（或：$n$ 个向量在 $\mathbb{R}^n$ 中，只需线性无关）
2. 看到"求列空间的基" → 行化简，取**原矩阵**主元列（注意不是行阶梯形的列）
...
10. 看到"扩充为基" → 现有向量 + 标准基向量合并 → 行化简选主元列

---

## 易错点

1. **求基时取了行阶梯形的列，而非原矩阵的列**：列空间的基必须从**原矩阵**的主元列中选取，行化简改变了列但没有改变主元列的"位置"（哪些列是主元列），不能把行阶梯形的列直接当作基。

2. **"基的大小唯一"不等于"基唯一"**：同一向量空间可以有无数不同的基，但任意两个基的**向量个数**相同，这个共同的数字叫做维数。混淆两点会误以为基只有一个。
...
> **题目**：设 $b_1=(1,1,0)^T$，$b_2=(1,0,1)^T$，$b_3=(0,1,1)^T$。(1) 证明 $\mathcal{B}=\{b_1,b_2,b_3\}$ 是 $\mathbb{R}^3$ 的基；(2) 求 $\mathbf{x}=(2,1,3)^T$ 在 $\mathcal{B}$ 下的坐标。

【解】

(1) 构造矩阵，算行列式：

$$\det\begin{pmatrix}1&1&0\\1&0&1\\0&1&1\end{pmatrix} = 1(0-1) - 1(1-0) + 0 = -1-1 = -2 \neq 0$$

行列式非零，3 个向量在 $\mathbb{R}^3$ 中线性无关，故构成 $\mathbb{R}^3$ 的一个基。

(2) 增广矩阵行化简：

$$\begin{pmatrix}1&1&0&2\\1&0&1&1\\0&1&1&3\end{pmatrix} \xrightarrow{R_2-R_1} \begin{pmatrix}1&1&0&2\\0&-1&1&-1\\0&1&1&3\end{pmatrix} \xrightarrow{R_3+R_2} \begin{pmatrix}1&1&0&2\\0&-1&1&-1\\0&0&2&2\end{pmatrix}$$
...
> **题目**：设 $A = \begin{pmatrix}1&2&1\\2&4&2\\3&6&3\end{pmatrix}$。不求零空间基向量，直接求 $\dim(\text{Null}(A))$。

【解】

观察：第2、3行分别是第1行的 2 倍和 3 倍，$\text{rank}(A) = 1$。

由秩-零化度定理（$n = 3$ 列）：

$$\dim(\text{Null}(A)) = 3 - 1 = 2$$

【答案】$\dim(\text{Null}(A)) = 2$（不计算任何基向量即得结论）。

### 例 3：求矩阵列空间的基

> **题目**：求 $A = \begin{pmatrix}1&3&2&-1\\2&6&5&0\\-1&-3&-1&2\end{pmatrix}$ 的列空间的一个基及维数。

【解】行化简：

$$A \xrightarrow{R_2-2R_1,\, R_3+R_1} \begin{pmatrix}1&3&2&-1\\0&0&1&2\\0&0&1&1\end{pmatrix} \xrightarrow{R_3-R_2} \begin{pmatrix}1&3&2&-1\\0&0&1&2\\0&0&0&-1\end{pmatrix}$$

主元在第 1、3、4 列，故取原矩阵的第 1、3、4 列构成列空间的基：

$$\mathcal{B}_{\text{Col}} = \left\{\begin{pmatrix}1\\2\\-1\end{pmatrix}, \begin{pmatrix}2\\5\\-1\end{pmatrix}, \begin{pmatrix}-1\\0\\2\end{pmatrix}\right\}$$
...
**自测 1**　判断 $\{(1,2,0)^T, (0,1,3)^T\}$ 是否构成 $\mathbb{R}^2$ 的子空间 $W = \text{Span}\{(1,2,0)^T,(0,1,3)^T\}$ 的基。

> 提示：两向量不成比例，线性无关；它们的张成即是 $W$，故构成 $W$ 的基，$\dim(W) = 2$。

**自测 2**　$A$ 是 $4 \times 6$ 矩阵，$\text{rank}(A) = 3$。列出 $\text{Col}(A)$、$\text{Row}(A)$、$\text{Null}(A)$、$\text{Null}(A^T)$ 的维数。
...
> 提示：保留前 2 个主成分（$\lambda_1, \lambda_2$ 远大于其余）。信息比例 $\approx \frac{50+30}{50+30+98\times 0.01} \approx \frac{80}{80.98} \approx 98.8\%$。维数从 100 降到 2，几乎无信息损失。

**自测 5**　设 $A$ 是 $n\times n$ 方阵。证明：$\text{rank}(A) = n \Leftrightarrow A$ 可逆 $\Leftrightarrow \det(A) \neq 0 \Leftrightarrow A\mathbf{x}=\mathbf{0}$ 只有零解 $\Leftrightarrow$ $A$ 的列构成 $\mathbb{R}^n$ 的一个基。

> 提示：这五个命题两两等价，都等价于"矩阵满秩"：$\text{rank}(A)=n \Leftrightarrow$ 列满秩（无自由列）$\Leftrightarrow$ 零空间只有零向量（$A\mathbf{x}=\mathbf{0}$ 唯一解）$\Leftrightarrow$ 行列式非零（方阵）$\Leftrightarrow$ 可逆。列线性无关且张成 $\mathbb{R}^n$（共 $n$ 个）$\Leftrightarrow$ 基。

---

**回头看一眼"一例速记"**：

> 基 = 线性无关 + 张成；维数 = 基的大小（唯一的数字）。
> 坐标 = 增广矩阵行化简到 RREF 的右列；不同基给同一向量不同坐标，但向量不变。
> 秩-零化度：$\text{rank} + \text{nullity} = n$（列数），快速算子空间维数的神器。

如果现在不看笔记，能独立完成例 1（完整解法）+ 例 2 + 自测 2——本章，你拿下了。
...
| 学习目标 + 11.1–11.4 严格正文 | 原版 | 完整定义与推导 |
| 本章小结 | 原版 | 公式速查 |
| 深度学习应用（PCA）+ 代码 | 原版 | 工业实战关联 |
| 练习题 + 详解 | 原版 | 系统巩固 |
