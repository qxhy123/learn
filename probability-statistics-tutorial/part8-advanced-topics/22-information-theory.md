...
| KL散度 | $D_{\mathrm{KL}}(P \vert Q) = \sum_x p(x)\log\frac{p(x)}{q(x)}$ | $P$ 与 $Q$ 的差异（不对称） |
| 交叉熵 | $H(P,Q) = H(P) + D_{\mathrm{KL}}(P \vert Q)$ | 用 $Q$ 编码 $P$ 的平均码长 |
...
| KL散度 $D_{\mathrm{KL}}(P \vert Q)$ | VAE正则项、知识蒸馏 | 驱使分布接近目标分布 |
