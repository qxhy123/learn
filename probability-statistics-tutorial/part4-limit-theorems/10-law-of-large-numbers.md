...
> **建议先读**：[第四部分导论《什么是收敛与极限定理？》](./09b-what-is-convergence.md) —— 先理解"随机序列收敛分强弱（依概率/几乎必然/依分布）"，再看下面的大数定律会清晰得多。
...
反之不成立（一般情况下）。其中**依分布收敛（d）** 指分布函数在连续点处收敛（$F_n(x)\to F(x)$），是最弱的一种——它只关心"分布形状趋同"，不要求随机变量本身接近。三种收敛模式的含义、为何分强弱、以及本部分的整体动机，见 [第三部分前的导论《什么是收敛与极限定理》](./09b-what-is-convergence.md)；严格定义见 [第12章](./12-convergence-theory.md)。
