...
![Venn 图：A、B、A∪B、A∩B 与补集](../figures/svg/prob-p1-01-1.svg)

### 图 1-2：古典概型 vs 几何概型

![古典概型（骰子）vs 几何概型（圆盘）对比](../figures/svg/prob-p1-01-2.svg)

---

## 思考路标（条件反射）

- 看到"随机试验"→ 立刻列出**样本空间** $\Omega$（不列全则概率算错）
- 看到"事件运算" → 翻译成集合语言：并 $\cup$、交 $\cap$、补 $\bar{\cdot}$
- 看到"概率三公理" → 非负 / 规范 $P(\Omega)=1$ / 可列可加（互斥才可加！）
- 看到"有限等可能" → **古典概型**：$P(A)=|A|/|\Omega|$
- 看到"连续区域上的随机点" → **几何概型**：$P(A)=\mu(A)/\mu(\Omega)$（选对度量：长度 / 面积 / 体积）
- 看到"独立 vs 互斥" → 独立：$P(AB)=P(A)P(B)$；互斥：$P(AB)=0$（两者不同！）
- 看到"至少一个" / "至多一个" → 优先用**对立事件**（正难则反）
- 看到"$P(A\cup B)$" → 容斥公式 $P(A)+P(B)-P(AB)$；单调性 $A\subseteq B\Rightarrow P(A)\leq P(B)$

## 易错点

1. **独立和互斥常混淆**：互斥意味着不能同时发生（$P(AB)=0$），独立意味着互不影响（$P(AB)=P(A)P(B)$）；正概率事件若互斥则必不独立。
2. **概率不能随意相加**：$P(A\cup B)=P(A)+P(B)$ 仅在 $A,B$ **互斥**时成立；一般情形须减去 $P(AB)$。
3. **几何概型选错度量**：问题涉及"长度"用长度，涉及"面积"用面积，选错度量则结果错误。
4. **样本空间未列全**：古典概型中遗漏样本点会导致概率偏大或偏小，务必穷举或用乘法原理计总数。
5. **概率值范围 $[0,1]$**：计算结果若超出此范围必然出错，可用于自检。
