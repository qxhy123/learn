...
> **一例速记**：
> - $P(B|A) = P(AB)/P(A)$
> - 链式：$P(ABC) = P(A)\,P(B|A)\,P(C|AB)$
> - 全概率：$P(B) = \sum_i P(A_i)\,P(B|A_i)$
> - **贝叶斯（由果求因）**：$P(A_k|B) = \dfrac{P(A_k)\,P(B|A_k)}{P(B)}$
> - **独立 ≠ 互斥**：独立 $P(AB)=P(A)P(B)$；互斥 $P(AB)=0$

---

## 引入题

某病发病率 $1\%$，检测灵敏度 $95\%$（真有诊出阳）、特异度 $90\%$（无病诊出阴）。某人检测阳性，求实际患病概率。

## 思维路径还原

> 识别题型：已知"检测阳性"这个**结果**，求"实际患病"这个**原因**的概率——典型的"由果求因"，用**贝叶斯公式**。
>
> 设 $A=$ 患病，$B=$ 检测阳性。
>
> 给出先验：$P(A) = 0.01$，$P(\bar{A}) = 0.99$。
>
> 灵敏度是真有患病时检出阳性的概率：$P(B|A) = 0.95$。
>
> 特异度 $90\%$ 是无病时检出阴性的概率，故假阳性率：$P(B|\bar{A}) = 1 - 0.90 = 0.10$。
>
> 第一步，**全概率公式**算分母 $P(B)$（阳性总概率）：
>
> $$P(B) = P(A)\,P(B|A) + P(\bar{A})\,P(B|\bar{A}) = 0.01 \times 0.95 + 0.99 \times 0.10 = 0.0095 + 0.0990 = 0.1085$$
>
> 第二步，**贝叶斯公式**算分子 / 分母：
>
> $$P(A|B) = \frac{P(A)\,P(B|A)}{P(B)} = \frac{0.0095}{0.1085} \approx 8.76\%$$
>
> **反直觉警报**：检测阳性后，实际患病概率仍不足 $9\%$！背后原因是**低先验（发病率仅 $1\%$）**放大了假阳性：1000人中约有 10 名患者，995 名健康人中有约 99.5 人假阳性，真阳性仅约 9.5 人。
>
> 启示：低先验下，单次阳性结果的多数是假阳性；提高检测价值需**提升特异度**或**针对高危人群**检测（提升先验）。
...
| 概念 | 公式 | 记忆要点 |
|------|------|----------|
| 条件概率 | $P(A\vert B) = P(A\cap B)/P(B)$ | 缩小样本空间，重新归一化 |
| 乘法公式 | $P(A\cap B) = P(A\vert B)P(B)$ | 由条件概率变形而来 |
| 链式法则 | $P(\bigcap A_i) = \prod P(A_i \vert A_1\cdots A_{i-1})$ | 语言模型的核心分解 |
| 全概率公式 | $P(A) = \sum P(A\vert B_i)P(B_i)$ | 按"原因"加权平均 |
| 贝叶斯公式 | $P(B_i\vert A) \propto P(A\vert B_i)P(B_i)$ | 后验 ∝ 似然 × 先验 |
...
- 看到 $P(B|A)$ → 公式：$P(AB)/P(A)$，**分母 $P(A)>0$**
- 看到"依次发生多个事件的联合概率" → **链式公式**：$P(ABC)=P(A)\,P(B|A)\,P(C|AB)$
- 看到"分情况（按原因）求某事件的总概率" → **全概率公式**：$P(B)=\sum_i P(A_i)P(B|A_i)$
- 看到"已知结果，推原因概率" → **贝叶斯（由果求因）**：后验 $\propto$ 似然 $\times$ 先验
- 看到 $P(AB)=P(A)P(B)$ → **独立**；看到 $P(AB)=0$ → **互斥**（两者完全不同）
- 看到"先验 / 后验 / 似然" → 贝叶斯框架：先验经似然更新得后验
- 看到"阳性检测 + 低发病率" → **假阳性反直觉**：低先验下大多数阳性是假阳性
- 看到多步随机试验 → 画**树状图**，叶节点概率=路径上各条件概率的乘积

## 易错点

1. **独立 ≠ 互斥**（最常见混淆）：独立的两个正概率事件**可以同时发生**（$P(AB)>0$）；互斥事件必然不能同时发生（$P(AB)=0$），故正概率的互斥事件必不独立。
2. **条件概率分母 $P(A)\neq 0$**：$P(B|A)$ 要求 $P(A)>0$，若 $P(A)=0$ 则条件概率无意义。
3. **链式公式顺序**：$P(ABC)=P(A)\cdot P(B|A)\cdot P(C|AB)$，条件要逐步累积，顺序不可随意交换。
4. **贝叶斯反演方向**：贝叶斯公式求 $P(\text{原因}|\text{结果})$，不能与 $P(\text{结果}|\text{原因})$ 混淆，两者通常差距极大。
5. **假阳性占多数（低发病率）**：当先验 $P(\text{患病})$ 很小时，即使检测很准，阳性结果中假阳性仍占多数——务必用贝叶斯公式计算，不可凭直觉判断。
