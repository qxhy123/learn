...
> **古典概型**：$P(A) = \dfrac{\vert A\vert}{\vert\Omega\vert}$（有限等可能）。
...
1. **有限性**：样本空间只含有限个样本点，即 $\vert\Omega\vert = n < +\infty$
2. **等可能性**：每个样本点出现的可能性相同，即每个基本事件的概率为 $\dfrac{1}{n}$

在古典概型中，若事件 $A$ 包含 $k$ 个样本点，则：

$$\boxed{P(A) = \frac{k}{n} = \frac{\vert A\vert}{\vert\Omega\vert} = \frac{\text{A 包含的样本点数}}{\text{样本空间的样本点总数}}}$$

### 1.4.2 古典概型例题

**例 1.6** 掷一枚均匀骰子，求"出现偶数点"的概率。

**解：** $\Omega = \{1,2,3,4,5,6\}$，$\vert\Omega\vert = 6$

$A = \{2,4,6\}$，$\vert A\vert = 3$
...
4. 看到"有限等可能" → **古典概型**：$P(A)=\vert A\vert/\vert\Omega\vert$；先数 $\vert\Omega\vert$，再数 $\vert A\vert$
...
4. **样本空间未列全**：古典概型中遗漏样本点会导致 $\vert\Omega\vert$ 偏小，概率偏大。务必穷举或用乘法原理计总数，尤其是有序 vs 无序的区分。
...
【解】$\vert\Omega\vert = 100$。

设 $A$ = "被 $3$ 整除"，$B$ = "被 $5$ 整除"，$AB$ = "被 $15$ 整除"。

$\vert A\vert = \lfloor 100/3 \rfloor = 33$，$\vert B\vert = \lfloor 100/5 \rfloor = 20$，$\vert AB\vert = \lfloor 100/15 \rfloor = 6$。

$$P(A \cup B) = \frac{33+20-6}{100} = \frac{47}{100} = 0.47$$

【答案】$\boxed{P = 0.47}$

【注】若不减 $\vert AB\vert=6$，会将能被 $15$ 整除的数计两次，得到错误答案 $53/100$。容斥原理是"避免重复"的核心工具。
...
【解】$\vert\Omega\vert = 2^5 = 32$（每次两种结果，共5次）。
...
$A$ = "两数之和为7"：$A = \{(1,6),(2,5),(3,4),(4,3),(5,2),(6,1)\}$，$\vert A\vert=6$
...
$C$ = "至少有一枚点数为6"：共 $6+6-1=11$ 个，$\vert C\vert=11$
...
已知：$\vert M\vert = 18$，$\vert C\vert = 15$，$\vert MC\vert = 8$

**（1）** 容斥原理：$\vert M \cup C\vert = 18 + 15 - 8 = 25$
...
**（3）** 只喜欢数学 = $M - C = M\bar{C}$：$\vert M - C\vert = \vert M\vert - \vert MC\vert = 18 - 8 = 10$
...
3封信投入3个信箱，总方案数 $\vert\Omega\vert = 3^3 = 27$（每封信3种选择，独立）。
...
> 三公理：非负、规范、互斥可加。
> 古典概型：$P(A) = \vert A\vert/\vert\Omega\vert$。
> 加法公式减交集；对立公式正难则反。
