...
> **难度**：★★★★☆
> **前置知识**：第 17 章假设检验基础、第 18 章正态分布与抽样分布、第 19 章非参数检验
> **本文件**：融合"原版严格推导 + 重写版速记 / 套路 / 自测"。保留原版完整正文（学习目标 / 20.1–20.5 / 深度学习应用 / 练习题）+ 在最前置、最后追加思维训练。

> **一例速记**：
> **z 检验**：$\sigma$ 已知，统计量 $Z = (\bar{X}-\mu_0)/(\sigma/\sqrt{n}) \sim N(0,1)$，大样本或方差已知才用。
> **单样本 t 检验**：$\sigma$ 未知，$t = (\bar{X}-\mu_0)/(S/\sqrt{n}) \sim t(n-1)$，小样本标配。
> **配对 t 检验**：差值 $D_i = X_{1i}-X_{2i}$，对 $D$ 做单样本 $t$，自由度 $n-1$（$n$ 为配对数）。
> **独立双样本 t**：等方差用合并方差 $S_p^2$，自由度 $n_1+n_2-2$；不等方差用 Welch，自由度用 Satterthwaite 公式。
> **F 检验方差比**：$F = S_1^2/S_2^2 \sim F(n_1-1, n_2-1)$，用于检验 $\sigma_1^2 = \sigma_2^2$。
> **$\chi^2$ 方差检验**：单总体 $(n-1)S^2/\sigma_0^2 \sim \chi^2(n-1)$，检验 $\sigma^2 = \sigma_0^2$。

---

## 引入：小样本为什么不能用 z 检验？t 分布的厚尾意义

> **题目**：某实验室声称新药可使患者血压降低 $\mu_0 = 10\,\text{mmHg}$。研究者只招募了 $n = 8$ 名受试者，测得降低量均值 $\bar{x} = 7.5\,\text{mmHg}$，样本标准差 $s = 4.2\,\text{mmHg}$。
>
> 问：(1) 能用 $z$ 检验吗？(2) 如果误用 $z$ 检验（把 $s$ 当 $\sigma$），结论会有什么偏差？(3) 正确方法是什么？

请先想一想：$n=8$ 时，$s$ 能代替 $\sigma$ 吗？

**正确认识**：

用 $z$ 检验要求 $\sigma$ **已知**（不是估计值）。当 $n=8$，$s=4.2$ 是用仅 8 个观测值估计出的样本标准差，本身带有**估计误差**——大约有 ±20% 的随机波动。若把 $s$ 当 $\sigma$ 代入 $z$ 检验，统计量

$$Z = \frac{\bar{X}-\mu_0}{s/\sqrt{n}} = \frac{7.5-10}{4.2/\sqrt{8}} \approx -1.683$$

查标准正态表得 $p \approx 0.046$，看起来在 $\alpha = 0.05$ 下刚刚显著。

但**这是错的**。$s/\sqrt{n}$ 不是 $\sigma/\sqrt{n}$，由于我们额外估计了 $\sigma$，引入了**额外的不确定性**，应当用 $t$ 分布来吸收这种不确定性。$t$ 分布比正态分布**尾部更厚**，对应更大的临界值：

$$t = \frac{7.5-10}{4.2/\sqrt{8}} \approx -1.683, \quad t_{0.025}(7) = 2.365$$

双侧 $p = 2P(T_7 < -1.683) \approx 0.136$，不显著！

**结论**：误用 $z$ 检验会让 $p$ 值偏小，增大 I 类错误（假阳性）。$t$ 分布的厚尾正是对"用 $s$ 估计 $\sigma$ 带来额外不确定性"的精确补偿。自由度越小，估计越不稳，尾部越厚；自由度趋于 $\infty$ 时，$t$ 分布趋近正态——此时 $s \approx \sigma$，$z$ 和 $t$ 等价。

---

## 思维路径还原（检验选择决策树）

> "拿到一道参数检验题，我的大脑自动走如下流程——
>
> **第一步：明确研究对象**
> 是均值还是方差？涉及几个总体？一个总体 → 单样本；两个总体 → 双样本；三个及以上 → ANOVA。
>
> **第二步：判断方差是否已知**
> 若 $\sigma$ 已知（或 $n \geq 30$ 近似）→ 用 $z$ 检验。
> 若 $\sigma$ 未知（最常见）→ 用 $t$ 检验，自由度 $n-1$。
> 若检验的对象本身就是方差 → 用 $\chi^2$（单总体）或 $F$（双总体）。
>
> **第三步：独立还是配对？**
> 若两组数据有天然配对关系（前后测、同一对象两次测量、同一数据集两模型评分）→ 配对 $t$，差值化为单样本。
> 若两组完全独立 → 先检验方差齐性（$F$ 检验或 Levene 检验）：
>   - 方差相等 → 等方差两样本 $t$，合并方差 $S_p^2$，自由度 $n_1+n_2-2$
>   - 方差不等 → Welch $t$，Satterthwaite 自由度（通常略小于 $n_1+n_2-2$）
>
> **第四步：单侧还是双侧？确认 $\alpha$**
> 有方向性预期（"至少"、"不低于"、"更大"）→ 单侧，临界值更宽松，更易拒绝。
> 无方向性 → 双侧，两侧各 $\alpha/2$，更保守。
>
> **第五步：计算、查表、下结论**
> 统计量落入拒绝域（$\vert t \vert > t_{\alpha/2}$，或 $F > F_{\alpha/2}$，或 $\chi^2$ 落入双侧尾） → 拒绝 $H_0$；反之不拒绝。
> 别忘：**不拒绝 ≠ 接受**，只是证据不足以否定 $H_0$。
>
> **易混淆点**：
> 配对 t 的自由度是配对数 $n$ 减一，而非 $2n-2$——因为把差值视为一个样本。
> Welch 自由度总是取整（向下取），比等方差情况小，临界值更大，更保守。
> ANOVA 的 $F$ 只能检测'至少有一对不等'，不告诉你哪对；需事后比较。"
...
| 备择假设 | 拒绝域 | $p$ 值 |
|----------|--------|--------|
| $\mu \neq \mu_0$（双侧） | $\vert t \vert > t_{\alpha/2}(n-1)$ | $2P(T > \vert t_{\text{obs}} \vert)$ |
...
$$\chi^2_{\text{Yates}} = \sum_{i,j} \frac{\left(\vert O_{ij} - E_{ij}\vert - 0.5\right)^2}{E_{ij}}$$
...
### 图 20-1：z 检验 vs t 检验（厚尾对比）

![z 检验 vs t 检验：σ 已知正态分布与 σ 未知不同自由度 t 分布对比](../figures/svg/prob-p7-20-1.svg)

上图展示标准正态（$z$ 检验）与不同自由度 $t$ 分布的尾部厚度差异。自由度越小（样本量越少），$t$ 分布尾部越厚，临界值越大——这正是对"用 $s$ 估计 $\sigma$ 引入额外不确定性"的精确补偿。随着自由度增大，$t$ 分布逐渐逼近 $N(0,1)$。

### 图 20-2：两样本均值/方差检验示意

![双总体 PDF 与两样本 t 检验及 F 检验示意图](../figures/svg/prob-p7-20-2.svg)

上图示意两个正态总体的概率密度函数，以及检验均值差（两样本 $t$ 检验）和方差比（$F$ 检验）时对应的统计量分布。左侧两个 PDF 重叠程度体现了均值差的显著性；右侧的 $F$ 分布形状和拒绝域展示方差齐性检验的判断逻辑。
...
| 问题类型 | 检验名称 | 统计量 | 原假设分布 | 自由度 |
|----------|----------|--------|-----------|--------|
| 单总体均值（$\sigma$ 已知） | $z$ 检验 | $Z = (\bar{X}-\mu_0)/(\sigma/\sqrt{n})$ | $N(0,1)$ | — |
| 单总体均值（$\sigma$ 未知） | 单样本 $t$ | $t = (\bar{X}-\mu_0)/(S/\sqrt{n})$ | $t(n-1)$ | $n-1$ |
| 配对两样本均值差 | 配对 $t$ | $t = \bar{D}/(S_D/\sqrt{n})$ | $t(n-1)$ | $n-1$ |
| 独立两样本均值差（等方差） | 两样本 $t$ | $t = (\bar{X}_1-\bar{X}_2)/(S_p\sqrt{1/n_1+1/n_2})$ | $t(n_1+n_2-2)$ | $n_1+n_2-2$ |
| 独立两样本均值差（异方差） | Welch $t$ | $t = (\bar{X}_1-\bar{X}_2)/\sqrt{S_1^2/n_1+S_2^2/n_2}$ | $t(\nu)$ | Satterthwaite |
| 单总体方差 | $\chi^2$ 检验 | $\chi^2 = (n-1)S^2/\sigma_0^2$ | $\chi^2(n-1)$ | $n-1$ |
| 两总体方差比 | $F$ 检验 | $F = S_1^2/S_2^2$ | $F(n_1-1,n_2-1)$ | $(n_1-1,n_2-1)$ |
| 多总体均值 | ANOVA $F$ | $F = MS_A/MS_E$ | $F(k-1,N-k)$ | $(k-1,N-k)$ |
...
- 均值 → 步骤 2
- 方差 → 单总体用 $\chi^2$；双总体用 $F$；多总体 Bartlett/Levene

**步骤 2** — 几个总体？

- 1 个 → 单样本 $t$（$\sigma$ 未知）或 $z$（$\sigma$ 已知）
- 2 个 → 步骤 3
- 3 个及以上 → 单因素 ANOVA

**步骤 3** — 独立还是配对？

- 配对（同一对象前后测，或相同数据集上两模型评分） → 配对 $t$，$df=n-1$
- 独立 → 步骤 4

**步骤 4** — 方差齐性？

- 先做 Levene 检验或 $F$ 检验；方差相等 → 等方差两样本 $t$（$df=n_1+n_2-2$）
- 方差不等 → Welch $t$（$df$ 按 Satterthwaite 公式，通常向下取整）
...
若数据有配对结构却用独立 $t$ 检验，会将"个体间差异"计入误差方差，导致 $S_p^2$ 虚大，$t$ 统计量偏小，更难拒绝 $H_0$（检验功效大幅下降）。反之，若数据独立却用配对 $t$，自由度减半（$n-1$ 而非 $2n-2$），临界值变大，也可能损失功效。**配对性由研究设计决定，不能事后凑数**。

### 变形 2：单总体 vs 双总体

单总体检验假设值 $\mu_0$ 是**已知常数**（规格标准、历史基准），而双总体检验两总体均值通过各自样本估计。若把双总体的其中一个样本均值当 $\mu_0$ 代入单样本公式，会忽略估计误差，导致统计量偏大，犯 I 类错误概率上升。

### 变形 3：等方差 vs 异方差（Welch 的优势）

实践中推荐**默认 Welch $t$**，理由：
- 方差相等时，Welch 与等方差检验的 $p$ 值几乎相同（功效损失 $< 1\%$）
- 方差不等时，等方差检验 I 类错误率可膨胀至 $10\%-15\%$（$\alpha=5\%$ 时）
- Welch 的唯一代价是自由度变成非整数（软件自动处理）

### 变形 4：似然比检验（广义框架）

上述所有参数检验均可统一在**Neyman-Pearson 似然比检验**框架下：

$$\Lambda = \frac{\sup_{H_0} L(\theta; \mathbf{x})}{\sup_{\Theta} L(\theta; \mathbf{x})}$$

在正则条件下，$-2\ln\Lambda \xrightarrow{d} \chi^2(r)$（$r$ 为 $H_0$ 约束维数）。单样本 $t$ 检验、$F$ 检验、$\chi^2$ 检验均是此框架的精确特例（小样本精确分布），无需依赖大样本近似。
...
5. 所有参数检验均依赖特定分布假设，使用前应进行正态性检验（Shapiro-Wilk）、方差齐性检验（Levene/Bartlett）等前提条件验证；条件不满足时应考虑第 19 章的非参数方法。

---

## 思考路标（条件反射）

1. 看到"$\sigma$ 已知" → 立刻用 $z$ 检验；看到"$\sigma$ 未知" → $t$ 检验，自由度 $n-1$
2. 看到"前后测"、"同一受试者"、"同一数据集两模型" → 配对 $t$，差值化单样本
3. 看到两个独立样本比均值 → 先做 Levene 检验，再决定等方差还是 Welch
4. 看到 $n=8$ 类小样本 → 不能用 $z$，必须用 $t$；$n \geq 30$ 才可近似正态
5. 看到"比较方差" → 单总体 $\chi^2$，双总体 $F$，注意分子分母自由度顺序
6. 看到"三组及以上" → ANOVA，$F = MS_A/MS_E$；拒绝后需事后比较（Tukey HSD）
7. 看到"100 个特征同时检验" → 多重比较问题，用 BH-FDR 而非单个 $p < 0.05$
8. 看到"单侧检验" → 临界值是 $t_\alpha$ 不是 $t_{\alpha/2}$，$p$ 值也只取一侧
9. 看到"不拒绝 $H_0$" → 不等于接受 $H_0$，只是"证据不足以否定"，区分统计显著性与实际意义
10. 看到"效应量" → $p$ 值显著未必有实践意义，还需报告 Cohen's $d$ 或 $\eta^2$
11. 看到"ANOVA $F$ 显著" → 只说"至少一对均值不同"，不告诉哪对；事后检验才能定位
12. 看到卡方拟合优度 → 自由度 $= k-1-r$（$r$ 为估计参数数），别忘减估计参数数

---

## 易错点

1. **$z$ 检验 vs $t$ 检验的选择**：$z$ 检验要求 $\sigma$ 真正已知（非估计），仅大样本（$n\geq 30$）时近似可用。将 $s$ 代入 $z$ 统计量会低估 $p$ 值，虚假显著（假阳性）。实践中绝大多数场景 $\sigma$ 未知，应优先用 $t$ 检验。

2. **配对 vs 独立混用**：配对 $t$ 的自由度是 $n-1$（$n$ 为配对数），不是 $2n-2$。若数据有配对关系却用独立 $t$，个体间变异被计入误差，功效大降；反向错误（独立数据用配对 $t$）则减少了有效自由度，同样损失功效。配对性必须在研究设计阶段确定。

3. **方差齐性检验与 F 检验关系**：先用 Levene（或 Bartlett）检验验证方差齐性，再决定用哪种两样本 $t$。Levene 检验本身是关于方差的检验，与比较均值的 $t$ 检验是独立步骤，不能混为一谈。

4. **效应量缺失**：$p$ 值仅说明"效果是否可能由随机产生"，不说明"效果有多大"。大样本时即使微小的差异也可 $p < 0.001$，却毫无实践意义。应同时报告 Cohen's $d = (\bar{X}_1 - \bar{X}_2)/S_p$（两样本均值差）或 $\eta^2 = SS_A/SS_T$（ANOVA 方差解释比例）。

5. **单尾 vs 双尾混用**：单侧检验的临界值更宽松（$t_\alpha$ 而非 $t_{\alpha/2}$），更容易拒绝 $H_0$。但单侧假设必须在实验**前**基于理论依据确定，不能"看到数据后"改为单侧（这等价于对 $p$ 值除以 2，是数据挖掘行为，严重膨胀 I 类错误）。

---

## 典型应用例题

### 例 A：单总体 $t$ 检验——某 AI 系统响应时间

**题目**：某 AI 推理系统的设计目标是平均响应时间不超过 $\mu_0 = 200\,\text{ms}$。测试团队随机采样 $n=25$ 次请求，得到响应时间均值 $\bar{x} = 215\,\text{ms}$，样本标准差 $s = 35\,\text{ms}$。在 $\alpha = 0.05$ 下，是否有统计证据认为系统响应时间超标？

**解**：

建立假设（单侧，右侧检验）：

$$H_0: \mu \leq 200 \quad \text{vs} \quad H_1: \mu > 200$$

计算统计量：

$$t = \frac{215 - 200}{35/\sqrt{25}} = \frac{15}{7} \approx 2.143$$

查 $t$ 分布表：$t_{0.05}(24) = 1.711$（单侧）。

因 $t = 2.143 > 1.711$，**拒绝** $H_0$。

**结论**：在 $\alpha = 0.05$ 显著性水平下，有统计证据认为该 AI 系统的平均响应时间超过 200 ms。

**置信区间**（$\mu$ 的 95% 单侧置信下界）：

$$\mu > \bar{x} - t_{0.05}(24) \cdot \frac{s}{\sqrt{n}} = 215 - 1.711 \times 7 = 215 - 11.98 \approx 203\,\text{ms}$$

即有 95% 把握认为真实均值超过 203 ms。

---

### 例 B：配对 $t$ 检验——优化器更新前后性能对比

**题目**：研究者在 8 个基准数据集上分别用旧版（$x_{1i}$）和新版优化器（$x_{2i}$）训练同一模型，记录准确率（%）如下：

| 数据集 | 旧版 $x_{1i}$ | 新版 $x_{2i}$ | 差值 $d_i = x_{2i}-x_{1i}$ |
|--------|--------------|--------------|---------------------------|
| A | 81.2 | 83.1 | 1.9 |
| B | 74.5 | 75.8 | 1.3 |
| C | 88.3 | 90.0 | 1.7 |
...
在 $\alpha = 0.05$ 下检验新版优化器是否显著提升准确率。

**解**：

因为同一数据集用两种优化器，存在天然配对关系 → 使用**配对 $t$ 检验**。

计算差值统计量：

$$\bar{d} = \frac{1.9+1.3+1.7+2.3+0.8+1.7+1.5+1.6}{8} = \frac{12.8}{8} = 1.6$$

$$s_D = \sqrt{\frac{\sum(d_i - \bar{d})^2}{n-1}} = \sqrt{\frac{(0.3)^2+(−0.3)^2+(0.1)^2+(0.7)^2+(−0.8)^2+(0.1)^2+(−0.1)^2+(0)^2}{7}}$$

$$= \sqrt{\frac{0.09+0.09+0.01+0.49+0.64+0.01+0.01+0}{7}} = \sqrt{\frac{1.34}{7}} \approx 0.437$$

建立假设（单侧，右侧）：$H_0: \mu_D \leq 0$ vs $H_1: \mu_D > 0$

$$t = \frac{1.6}{0.437/\sqrt{8}} = \frac{1.6}{0.1546} \approx 10.35$$

$t_{0.05}(7) = 1.895$，因 $t = 10.35 \gg 1.895$，**拒绝** $H_0$。

**结论**：新版优化器在这 8 个数据集上平均准确率提升约 1.6%，差异高度显著（$p \ll 0.001$）。

---

### 例 C：独立双样本 $t$ 检验——两实验室测量值对比

**题目**：两个实验室用不同仪器测量同一批样本的某蛋白质浓度（单位：mg/L）。实验室 A 测 $n_1=10$ 次，得 $\bar{x}_1=52.4$，$s_1=3.8$；实验室 B 测 $n_2=12$ 次，得 $\bar{x}_2=49.1$，$s_2=5.2$。在 $\alpha = 0.05$ 下，两实验室测量均值是否存在显著差异？

**解**：

**第一步**：方差齐性检验（$F$ 检验，双侧）：

$$F = \frac{s_2^2}{s_1^2} = \frac{27.04}{14.44} \approx 1.87$$

$F_{0.025}(11, 9) \approx 3.91$（查表），因 $F < 3.91$，不拒绝方差相等假设，使用**等方差两样本 $t$ 检验**。

**第二步**：合并方差：

$$S_p^2 = \frac{(10-1)\times 14.44 + (12-1)\times 27.04}{10+12-2} = \frac{129.96 + 297.44}{20} = \frac{427.40}{20} = 21.37$$

$$S_p = \sqrt{21.37} \approx 4.623$$

**第三步**：计算统计量：

$$t = \frac{52.4 - 49.1}{4.623\sqrt{1/10 + 1/12}} = \frac{3.3}{4.623 \times 0.4282} \approx \frac{3.3}{1.980} \approx 1.667$$

查 $t$ 分布表：$t_{0.025}(20) = 2.086$（双侧）。

因 $|t| = 1.667 < 2.086$，**不拒绝** $H_0$。

**结论**：在 $\alpha = 0.05$ 水平下，没有足够证据认为两实验室测量均值存在显著差异。
...
<details>
<summary>点击展开 练习 20.1 答案</summary>
...
$\sum x_i = 90.1+91.5+89.8+92.3+91.0+90.7+91.8+90.5+91.2 = 818.9$，$\bar{x} = 818.9/9 \approx 91.0$
...
$$s = \sqrt{\frac{5.21}{8}} = \sqrt{0.651} \approx 0.807, \quad t = \frac{91.0 - 92}{0.807/\sqrt{9}} = \frac{-1.0}{0.269} \approx -3.717$$

**查表与决策**：$t_{0.05}(8) = 1.860$，左侧检验拒绝域为 $t < -1.860$。
...
<details>
<summary>点击展开 练习 20.2 答案</summary>
...
$$\bar{d} = \frac{0.090}{8} = 0.01125, \quad s_D \approx 0.003964$$

$$t = \frac{0.01125}{0.003964/\sqrt{8}} \approx 8.03, \quad t_{0.025}(7) = 2.365$$

因 $t = 8.03 \gg 2.365$，**拒绝** $H_0$，ResNet-50 显著优于 VGG-16。

**95% 置信区间**：$0.01125 \pm 2.365 \times 0.001401 = (0.00794,\; 0.01456)$
...
<details>
<summary>点击展开 练习 20.3 答案</summary>

**(a) ANOVA 计算**：

各组均值：$\bar{x}_{1.} \approx 0.4102$，$\bar{x}_{2.} = 0.3895$，$\bar{x}_{3.} = 0.4295$，总均值 $\bar{x}_{..} \approx 0.4097$

**组间平方和** $SS_A \approx 4.802\times10^{-3}$，**组内平方和** $SS_E \approx 1.373\times10^{-3}$
...
三种 dropout 率下的验证损失均值存在显著差异，应进一步进行 Tukey HSD 事后比较。
...
<details>
<summary>点击展开 练习 20.4 答案</summary>

**(a) Bonferroni 校正**（$m=100$，阈值 $= 0.05/100 = 0.0005$）：

$p_{(1)} = 0.00008 < 0.0005$ 通过；$p_{(2)} = 0.00021 < 0.0005$ 通过；$p_{(3)} = 0.00089 > 0.0005$ 不通过。

**选中特征：2 个**（排名第 1、2 的特征）。

**(b) BH 方法**（$m=100$，$q=0.05$，判断条件 $p_{(j)} \leq j \times 0.0005$）：

| $j$ | $p_{(j)}$ | 阈值 $j \times 0.0005$ | 是否通过 |
|-----|-----------|----------------------|---------|
| 1 | 0.00008 | 0.00050 | 通过 |
| 2 | 0.00021 | 0.00100 | 通过 |
| 3 | 0.00089 | 0.00150 | 通过 |
| 4 | 0.00203 | 0.00200 | 边界（$0.00203 > 0.00200$ 不通过） |
| 5 | 0.00412 | 0.00250 | 不通过 |

精确比较后最大通过下标 $k=3$，**选中特征：3 个**（排名第 1~3）。

**(c) 比较与建议**：Bonferroni 选中 2 个，BH 选中 3 个。在 100 个特征同时检验的场景下，**BH 方法更合适**：FDR=5% 允许少量误判，功效更高，能发现更多真正相关特征。Bonferroni 适用于假阳性代价极高的场景（如医学诊断）。
...
<details>
<summary>点击展开 练习 20.5 答案</summary>

**(a) 期望频数矩阵**：

各行/列合计：预测猫 110，预测狗 100，预测鸟 90，预测鱼 90；真实猫 100，真实狗 100，真实鸟 90，真实鱼 100；总计 390。

期望频数 $E_{ij} = R_i C_j / 390$，例如 $E_{11} = 110\times100/390 \approx 28.2$（完整矩阵按公式逐一计算）。

**(b) 卡方统计量**：对角线元素远大于期望，$\chi^2$ 远超 $\chi^2_{0.05}(9) = 16.92$，**拒绝** $H_0$。

**(c) 含义与联系**：卡方检验显著说明预测与真实标签不独立——此情境下是好事，模型有判别能力（随机猜测时两者独立）。混淆矩阵提供更细粒度信息（哪些类容易混淆），卡方检验只给出全局显著性结论。
...
**自测 1** 某药物试验中，研究者测量 $n=6$ 名患者服药前后的血压差值（mmHg）：$5, 8, 3, 7, 6, 9$。在 $\alpha = 0.05$ 下，药物是否显著降低血压？（双侧检验）

> 💡 提示：$\bar{d} = 38/6 \approx 6.33$，$s_D \approx 2.16$，$t = 6.33/(2.16/\sqrt{6}) \approx 7.18$，$t_{0.025}(5)=2.571$，拒绝 $H_0$，药物显著有效。

**自测 2** 两组学生各 20 人分别参加传统授课和 AI 辅助授课，期末成绩方差分别为 $s_1^2 = 64$，$s_2^2 = 36$。在 $\alpha = 0.10$ 下，两组成绩方差是否有显著差异？

> 💡 提示：$F = 64/36 \approx 1.78$，$F_{0.05}(19,19) \approx 2.17$（双侧各 $0.05$）。因 $1.78 < 2.17$，不拒绝方差相等假设，两组方差无显著差异。

**自测 3** 对同一批 15 个特征同时做 $t$ 检验，有 3 个 $p$ 值小于 0.05。若不做多重比较校正，族错误率 FWER 约为多少？

> 💡 提示：FWER $= 1 - (1-0.05)^{15} \approx 1 - 0.463 = 0.537$，约 54%。即使所有原假设均真，也有约一半概率犯至少一次假阳性。

**自测 4** 某 ANOVA 实验比较 $k=4$ 组，每组 $n_i=5$ 个观测值，$SS_A = 12.6$，$SS_E = 8.4$。计算 $F$ 统计量并判断是否显著（$\alpha=0.05$）。

> 💡 提示：$df_A = 3$，$df_E = 16$，$MS_A = 4.2$，$MS_E = 0.525$，$F = 4.2/0.525 = 8.0$，$F_{0.05}(3,16) \approx 3.24$，$F \gg 3.24$，拒绝 $H_0$，各组均值存在显著差异。

**自测 5** 两个分类模型在同一测试集上预测：模型 A 正确但 B 错误的样本 $b=18$，A 错误但 B 正确的 $c=6$。用 McNemar 检验（Yates 校正），两模型是否有显著差异（$\alpha=0.05$）？

> 💡 提示：$\chi^2 = (\vert 18-6\vert - 1)^2/(18+6) = (11)^2/24 = 121/24 \approx 5.04$，$\chi^2_{0.05}(1) = 3.84$，因 $5.04 > 3.84$，拒绝 $H_0$，模型 A 显著优于模型 B。

---

**回头看一眼"一例速记"**：

> z 检验：$\sigma$ 已知用 $N(0,1)$；t 检验：$\sigma$ 未知用 $t(n-1)$，尾更厚。
> 配对 t：差值化单样本，$df=n-1$；独立 t：等方差用 $S_p^2$，异方差用 Welch。
> 方差检验：单总体 $\chi^2$，双总体 $F$；多重比较：少量用 Bonferroni，大量用 BH-FDR。

如果现在不看笔记，能独立解出引入题（小样本 t vs z）+ 例 B（配对 t）+ 自测 4（ANOVA）——本章，你拿下了。

---

## 融合版说明

本版 = **原版（严格大学教材 + 深度学习应用）** + **重写版（速记 / 决策树 / 典型例题 / 自测）** 融合：

| 段落 | 来源 | 价值 |
|------|------|------|
| 一例速记 + 引入（小样本 z vs t）+ 思维路径还原 | 重写版（前置） | 建立直觉 / 反射 |
| 学习目标 + 20.1–20.5 严格正文 | 原版 | 完整推导 |
| 几何示意（2 张 SVG 图） | 配图 | 可视化 |
| 抽象成方法（速查表 + 4 步流程） | 重写版（中段） | 套路总结 |
| 方法变形（4 类） | 融合两版 | 边界条件 |
| 本章小结 | 原版 | 公式速查 |
| 思考路标（12 条）+ 易错点（5 条） | 融合两版 | 条件反射 |
| 典型应用例题（3 例） | 重写版 | 演练 |
| 深度学习应用 + PyTorch | 原版 | 工业实战 |
| 练习题 + `<details>` 详解 | 原版（折叠） | 巩固 |
| 自测题（5 题带提示） | 重写版 | 额外训练 |
