...
要从这个分解得出"第一项 $\sim\chi^2(n-1)$ 且两项独立"，依据的就是下面这条定理。

> **Cochran 定理（本处所需形式）**：设 $Z_1,\dots,Z_n$ 独立同 $\mathcal{N}(0,1)$。若它们的平方和被分解为若干**二次型**之和
> $$\sum_{i=1}^n Z_i^2 = Q_1 + Q_2 + \cdots + Q_k,$$
> 其中 $Q_j$ 是 $Z_i$ 的二次型、秩为 $r_j$，且**秩之和恰好等于总自由度** $r_1+\cdots+r_k = n$，则：① 各 $Q_j$ **相互独立**；② 每个 $Q_j \sim \chi^2(r_j)$。
>
> **直觉**：$n$ 个独立标准正态的"自由度总量"为 $n$；正交分解把这 $n$ 个自由度无重叠地分给各二次型，每块自由度数 $r_j$ 决定它是 $\chi^2(r_j)$，且因正交（不共享自由度）而相互独立。

套用到本处：左边 $\sum Y_i^2 \sim\chi^2(n)$（总自由度 $n$），右边第一项 $\dfrac{(n-1)S^2}{\sigma^2}$ 的秩为 $n-1$、第二项的秩为 $1$，$ (n-1)+1 = n$ 满足秩和条件。故第一项 $\sim \chi^2(n-1)$，且两项相互独立。$\blacksquare$
...
**证明**：这正是上面 Cochran 定理结论 ① 的直接推论——分解式中第二项 $n\big(\tfrac{\bar X-\mu}{\sigma}\big)^2$ 是 $\bar X$ 的函数、第一项 $\tfrac{(n-1)S^2}{\sigma^2}$ 是 $S^2$ 的函数，两个二次型独立，故 $\bar X$ 与 $S^2$ 独立。$\blacksquare$
