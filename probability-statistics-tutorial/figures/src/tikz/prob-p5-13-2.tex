...
\begin{axis}[
  ...
  xlabel={$\bar{x}$},
  ylabel={$f(\bar{x})$},
  title={\textbf{$\bar{X}$ 的抽样分布（$n$ 越大越集中）}},
  axis lines=left,
  xtick={-3,-2,-1,0,1,2,3},
  ...
  \node[font=\scriptsize, align=left] at (axis cs:-2.8,1.1)
...
\begin{axis}[
  ...
  xlabel={$\chi^2$},
  ylabel={$f(\chi^2)$},
  title={\textbf{$(n{-}1)S^2/\sigma^2$ 的分布（$\chi^2$ 分布）}},
  axis lines=left,
...
\node[font=\large\bfseries]
  at ($(ax1.north)!0.5!(ax2.north)+(0,0.6cm)$)
  {样本均值与样本方差的抽样分布};
