...
\begin{tikzpicture}[
  ...
  % 6 unique subsets directly
  \node[nd=red!15] (R1) at (2.0,0.8) {$\{A,B\}$};
  \node[nd=red!15] (R2) at (3.4,0.8) {$\{A,C\}$};
  \node[nd=red!15] (R3) at (4.8,0.8) {$\{A,D\}$};
  \node[nd=red!15] (R4) at (6.2,0.8) {$\{B,C\}$};
  \node[nd=red!15] (R5) at (7.0,-0.6) {$\{B,D\}$};
  \node[nd=red!15] (R6) at (5.6,-0.6) {$\{C,D\}$};
  ...
  \node[draw,rounded corners=3pt,fill=gray!10,font=\scriptsize,align=center]
    at (4.5,-0.1) {Order removed: $\{A,B\}=\{B,A\}$};
  \node[font=\small,red!80!black] at (4.5,-1.5)
    {$C(4,2) = P(4,2)/2! = 12/2 = 6$};

  %% Divider
  \draw[thick,dashed,gray!50] (0,-2.0) -- (0,3.5);
  \node[font=\bfseries\small,gray,align=center] at (0,-2.4)
