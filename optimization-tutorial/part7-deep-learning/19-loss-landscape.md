...
> **难度**：★★★☆☆
> **前置知识**：多元微积分（偏导数、Hessian 矩阵）、线性代数（特征值）、第18章自适应学习率
> **本文件**：融合"原版严格推导 + 速记 / 套路 / 自测"。保留原版完整正文（学习目标 / 19.1–19.5 / 深度学习应用 / 练习题）+ 在最前置一例速记 / 思维路径 + 最后追加方法总结与自测。

> **一例速记**：
> **临界点**：$\nabla \mathcal{L}(\boldsymbol{\theta}^*) = \mathbf{0}$；由 Hessian 特征值判断：全正 → 极小，全负 → 极大，有正有负 → 鞍点。
> **高维关键结论**：$n$ 维空间中随机临界点是极小的概率约 $p^n \approx 0$（$n=10^6$）；**高损失处几乎全是鞍点，低损失处才是极小**。
> **平坦 vs 尖锐**：Hessian 最大特征值 $\lambda_{\max}$ 小 → 平坦极小 → 泛化好；大批量训练 → 尖锐极小；小批量 / 大噪声 → 平坦极小。
> **SAM**：$\min_\theta \max_{\|\epsilon\| \leq \rho} \mathcal{L}(\theta+\epsilon)$，两步更新，主动找平坦极小，泛化更好但每步两次反传。
> **模式连通性**：两个独立解之间存在损失近似不变的弯曲路径（非直线），支持快照集成。

---

## 引入：一道反直觉的"高维局部极小"题

> **题目**：假设神经网络参数 $\boldsymbol{\theta} \in \mathbb{R}^n$，$n = 10^6$。假设 Hessian 每个特征值独立地以概率 $p = 0.5$ 为正。
> (1) 该参数点是局部极小（全部特征值为正）的概率是多少？
> (2) 实践中反复观察到 SGD 可以快速找到好的解，这与"高度非凸"的说法矛盾吗？

请先停下来想一想：深度网络损失函数高度非凸，直觉上似乎到处都是局部极小陷阱，但实验显示 SGD 从几乎任意初始点出发都能收敛到好的解。这一现象的根本原因是什么？

---

## 思维路径还原（解题者的内心独白）

> "第 (1) 问是概率计算。$n = 10^6$ 个特征值各自以 $p = 0.5$ 为正，**全部为正**的概率是 $p^n = (0.5)^{10^6}$，这是一个比 $10^{-300000}$ 还小得多的数，实际上等于零。
>
> 直觉立刻更新：**在高维空间里，随机临界点几乎不可能是局部极小**——需要所有特征值同时为正，这是指数级稀有事件。
>
> 第 (2) 问需要更深的理解。'高度非凸'确实成立，但这不意味着'到处都是局部极小陷阱'——它意味着'到处都是鞍点'。鞍点有负曲率方向，梯度下降（特别是带噪声的 SGD）会沿负曲率方向逃逸。
>
> **Dauphin et al. (2014) 的关键发现**：高损失临界点几乎全是鞍点（有很多负特征值，逃逸容易）；低损失临界点才接近局部极小（少数负特征值，但损失值已经很低）。换句话说，能被 SGD 卡住的局部极小，其损失值就已经很低了，等价于"好的解"。
>
> **所以 SGD 在非凸曲面上训练效果好的原因**：不是因为它"找到了全局极小"，而是因为低损失区域的临界点几乎都是极小，且这些极小的质量相差不大（特别是过参数化网络）。这是深度学习成功的几何基础。"
...
### 图 19-1：损失景观（鞍点 / 平坦极小 / 尖锐极小）

![神经网络损失景观：多鞍点、局部极小与平坦 vs 尖锐极小对比](../figures/svg/opt-p7-19-1.svg)

### 图 19-2：平坦 vs 尖锐极小（泛化角度）

![平坦极小与尖锐极小泛化对比：训练测试损失差距](../figures/svg/opt-p7-19-2.svg)
...
| 名称 | 公式 / 判据 | 关键性质 |
|---|---|---|
| **临界点** | $\nabla \mathcal{L}(\theta^*) = \mathbf{0}$ | 梯度为零是必要条件，不充分 |
| **鞍点判断** | Hessian 有正有负特征值 | 高维中主导型，低损失处罕见 |
| **极小点** | Hessian 全正特征值 | 低损失区常见；全局极小因对称性大量存在 |
| **平坦度** | $\lambda_{\max}(\mathbf{H})$ 小；$\text{Tr}(\mathbf{H})$ 小 | 平坦 → 泛化好；小批量 → 平坦 |
| **SAM 目标** | $\min_\theta \max_{\|\epsilon\|\leq\rho}\mathcal{L}(\theta+\epsilon)$ | 两步更新；$\rho$ 控制邻域半径 |

### 临界点判别 3 步

1. **令梯度为零**：$\nabla \mathcal{L} = \mathbf{0}$，解出候选点 $\theta^*$
2. **计算 Hessian**：$\mathbf{H} = \nabla^2 \mathcal{L}(\theta^*)$，分析特征值符号
3. **判断类型**：全正 → 极小；全负 → 极大；有正有负 → 鞍点；有零 → 退化，需高阶导数

### Filter Normalization 可视化 2 步

1. **生成两个随机方向** $\boldsymbol{\delta}, \boldsymbol{\eta}$，对每个"滤波器"做范数归一化（与参数同范数）
2. **在二维网格上采样**：$\mathcal{L}(\alpha, \beta) = \mathcal{L}(\theta^* + \alpha\boldsymbol{\delta} + \beta\boldsymbol{\eta})$，绘制等高线
...
当 Hessian 有零特征值时，需看四阶导数（Taylor 展开高阶项）才能判断极小类型。例 $f(x) = x^4$ 在 $x = 0$ 处 $f'' = 0$ 但 $f^{(4)} = 24 > 0$，是极小点。

### 变形 2：高维概率修正

若特征值不等概率，$P(\text{局部极小}) = \prod_{i=1}^n p_i^+$。当网络已训练到低损失区，负特征值比例极小，局部极小比例上升，但仍以 SGD 发现的"近似极小"为主。

### 变形 3：Hessian 迹的快速估计

直接计算 $n\times n$ Hessian 代价 $O(n^2)$，用 Hutchinson 估计器：$\text{Tr}(\mathbf{H}) \approx \frac{1}{k}\sum_{i=1}^k \mathbf{z}_i^\top \mathbf{H} \mathbf{z}_i$，其中 $\mathbf{z}_i \sim \text{Rademacher}(\pm 1)$，每次只需一个 HVP（Hessian-向量积），代价等同一次反传。

### 变形 4：SAM 的变种

**mSAM**（mini-batch SAM）：只用一半 batch 计算扰动，另一半计算梯度，降低方差。**ASAM**（自适应 SAM）：用自适应的 $\rho$ 解决尺度不变性问题，消除 Dinh et al. 指出的尺度变换漏洞。
...
| 概念 | 核心结论 | 实践 takeaway |
|---|---|---|
| 鞍点主导性 | 高维中几乎所有临界点是鞍点 | 不必担心"卡在局部极小"，真正的障碍是鞍点 |
| 低损失等价性 | 低损失极小质量相近 | SGD 找到的任意低损失解泛化能力相近 |
| 平坦极小假说 | 平坦 → 更好泛化（PAC-Bayes 支持） | 用小批量 / 大噪声 / SAM 鼓励平坦极小 |
| 模式连通性 | 独立解之间有低损失路径 | 参数空间平均效果差；曲线路径集成效果好 |
| 可视化注意 | Filter Normalization 消除尺度 | 用统一尺度才能公平比较不同网络/优化器 |

---

## 思考路标（条件反射）

1. 看到"临界点" → 梯度为零；看 Hessian 特征值判断类型
2. 看到"高维极小" → 几乎全是鞍点（$p^n \to 0$）；低损失区才有极小
3. 看到"鞍点逃逸慢" → SGD 噪声帮助逃逸；逃逸时间 $\sim \frac{1}{\lambda}\ln(\delta/\sigma)$
4. 看到"平坦极小" → Hessian $\lambda_{\max}$ 小；泛化更好；小批量/SAM 鼓励
5. 看到"大批量训练泛化差" → 尖锐极小假说；线性缩放学习率缓解但不能完全弥补
6. 看到"损失曲面可视化" → Filter Normalization 必须用（否则尺度不可比）
7. 看到"SAM" → 两步更新：先扰动 $\hat\epsilon = \rho\nabla/\|\nabla\|$，再用扰动处梯度更新
8. 看到"模式连通性" → 两个独立解之间有低损失路径（非直线），支持快照集成
9. 看到"ResNet vs VGG 损失曲面" → ResNet 曲面更平滑，归功于残差连接
10. 看到"BN 与损失曲面" → BN 平滑损失曲面，使梯度更 Lipschitz，可用更大 LR

---

## 易错点

1. **混淆鞍点与局部极小**：在低维直觉里，极小点"常见"，鞍点是过渡。高维中完全相反。"我的网络卡在局部极小了"——更可能是卡在鞍点附近，SGD 噪声不够大逃逸。

2. **平坦极小假说的漏洞**：Dinh et al. (2017) 用 ReLU 尺度不变性构造了"同一函数、任意平坦度"的例子。因此 Hessian 特征值不是普适的泛化预测指标——需配合尺度不变的度量（如 ASAM 的自适应 $\rho$）。

3. **Filter Normalization 不等于真实曲面**：可视化是二维投影，可能遗漏高维中的重要结构。两张可视化图的"形状相似"不代表曲面全局等价。

4. **SAM 的计算代价**：每步两次前后向传播，代价约为 SGD 的 2 倍。在资源受限时，可用 mSAM（只对部分 batch 做扰动）降低代价。

5. **模式连通性 $\neq$ 参数平均**：直线插值通常经过高损失势垒，而弯曲路径才是低损失通道。直接做模型参数平均（Model Soups 等）在权重对齐时有效，否则效果差。
...
> **题目**：函数 $f(x, y, z) = x^2 - y^2 + z^4$。
> (1) 找出所有临界点；(2) 判断类型；(3) 梯度下降从 $(0.1, 0.1, 0.1)$ 出发（步长 0.1），前几步会往哪个方向主要移动？

【思路】对三个变量求偏导，令全为零。

【解】
(1) $f_x = 2x = 0,\ f_y = -2y = 0,\ f_z = 4z^3 = 0$ → 唯一临界点 $(0, 0, 0)$。
(2) Hessian 对角为 $(2, -2, 0)$（退化）：$x$ 方向正曲率，$y$ 方向负曲率，$z$ 方向零曲率（需四阶判断：$f_{zzzz} = 24 > 0$，极小方向）。综合：有正有负特征值，**鞍点**。
(3) 从 $(0.1, 0.1, 0.1)$ 出发，梯度 $= (0.2, -0.2, 0.004)$。梯度下降：$x$ 方向收缩（正曲率），$y$ 方向扩散（负曲率，梯度反号后 $y$ 增大）。即主要向 $y$ 增大方向运动，逃离鞍点的 $y$ 方向。

【答案】$\boxed{(0,0,0) \text{ 是鞍点}}$；梯度下降主要沿 $y$ 正方向逃离。

### 例 2：批大小与极小点形状

> **题目**：用 ResNet-18 在 CIFAR-10 上分别用 batch size = 64 和 2048 训练（学习率线性缩放）。预测哪种 batch 收敛到更平坦的极小，并用 Hessian 迹的增减说明。

【思路】小批量 SGD 有更高噪声方差（$\sigma^2 \propto 1/\sqrt{m}$），等效于在轻微扰动曲面上优化，倾向宽谷。

【解】batch = 64 噪声大 → 倾向平坦极小，$\text{Tr}(\mathbf{H})$ 较小；batch = 2048 梯度更准确 → 倾向尖锐极小，$\text{Tr}(\mathbf{H})$ 较大。即使用学习率线性缩放（$\eta_{2048} = 32\eta_{64}$），大批量仍然有"泛化差距"扩大的倾向（Keskar et al., 2017）。

【答案】$\boxed{\text{batch=64 更平坦（Tr(H) 更小），batch=2048 更尖锐（Tr(H) 更大）}}$。

### 例 3：SAM 两步更新验证

> **题目**：设当前参数 $\theta = (1.0, 0.5)^T$，梯度 $\nabla = (0.6, 0.8)^T$，$\rho = 0.1$。
> (1) 计算扰动 $\hat\epsilon$；(2) 参数移至扰动位置；(3) 若扰动处梯度为 $(0.5, 0.7)^T$，基础优化器为 SGD（$\eta = 0.01$），写出最终参数更新。

【解】
(1) $\|\nabla\| = \sqrt{0.36 + 0.64} = 1.0$，$\hat\epsilon = 0.1 \times (0.6, 0.8) / 1.0 = (0.06, 0.08)^T$。
(2) 扰动位置：$\theta + \hat\epsilon = (1.06, 0.58)^T$。
(3) 用扰动处梯度 $(0.5, 0.7)^T$ 做 SGD 更新，还原到原始参数后再更新：$\theta_{\text{new}} = (1.0, 0.5) - 0.01 \times (0.5, 0.7) = (0.995, 0.493)^T$。

【答案】$\boxed{\theta_{\text{new}} = (0.995, 0.493)^T}$。

---

## 自测题

**自测 1**　二维函数 $f(x, y) = x^4 + 2x^2 y + y^2$。(1) 求所有临界点；(2) 判断类型。

> 提示：$f_x = 4x^3 + 4xy = 4x(x^2 + y) = 0$；$f_y = 2x^2 + 2y = 0 \Rightarrow y = -x^2$。代入得 $4x(x^2 - x^2) = 0$，所有满足 $y = -x^2$ 的点均为临界点！Hessian 在这些点上有一个零特征值——退化临界点，$f(x, -x^2) = 0$ 是全局极小值。这说明 ReLU 网络的尺度对称性造成的等值流形：损失在整个流形上恒为极小。

**自测 2**　某神经网络的 Hessian 迹 $\text{Tr}(\mathbf{H}) = 50$（大批量训练）和 $\text{Tr}(\mathbf{H}) = 8$（小批量训练）。哪个更可能泛化更好？

> 提示：Tr 越小 → 平均曲率越小 → 更平坦极小 → 泛化更好。小批量 $\text{Tr}=8$ 更好。注意：需用同一网络同一任务比较，避免不同架构干扰。

**自测 3**　SAM 的外层目标是 $\min_\theta \max_{\|\epsilon\| \leq \rho} \mathcal{L}(\theta+\epsilon)$。如果 $\rho \to 0$，SAM 退化为什么方法？如果 $\rho \to \infty$，SAM 会出现什么问题？

> 提示：$\rho \to 0$：扰动为零，退化为普通 SGD。$\rho \to \infty$：内层最大化无界，扰动方向 $\propto \nabla$（梯度方向为最陡上升），最终在参数空间跳跃极大，训练不稳定/发散。最优 $\rho$ 在 $0.01$ 到 $0.2$ 之间，需调参。

**自测 4**　Filter Normalization 中，若不做归一化而直接用随机方向 $\mathbf{d}$（各层范数差异大），可视化会出现什么问题？

> 提示：若某层权重范数很小（如经 L2 正则后），随机方向该层扰动相对偏大（扰动幅度 $\alpha |\mathbf{d}_{l}|$ 远大于该层权重），损失曲面被"小权重层"主导，呈现人为尖锐。Filter Normalization 使每层扰动与该层权重范数匹配，避免此偏差。

**自测 5**　某人用线性插值 $(1-t)\theta_A + t\theta_B$（$t\in[0,1]$）连接两个独立训练的解，发现插值路径上损失在 $t=0.5$ 处大幅上升（势垒）。这说明什么？能否改进？

> 提示：线性插值经过高损失区域 → 两解之间存在势垒 → 直接模型平均效果差。改进：用 Garipov et al. (2018) 的"模式连通性"弯曲路径（贝塞尔曲线，中间点作为可学习参数），使整条路径损失接近端点 → 低损失路径存在，可做路径上的集成（快照集成）。

---

**回头看一眼"一例速记"**：

> 临界点由 Hessian 特征值判断；高维中鞍点主导、低损失处才有极小。
> 平坦极小由 $\lambda_{\max}$ 度量，小批量/SAM 鼓励。
> 模式连通性：弯曲路径连通两解，直线插值有势垒。
...
| 段落 | 来源 | 价值 |
|---|---|---|
| 一例速记 + 引入 + 思维路径还原 | 重写版（前置） | 建立直觉 / 反直觉校正 |
