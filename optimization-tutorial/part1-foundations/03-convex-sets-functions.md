...
> **一例速记**：
> **凸集**：任意两点连线段仍在集合内；$\theta\mathbf{x}+(1-\theta)\mathbf{y}\in C$（$\theta\in[0,1]$）。交集保凸，仿射变换保凸，并集**不保凸**。
> **凸函数**：弦在图像上方；一阶条件：切平面是全局下界 $f(\mathbf{y})\geq f(\mathbf{x})+\nabla f^\top(\mathbf{y}-\mathbf{x})$；二阶条件：Hessian $\succeq 0$。
> **Jensen 不等式**：$f(\mathbb{E}[X]) \leq \mathbb{E}[f(X)]$（$f$ 凸）；推出 KL 散度非负、AM-GM 等。
> **强凸**：$\nabla^2 f \succeq mI$（$m>0$）$\Rightarrow$ 唯一全局极小 + 线性收敛；L2 正则化使问题强凸。
> **AI 关联**：逻辑回归损失凸 $\Rightarrow$ 全局最优有保证；神经网络损失非凸 $\Rightarrow$ 大量鞍点；ReLU 本身是凸函数但多层复合非凸；条件数 $\kappa = L/m$ 决定收敛速度。

---

## 引入：凸优化 vs 非凸优化——一个直觉题

> **题目**：设 $f_1(x) = x^2 - 4x + 5$ 和 $f_2(x) = x^4 - 4x^3 + 3x^2 + 2x$。(1) 判断 $f_1, f_2$ 是否为凸函数（用二阶导数判断）；(2) 对凸函数，梯度下降会不会陷入局部极小？(3) 对非凸函数，梯度下降从不同起点出发会得到不同结果吗？

请先停下来想一想：$f''(x) \geq 0$ 等价于凸函数；凸函数的局部极小 = 全局极小；非凸函数可能有多个局部极小。
...
> "两个函数，先判断凸性再分析优化性质。
>
> **$f_1(x) = x^2 - 4x + 5$**：$f_1'(x) = 2x - 4$，$f_1''(x) = 2 > 0$ 恒成立，**严格凸**。
>
> 驻点：$f_1'(x) = 0 \Rightarrow x = 2$，$f_1(2) = 1$。因为是凸函数，这是**唯一全局最小值**。
>
> 梯度下降从任何起点出发，都会收敛到 $x=2$，不会卡在别处。
>
> **$f_2(x) = x^4 - 4x^3 + 3x^2 + 2x$**：$f_2''(x) = 12x^2 - 24x + 6 = 6(2x^2-4x+1)$。
>
> $f_2''(0) = 6 > 0$，但 $f_2''(1) = 6(2-4+1) = -6 < 0$，**Hessian 不恒非负 $\Rightarrow$ 非凸**。
>
> 求驻点：$f_2'(x) = 4x^3 - 12x^2 + 6x + 2 = 0$，有多个实根。$f_2''(x)<0$ 处是局部极大，$f_2''(x)>0$ 处是局部极小——**多个局部极小**。
>
> 从不同起点出发的梯度下降会陷入不同的局部极小。深度神经网络的损失函数正是类似 $f_2$ 的非凸函数，这就是为什么实践中需要多次随机初始化，或使用 Adam / SGD + 动量等方法探索更好的解。
>
> **关键对比**：凸优化有理论保障（梯度为 0 = 全局最优）；非凸优化只能找到局部最优，但足够大的过参数化网络在实践中通常能找到质量很好的解。"
...
![椭圆（凸）与月牙形（非凸）对比](../figures/svg/opt-p1-03-1.svg)

### 图 3-2：凸函数 vs 非凸函数

![碗形与多峰函数对比](../figures/svg/opt-p1-03-2.svg)
...
| 名称 | 公式 | 关键性质 |
|---|---|---|
| **凸集定义** | $\theta\mathbf{x}+(1-\theta)\mathbf{y}\in C$，$\theta\in[0,1]$ | 线段在集合内 |
| **凸函数定义** | $f(\theta\mathbf{x}+(1-\theta)\mathbf{y})\leq\theta f(\mathbf{x})+(1-\theta)f(\mathbf{y})$ | 弦在图像上方 |
| **一阶条件** | $f(\mathbf{y})\geq f(\mathbf{x})+\nabla f(\mathbf{x})^\top(\mathbf{y}-\mathbf{x})$ | 切平面全局下界 |
| **二阶条件** | $\nabla^2 f(\mathbf{x})\succeq 0$ | Hessian 半正定 |
| **Jensen 不等式** | $f(\mathbb{E}[X])\leq\mathbb{E}[f(X)]$ | $f$ 凸时成立 |
| **强凸** | $\nabla^2 f\succeq mI$（$m>0$）| 唯一极小，收敛率 $(1-1/\kappa)^k$ |
| **L-光滑** | $\|\nabla f(\mathbf{x})-\nabla f(\mathbf{y})\|\leq L\|\mathbf{x}-\mathbf{y}\|$ | 步长 $\leq 1/L$ 安全 |
| **条件数** | $\kappa = L/m$ | 决定梯度下降收敛速度 |

### 判断凸集的 3 步流程

1. 直接验证定义：任取两点 $\mathbf{x},\mathbf{y}\in C$，验证凸组合 $\in C$
2. 利用保凸运算：能否表示为凸集的交集 / 仿射变换的像
3. 如是函数的下水平集：先证函数是凸函数，下水平集自然凸
...
(a) **定义**：直接验证不等式 $f(\theta\mathbf{x}+(1-\theta)\mathbf{y})\leq\theta f(\mathbf{x})+(1-\theta)f(\mathbf{y})$；

(b) **一阶**：$f(\mathbf{y})\geq f(\mathbf{x})+\nabla f^\top(\mathbf{y}-\mathbf{x})$（可微情形）；

(c) **二阶**：$\nabla^2 f\succeq 0$（二次可微情形，最常用）；

(d) **保凸运算**：非负线性组合、逐点上确界、仿射复合均保凸。

### 变形 2：从凸到强凸

加 L2 正则项 $\lambda\|\mathbf{w}\|^2/2$（$\lambda > 0$）可将凸损失变为 $\lambda$-强凸：条件数 $\kappa = (L+\lambda)/\lambda$。$\lambda$ 越大，条件数越小，收敛越快（但正则化越强，解离真实值越远）。

### 变形 3：Jensen 不等式的使用步骤

1. 确定 $f$ 是凸函数（通常 $f'' > 0$）
2. 确定 $\theta_i\geq 0, \sum\theta_i=1$（权重归一）
3. 写 Jensen：$f(\sum\theta_i x_i)\leq\sum\theta_i f(x_i)$
4. 概率形式：$f(\mathbb{E}[X])\leq\mathbb{E}[f(X)]$（$f$ 凸）/ $\geq$（$f$ 凹）

### 变形 4：非凸情形的实用对策

虽然深度学习损失非凸，但以下策略有效：(a) 多次随机初始化，取最优；(b) 过参数化（宽网络）使局部极小质量接近全局；(c) SGD 的随机噪声帮助逃离鞍点和浅局部极小；(d) 批归一化改善条件数，加速收敛。

---

## 思考路标（条件反射）

1. 看到"判断集合是否为凸集" → 先试交集 / 半空间表示；再直接验证线段在集合内
2. 看到"判断函数凸性" → 一阶：画切线看是否全局下界；二阶：Hessian $\succeq 0$
3. 看到"Jensen 不等式" → 确认凸函数 + 概率权重 $\sum\theta_i=1$，则 $f(\mathbb{E})\leq\mathbb{E}[f]$
4. 看到"局部最优 = 全局最优" → 凸函数的关键性质；非凸时**不成立**
5. 看到"KL 散度 $D_{KL}(P\|Q)\geq 0$" → Jensen 不等式 + $-\log$ 是凸函数
6. 看到"加 L2 正则化 $\lambda\|\mathbf{w}\|^2$" → 变强凸，条件数改善为 $(L+\lambda)/\lambda$
7. 看到"逻辑回归损失" → 凸函数（Hessian 半正定）；梯度下降保证收敛
8. 看到"神经网络损失" → 非凸；鞍点为主；局部极小质量通常较好（过参数化时）
9. 看到"ReLU 是否凸" → **是**（$f'' = 0 \geq 0$）；多层复合**非凸**
10. 看到"条件数 $\kappa$" → $= L/m$；$\kappa = 1$ 最理想；$\kappa \gg 1$ 收敛慢
11. 看到"支撑超平面" → 凸集边界点处存在，切线 / 切平面不穿入集合内部

---

## 易错点

1. **凸集的并集不保凸**：两个圆盘的交集是凸的，但并集通常不是（连接两圆盘的"哑铃形"有凹陷）。只有交集、仿射变换的像/原像才保凸。

2. **下水平集凸 $\not\Rightarrow$ 函数凸**：$f(x)=x^3$ 的下水平集 $\{x \mid x^3\leq\alpha\} = (-\infty, \alpha^{1/3}]$ 是凸集，但 $f$ 不是凸函数（在 $x<0$ 段凹）。满足"下水平集均凸"的函数叫**拟凸函数**，比凸函数更广泛。

3. **Jensen 方向**：凸函数 $f$ 有 $f(\mathbb{E}[X])\leq\mathbb{E}[f(X)]$；凹函数（如 $\log$）反向 $f(\mathbb{E}[X])\geq\mathbb{E}[f(X)]$。搞反方向是最常见的错误。

4. **强凸 $\neq$ 凸**：凸函数包括线性函数（既凸又凹），但线性函数不强凸。强凸 $\Rightarrow$ 严格凸 $\Rightarrow$ 凸，但反向均不成立。

5. **凸函数局部极小是全局极小，但不一定唯一**：凸函数可能有多个全局极小（如 $f(x)=0$ 在 $[0,1]$ 上每点都是极小）。只有**严格凸**才保证最多一个极小；**强凸**才保证恰好一个极小。
...
> **题目**：判断以下集合是否为凸集：(1) $S_1 = \{(x,y)\mid x^2+y^2\leq 4\}$；(2) $S_2 = \{(x,y)\mid x^2+y^2\geq 1\}$；(3) $S_3 = S_1\cap\{(x,y)\mid x+y\leq 2\}$。

【思路】(1)(2) 直接验证或举反例；(3) 利用"凸集交集保凸"。

【解】
(1) 对任意 $\mathbf{x},\mathbf{y}\in S_1$（$\|\mathbf{x}\|\leq 2,\|\mathbf{y}\|\leq 2$），凸组合的范数 $\|\theta\mathbf{x}+(1-\theta)\mathbf{y}\|\leq\theta\|\mathbf{x}\|+(1-\theta)\|\mathbf{y}\|\leq 2$。**凸集**。

(2) 取 $\mathbf{x}=(1,0), \mathbf{y}=(-1,0)\in S_2$，中点 $(0,0)$ 满足 $\|(0,0)\|=0<1$，不在 $S_2$。**非凸集**。

(3) $S_1$ 是凸集（已证）；半平面 $\{x+y\leq 2\}$ 是凸集（超平面半空间）；两凸集交集仍是凸集。**凸集**。

【答案】$\boxed{S_1 \text{ 凸},\ S_2 \text{ 非凸},\ S_3 \text{ 凸}}$。

### 例 2：Jensen 不等式应用

> **题目**：设 $X$ 是随机变量，$P(X=1)=0.3, P(X=3)=0.7$。(1) 计算 $e^{\mathbb{E}[X]}$ 和 $\mathbb{E}[e^X]$；(2) 验证 Jensen 不等式；(3) 用 Jensen 证明 AM-GM：$\frac{a+b}{2}\geq\sqrt{ab}$（$a,b>0$）。

【解】
(1) $\mathbb{E}[X] = 0.3\times 1 + 0.7\times 3 = 2.4$；$e^{\mathbb{E}[X]} = e^{2.4}\approx 11.02$。

$\mathbb{E}[e^X] = 0.3\times e^1 + 0.7\times e^3 = 0.3\times 2.718 + 0.7\times 20.09 \approx 14.88$。

(2) $e^{2.4}\approx 11.02 < 14.88\approx\mathbb{E}[e^X]$，验证 Jensen 不等式 $f(\mathbb{E}[X])\leq\mathbb{E}[f(X)]$（$f=e^x$ 凸）。

(3) 令 $f(t) = -\log t$（凸函数，$f'' = 1/t^2 > 0$），$X\in\{a,b\}$，权重各 $1/2$：

$f(\mathbb{E}[X])\leq\mathbb{E}[f(X)] \Rightarrow -\log\frac{a+b}{2}\leq\frac{-\log a - \log b}{2} = -\log\sqrt{ab}$

即 $\log\frac{a+b}{2}\geq\log\sqrt{ab}$，故 $\frac{a+b}{2}\geq\sqrt{ab}$。$\square$

【答案】$\boxed{e^{2.4}\approx 11.02 < 14.88;\text{ AM-GM 用} -\log \text{ 的 Jensen 证得}}$。

### 例 3：L2 正则化与强凸性

> **题目**：逻辑回归损失 $\ell(\mathbf{w}) = \frac{1}{N}\sum_{i=1}^N\log(1+e^{-y_i\mathbf{w}^\top\mathbf{x}_i})$，其 Hessian 满足 $\mathbf{0}\preceq H_\ell\preceq \frac{1}{4N}\|X\|_F^2 I$（光滑参数 $L = \|X\|_F^2/(4N)$，强凸参数 $m=0$）。加 L2 正则 $\lambda\|\mathbf{w}\|^2/2$ 后，(1) 新的强凸参数和光滑参数是什么？(2) 条件数如何？(3) $\lambda$ 越大对优化有何影响？

【解】
(1) $H_{\text{new}} = H_\ell + \lambda I$，特征值范围 $[\lambda, L+\lambda]$，强凸参数 $m = \lambda$，光滑参数 $L_{\text{new}} = L+\lambda$。

(2) 条件数 $\kappa = (L+\lambda)/\lambda$。当 $\lambda\to\infty$ 时 $\kappa\to 1$（收敛最快）。

(3) $\lambda$ 越大：优化越快（条件数小）；但正则化越强，解越偏向 $\mathbf{w}=\mathbf{0}$，泛化与拟合的权衡越不利。$\lambda$ 的选择是优化速度与统计效率的权衡。

【答案】$\boxed{m=\lambda,\ L_{\text{new}}=L+\lambda,\ \kappa=(L+\lambda)/\lambda;\text{ $\lambda$ 越大优化越快但正则化越强}}$。

---

## 自测题

**自测 1**　判断 $f(x) = x\log x$（$x>0$）是否为凸函数。用二阶导数验证，并写出相应的 Jensen 不等式。

> 💡 提示：$f'(x) = \log x + 1$，$f''(x) = 1/x > 0$，**凸函数**。Jensen：$\mathbb{E}[X\log X]\geq\mathbb{E}[X]\log\mathbb{E}[X]$（非负熵不等式）。

**自测 2**　设 $C_1 = \{(x,y)\mid y\geq x^2\}$（抛物线上方），$C_2 = \{(x,y)\mid y\leq 4\}$。证明 $C_1$ 是凸集，并判断 $C_1\cap C_2$ 的凸性。

> 💡 提示：$C_1$ 是凸函数 $g(x)=x^2$ 的上图集（epigraph），上图集凸 $\Leftrightarrow$ 函数凸，故 $C_1$ 凸。$C_2$ 是半平面，凸。$C_1\cap C_2$ 是两凸集的交，仍凸——是抛物线与水平线围成的"碗状"区域。

**自测 3**　证明：若 $f_1, f_2$ 是凸函数，则 $f(x) = \max(f_1(x), f_2(x))$ 也是凸函数。

> 💡 提示：对任意 $\theta\in[0,1]$：$\max(f_1,f_2)(\theta x+(1-\theta)y) = \max(f_1(\theta x+(1-\theta)y), f_2(\theta x+(1-\theta)y)) \leq \max(\theta f_1(x)+(1-\theta)f_1(y), \theta f_2(x)+(1-\theta)f_2(y)) \leq \theta\max(f_1,f_2)(x) + (1-\theta)\max(f_1,f_2)(y)$。

**自测 4**　Hinge 损失 $\ell(z) = \max(0, 1-z)$（SVM 用）。(1) 判断 $\ell$ 是否为凸函数；(2) 在 $z=1$ 处 $\ell$ 是否可微？次梯度是什么？

> 💡 提示：(1) $\ell$ 是两个凸函数 $0$ 和 $1-z$（线性 = 凸）的逐点最大，故**凸**。(2) $z=1$ 处不可微（折点）；次梯度集合 $= [-1, 0]$（左导数 $-1$，右导数 $0$）。优化算法选择次梯度 $\in [-1,0]$ 中的任意值均可。

**自测 5**　ReLU 网络：单个 ReLU 是凸函数，但两层 ReLU 网络 $f(x) = \text{ReLU}(\text{ReLU}(x)-1)$ 是否是凸函数？给出反例或证明。

> 💡 提示：取 $x_1=0, x_2=3, \theta=0.5$；$f(0)=0, f(3)=\text{ReLU}(\text{ReLU}(3)-1)=\text{ReLU}(2)=2$；$\frac{f(0)+f(3)}{2}=1$。$f(1.5)=\text{ReLU}(\text{ReLU}(1.5)-1)=\text{ReLU}(0.5)=0.5<1$。凸性不等式违反（$0.5 < 1$），故**非凸**。凸函数的非线性复合通常破坏凸性。

---

**回头看一眼"一例速记"**：

> 凸集：线段在集合内；交集保凸，并集不保凸。
> 凸函数：弦在图上方；一阶切平面下界；二阶 Hessian $\succeq 0$；局部最优 = 全局最优。
> Jensen：$f$ 凸 $\Rightarrow f(\mathbb{E}[X])\leq\mathbb{E}[f(X)]$；强凸 = 凸 + 最小特征值 $\geq m > 0$。
