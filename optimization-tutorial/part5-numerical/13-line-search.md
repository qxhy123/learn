...
> **难度**：★★★☆☆
> **前置章节**：第5章（梯度下降法）、第6章（牛顿法与拟牛顿法）
> **本文件**：融合"原版严格推导 + 速记 / 套路 / 自测"。保留原版完整正文 + 在最前置一例速记 / 思维路径 + 最后追加方法总结与自测。

> **一例速记**：
> **框架**：$\mathbf{x}_{k+1} = \mathbf{x}_k + \alpha_k \mathbf{d}_k$；$\phi(\alpha) = f(\mathbf{x}_k + \alpha\mathbf{d}_k)$，$\phi'(0) < 0$（下降方向）。
> **Armijo 条件**：$\phi(\alpha) \leq \phi(0) + c_1 \alpha \phi'(0)$，$c_1 \in (0,1)$（典型 $10^{-4}$）；排除步长太大。
> **Wolfe 条件**：Armijo + 曲率条件 $\phi'(\alpha) \geq c_2 \phi'(0)$，$c_2 \in (c_1,1)$（典型 $0.9$）；排除步长太小。
> **强 Wolfe**：将曲率条件改为 $|\phi'(\alpha)| \leq c_2 |\phi'(0)|$；保证步长远离极大值，适配拟牛顿法。
> **回溯**：初始 $\alpha=1$，若不满足 Armijo 则乘 $\beta \in (0,1)$ 缩短，有限步内终止。
> **AI 关联**：学习率调度 = 非精确线搜索的工程简化；余弦退火 / ReduceLROnPlateau 均是对 Wolfe 思想的启发式替代。

---

## 引入：一步"看似合理"的梯度步为何让损失飙升？

> **题目**：对目标函数 $f(x) = x^4 - 4x^2 + 4$，当前点 $x_k = 2$，梯度 $f'(2) = 4\cdot8 - 8\cdot2 = 16$，故搜索方向 $d_k = -16$（负梯度）。取步长 $\alpha = 0.3$，新点 $x_{k+1} = 2 + 0.3\times(-16) = -2.8$。
>
> 请问：(1) 新点的函数值 $f(-2.8)$ 是多少？(2) 它满足 Armijo 条件吗（取 $c = 10^{-4}$）？(3) 为什么直觉上"朝下降方向走一步"会使函数值大幅上升？

请先停下来想一想：方向正确不代表步长合适。步长过大时，虽然方向指向"下坡"，但可能越过谷底、爬上对面的山坡。下面还原完整解题思路。

---

## 思维路径还原（解题者的内心独白）

> "给了 $f(x) = x^4 - 4x^2 + 4 = (x^2 - 2)^2$，最小值在 $x = \pm\sqrt{2} \approx \pm 1.414$，最小函数值为 $0$。
>
> **第 (1) 问：计算 $f(-2.8)$。**
>
> $f(-2.8) = (-2.8)^4 - 4(-2.8)^2 + 4 = 61.4656 - 31.36 + 4 = 34.106$。
>
> 原来 $f(2) = 16 - 16 + 4 = 4$，新点函数值从 $4$ 跳升到 $34.1$，明显上升了！
>
> **第 (2) 问：验证 Armijo 条件。**
>
> $\phi(0) = f(2) = 4$，$\phi'(0) = f'(2) \cdot d_k = 16 \times (-16) = -256$（方向导数）。
>
> Armijo 右侧：$\phi(0) + c\alpha\phi'(0) = 4 + 10^{-4} \times 0.3 \times (-256) = 4 - 0.00768 = 3.992$。
>
> Armijo 要求 $\phi(\alpha) \leq 3.992$，而 $\phi(0.3) = 34.1 \gg 3.992$。**不满足！** 回溯线搜索会立即缩短步长。
>
> **第 (3) 问：直觉解释。**
>
> $d_k = -16$ 方向确实是下降方向（$\phi'(0) = -256 < 0$），但步长 $\alpha = 0.3$ 太大——它把 $x = 2$ 一次带到 $x = -2.8$，越过了 $x = -\sqrt{2}$ 处的极小值，继续向右（$x$ 绝对值方向）冲上了另一侧更高的位置。Armijo 条件正是"早期预警系统"，通过与线性预测值比较，及时拦截这类越界步。
>
> **延伸思考**：若改用 $\alpha = 0.01$，$x_{k+1} = 2 + 0.01 \times (-16) = 1.84$，$f(1.84) \approx (1.84^2-2)^2 = (3.3856-2)^2 = 1.921 < 4$。这个步长满足 Armijo 条件，函数值确实下降了。然而 $\alpha = 0.01$ 可能又太小（离 Wolfe 曲率条件差远），导致进展缓慢。线搜索的核心挑战就在于：**既不太大、又不太小**。"
...
线搜索在不同搜索方向下的全局收敛保证（Nocedal & Wright, 2006）：

| 搜索方向 $\mathbf{d}_k$ | 使用 Wolfe 条件 | 收敛性保证 |
|----------------------|--------------|----------|
| 梯度下降 $-\nabla f$ | 是 | $\liminf\|\nabla f_k\| = 0$（全局） |
| 牛顿法（精确 Hessian） | 是（步长 $\leq 1$） | 局部二次收敛（靠近 $\mathbf{x}^*$ 后 $\alpha_k = 1$ 被接受） |
| BFGS | Wolfe 条件 | 超线性局部收敛 + 全局收敛（满足 Dennis-Moré 条件） |
| L-BFGS | Wolfe 条件 | 全局收敛，线性局部收敛 |

**关键引理（Zoutendijk 条件）**：若 $\mathbf{d}_k$ 满足 $\cos\theta_k = -\frac{\nabla f_k^\top\mathbf{d}_k}{\|\nabla f_k\|\|\mathbf{d}_k\|} \geq \delta > 0$（方向不趋于垂直梯度），则 Wolfe 线搜索保证：

$$\sum_{k=0}^{\infty} \cos^2\theta_k \|\nabla f_k\|^2 < \infty$$

这意味着 $\cos^2\theta_k\|\nabla f_k\|^2 \to 0$，若 $\cos\theta_k \geq \delta > 0$，则 $\|\nabla f_k\| \to 0$，即收敛到驻点。

### Barzilai-Borwein 步长（学习率热身）

BB 步长利用相邻两步信息估计"局部曲率"，避免精确线搜索：

$$\alpha_k^{\text{BB1}} = \frac{\mathbf{s}_{k-1}^\top\mathbf{s}_{k-1}}{\mathbf{s}_{k-1}^\top\mathbf{y}_{k-1}}, \quad \alpha_k^{\text{BB2}} = \frac{\mathbf{s}_{k-1}^\top\mathbf{y}_{k-1}}{\mathbf{y}_{k-1}^\top\mathbf{y}_{k-1}}$$

其中 $\mathbf{s}_{k-1} = \mathbf{x}_k - \mathbf{x}_{k-1}$，$\mathbf{y}_{k-1} = \nabla f_k - \nabla f_{k-1}$。

BB 步长无需额外函数评估，收敛速度接近共轭梯度法，在非精确线搜索中广泛应用。

### 深度学习视角：Wolfe 条件的工程替代

| 工程策略 | 对应的线搜索理念 | 缺陷 |
|---------|---------------|------|
| 固定学习率 $\alpha$ | 无线搜索 | 对不同问题需手动调参 |
| 学习率衰减（固定周期） | 极粗糙的 Armijo 近似 | 不依赖实时损失信息 |
| ReduceLROnPlateau | 类 Armijo：损失不下降则缩减 | 反应滞后，不保证每步下降 |
| Cyclical LR | 无明确对应 | 理论保证弱，依赖经验 |
| **自适应优化器（Adam）** | 二阶矩估计隐式实现曲率感知 | 对非凸问题无理论保证 |

**结论**：严格的 Wolfe 条件线搜索在小批量随机梯度场景下成本过高（每次线搜索需多次前向传播），因此被上述启发式方法替代。但在全批量（batch）优化或拟牛顿法中，Wolfe 线搜索仍是不可或缺的收敛保证。

## 几何示意

### 图 13-1：Wolfe 条件可接受区间

![Armijo 下降 + 曲率改善](../figures/svg/opt-p5-13-1.svg)
...
| 名称 | 公式 / 条件 | 关键参数 |
|---|---|---|
| **辅助函数** | $\phi(\alpha) = f(\mathbf{x}_k + \alpha\mathbf{d}_k)$ | $\phi'(0) = \nabla f(\mathbf{x}_k)^\top\mathbf{d}_k < 0$ |
| **Armijo 条件** | $\phi(\alpha) \leq \phi(0) + c_1\alpha\phi'(0)$ | $c_1 \in (0,1)$，典型 $10^{-4}$ |
| **Wolfe 曲率条件** | $\phi'(\alpha) \geq c_2\phi'(0)$ | $c_2 \in (c_1,1)$，典型 $0.9$ |
| **强 Wolfe 曲率** | $\vert \phi'(\alpha)\vert  \leq c_2\vert \phi'(0)\vert$ | 比 Wolfe 更严，适配 L-BFGS |
| **回溯更新** | $\alpha \leftarrow \beta\alpha$（直到 Armijo 满足）| $\beta \in (0,1)$，典型 $0.5$ |
| **黄金分割比** | $\tau = (\sqrt{5}-1)/2 \approx 0.618$ | 每步区间缩至 $\tau$ 倍 |
| **割线法更新** | $\alpha_{k+1} = \alpha_k - \phi'(\alpha_k)\dfrac{\alpha_k-\alpha_{k-1}}{\phi'(\alpha_k)-\phi'(\alpha_{k-1})}$ | 超线性收敛，阶 $\approx 1.618$ |

### 判断线搜索结果的 5 步流程

1. **定辅助函数**：写出 $\phi(\alpha) = f(\mathbf{x}_k + \alpha\mathbf{d}_k)$，计算 $\phi(0)$ 和 $\phi'(0)$；验证 $\phi'(0) < 0$（若非下降方向则报错）
2. **确定方法**：小维度 / 函数值廉价 → 精确线搜索（黄金分割或割线法）；高维 / 梯度廉价 → Armijo 或 Wolfe 非精确线搜索
3. **执行回溯**（非精确）：初始 $\alpha = 1$，循环检查 Armijo：若不满足，$\alpha \leftarrow \beta\alpha$；有限步终止（由 $f$ 可微 + 下降方向保证）
4. **验证充分下降 / 曲率**（若用 Wolfe）：先检查 W1（Armijo），再检查 W2（$\phi'(\alpha) \geq c_2\phi'(0)$）；两者同时满足才算通过
5. **报告结果**：输出 $\alpha$、函数评估次数、新迭代点 $\mathbf{x}_{k+1} = \mathbf{x}_k + \alpha\mathbf{d}_k$

### 常用参数对照表

| 方法 | $c_1$ | $c_2$ | $\beta$ | 适用场景 |
|---|---|---|---|---|
| 梯度下降 + Armijo | $10^{-4}$ | — | $0.5$ | 通用，实现简单 |
| CG + Wolfe | $10^{-4}$ | $0.1$ | — | 共轭梯度，精确线搜索近似 |
| L-BFGS + 强 Wolfe | $10^{-4}$ | $0.9$ | — | 拟牛顿法，须保正定 |
| 牛顿法 + Armijo | $10^{-4}$ | — | $0.5$ | 初始步长 $\bar\alpha=1$ |
| 精确线搜索 | — | — | — | 低维 / 函数值廉价场景 |
...
$c_1 = 10^{-4}$ 是最宽松的 Armijo 参数，接受区域最大；$c_1 = 0.1$ 更严，要求每步下降量达线性预测的 $10\%$。宽松参数用于高维问题（每次函数评估代价高，尽早接受步长），严格参数用于低维精确求解。

### 变形 2：Wolfe 参数对算法类型的依赖

梯度下降类方法通常取 $c_2 = 0.9$（曲率条件宽松，减少线搜索次数）；共轭梯度法取 $c_2 = 0.1$（更严格，确保 $\beta_k$ 公式稳定）；L-BFGS 取 $c_2 = 0.9$ 并要求强 Wolfe 以保证 Hessian 近似正定。

### 变形 3：精确线搜索退化情形

二次函数 $\phi(\alpha) = a\alpha^2 + b\alpha + c$（$a > 0$）精确最优步长为 $\alpha^* = -b/(2a) = -\phi'(0)/(2a)$。对一般函数可用插值近似：用 $\phi(0)$、$\phi'(0)$ 和 $\phi(\alpha_0)$ 拟合二次多项式，取其极小值作为新试探步长，减少函数评估次数。

### 变形 4：回溯线搜索的步长初始化

牛顿 / 拟牛顿方向配合 $\bar{\alpha} = 1$（单位步长），因为靠近最优解时牛顿步精确无误；梯度下降方向可用 Barzilai-Borwein（BB）步长 $\bar{\alpha} = \|\mathbf{s}_{k-1}\|^2/\vert \mathbf{s}_{k-1}^\top\mathbf{y}_{k-1}\vert$ 作为初始化，利用历史曲率估计。

### 变形 5：非单调线搜索

标准 Armijo 要求 $\phi(\alpha) \leq \phi(0) + c_1\alpha\phi'(0)$，即相比**当前点**充分下降。非单调（Grippo-Lampariello-Lucidi）条件改为相比**过去 $m$ 步最大函数值**下降：$\phi(\alpha) \leq \max_{0\leq j\leq m-1} \phi(\alpha_{k-j}) + c_1\alpha\phi'(0)$，允许短暂函数值上升，有助于跳出局部极小。适合高度非凸问题，但理论保证较弱。

---

## 思考路标（条件反射）

1. 看到"$\phi'(0)$"→ 立刻计算方向导数 $\nabla f(\mathbf{x}_k)^\top\mathbf{d}_k$；若 $\geq 0$ 则 $\mathbf{d}_k$ 不是下降方向，线搜索无意义
2. 看到"Armijo 条件"→ 想到直线 $\phi(0) + c_1\alpha\phi'(0)$（斜率 $c_1\phi'(0) < 0$）；函数值必须落在这条线**之下**
3. 看到"步长太小"→ 想到 Wolfe 曲率条件 $\phi'(\alpha) \geq c_2\phi'(0)$；Armijo 单独无法防止过小步长
4. 看到"L-BFGS"→ 必须使用 Wolfe 条件（或强 Wolfe）；保证 $\mathbf{y}_k^\top\mathbf{s}_k > 0$，维持 Hessian 近似正定
5. 看到"回溯线搜索有限步终止"→ 想到关键条件：$f$ 连续可微 + $\mathbf{d}_k$ 是下降方向；充分小步长必然满足 Armijo
6. 看到"黄金分割法"→ 想到单峰假设 + 每步只需一次新函数评估（复用另一点）；收敛比 $\tau \approx 0.618$（线性收敛）
7. 看到"二分法"→ 需要导数信息 $\phi'(\alpha) = 0$；每步区间减半，收敛比 $1/2$，优于黄金分割
8. 看到"割线法"→ 超线性收敛（阶约 $1.618$），用两点导数差近似二阶导；比牛顿法更实用（无需二阶导）
9. 看到"精确线搜索效率低"→ 想到锯齿现象：梯度下降精确线搜索后，相邻两步方向正交，整体收敛慢；非精确反而更快
10. 看到"Zoutendijk 条件"→ 想到 Wolfe 步长 $\Rightarrow$ $\sum\cos^2\theta_k\|\nabla f_k\|^2 < \infty$，推出 $\liminf\|\nabla f_k\| = 0$
11. 看到"深度学习学习率调度"→ 类比 Armijo 回溯精神：损失停止充分下降则降学习率（ReduceLROnPlateau）；余弦退火是全局预设，非实时反馈
12. 看到"强 Wolfe vs Wolfe"→ 区别在曲率条件：Wolfe 允许 $\phi'(\alpha)$ 为负（仍在下坡段）；强 Wolfe 要求 $\vert\phi'(\alpha)\vert$ 小（靠近极小值），更适合拟牛顿法
13. 看到"回溯参数 $\beta$"→ $\beta = 0.5$ 最常用（每次减半）；$\beta = 0.8$ 更精细（步长减少慢，函数评估多但步长更接近最优）；$\beta$ 越小收敛越快但精度越差
14. 看到"方向角 $\theta_k$"（$\mathbf{d}_k$ 与 $-\nabla f_k$ 的夹角）→ Zoutendijk 条件要求 $\cos\theta_k \geq \delta > 0$（不能与梯度方向接近垂直）；梯度下降的 $\cos\theta_k = 1$，CG 的 $\cos\theta_k$ 接近 1 但可能偶尔变小

---

## 易错点清单

1. **回溯终止性证明被误解**：有人认为回溯线搜索"可能无限循环"。实际上，由于 $\phi'(0) < 0$ 且 $f$ 连续可微（梯度 Lipschitz），对足够小的 $\alpha$，$\phi(\alpha) \approx \phi(0) + \alpha\phi'(0)$，Armijo 条件的右侧 $\phi(0) + c_1\alpha\phi'(0)$ 比 $\phi(\alpha)$ 下降更慢（因 $c_1 < 1$），故**有限步内一定终止**。

2. **Wolfe 条件的存在性**：不是所有 $f$ 和 $\mathbf{d}_k$ 组合都存在满足 Wolfe 条件的 $\alpha$，但当 $f$ 下有界且 $\mathbf{d}_k$ 是下降方向时，满足 Wolfe 条件的步长**一定存在**（Lemma 3.1, Nocedal & Wright）。

3. **强 Wolfe 与 Wolfe 的区别**：Wolfe 曲率条件 $\phi'(\alpha) \geq c_2\phi'(0)$ 允许 $\phi'(\alpha)$ 仍为负（只要不太负），步长可能停在下坡段；强 Wolfe $|\phi'(\alpha)| \leq c_2|\phi'(0)|$ 要求步长更接近局部极小值，适配拟牛顿法（BFGS 需要保证 $\mathbf{y}_k^\top\mathbf{s}_k > 0$）。

4. **$c_2$ 的取值对 L-BFGS 的影响**：L-BFGS 通常取 $c_2 = 0.9$，而纯梯度法可取 $c_2 = 0.1$（更严格），这是因为 L-BFGS 的方向已包含曲率信息，无需步长过于精确；梯度法方向质量差，需要更精确步长来保证下降量。

5. **黄金分割法的单调性假设**：黄金分割法要求目标函数 $\phi(\alpha)$ 在给定区间 $[a, b]$ 内**单峰**（先减后增）。若函数有多个局部极小值，黄金分割可能找到局部极小而非全局最小——这在深度学习的损失曲面（高度非凸）中几乎必然发生，因此精确线搜索在深度学习中基本不用。

## 方法链接：从线搜索到更大框架

线搜索是优化方法的"基础构件"，在不同框架中扮演不同角色：

**在梯度下降中**：线搜索的主要作用是确定正确步长量级，防止发散；当学习率固定时，学习率调度策略（余弦退火等）是线搜索的统计替代品。

**在牛顿/拟牛顿法中**：线搜索的关键作用是维护 BFGS 矩阵的正定性（通过 Wolfe 条件保证 $\mathbf{y}_k^\top\mathbf{s}_k > 0$），并将局部二次收敛区域"扩展"到远离最优点的情形。

**在非线性最小二乘中**：高斯-牛顿法配合线搜索（或信赖域）是求解残差的标准框架；Levenberg-Marquardt 方法可视为自适应阻尼的信赖域方法，但其实现通常基于线搜索的理念。

**在强化学习中**：线搜索用于验证策略更新的单调改进（PPO 的 clip 机制是隐式的 Armijo 近似——若 $r(\theta) > 1+\epsilon$ 则停止，类比"步长上限"）。

## 典型应用例题

### 例 1：Armijo 回溯线搜索手算

> **题目**：对函数 $f(x_1, x_2) = x_1^2 + 4x_2^2$，当前点 $\mathbf{x}_k = (3, 1)^\top$，搜索方向 $\mathbf{d}_k = (-1, -1)^\top$。取 $c_1 = 0.1$，$\beta = 0.5$，初始步长 $\alpha_0 = 4$。运行回溯线搜索，直到满足 Armijo 条件。

【思路】依次计算：$\phi(0) = f(\mathbf{x}_k)$、$\phi'(0)$，然后逐步缩减 $\alpha$ 直到满足条件。

【解】

$\phi(0) = f(3, 1) = 9 + 4 = 13$。

$\phi'(0) = \nabla f(\mathbf{x}_k)^\top \mathbf{d}_k = (2\times3, 8\times1)\cdot(-1,-1)^\top = -6 - 8 = -14 < 0$。✓（下降方向）

Armijo 右侧：$\phi(0) + c_1\alpha\phi'(0) = 13 + 0.1\times\alpha\times(-14) = 13 - 1.4\alpha$。

**试 $\alpha_0 = 4$**：$\mathbf{x}_{\text{new}} = (3,1)^\top + 4(-1,-1)^\top = (-1,-3)^\top$，$\phi(4) = 1 + 36 = 37$。
Armijo 右侧：$13 - 1.4\times4 = 7.4$。$37 > 7.4$，**不满足**，缩减。

**试 $\alpha_1 = 2$**：$\mathbf{x}_{\text{new}} = (1,-1)^\top$，$\phi(2) = 1 + 4 = 5$。
Armijo 右侧：$13 - 2.8 = 10.2$。$5 \leq 10.2$，**满足**！✓

【答案】$\boxed{\alpha = 2}$，回溯 1 次。新点 $(1,-1)^\top$，函数值从 $13$ 降至 $5$。

### 例 2：Wolfe 条件验证（二维二次函数）

> **题目**：$f(\mathbf{x}) = x_1^2 + 5x_2^2$，$\mathbf{x}_k = (2, 1)^\top$，$\mathbf{d}_k = (-2, -1)^\top$（负梯度方向）。对步长 $\alpha = 0.15$，验证是否满足 Wolfe 条件（$c_1 = 10^{-4}$，$c_2 = 0.9$）。

【解】

$\phi(0) = f(2,1) = 4 + 5 = 9$。

$\phi'(0) = (2\times2, 10\times1)\cdot(-2,-1)^\top = -8 - 10 = -18$。

**Armijo 条件**：$\phi(0.15) = f(2+0.15\times(-2), 1+0.15\times(-1)) = f(1.7, 0.85)$。

$\phi(0.15) = 2.89 + 3.6125 = 6.5025$。

右侧：$9 + 10^{-4}\times0.15\times(-18) = 9 - 0.00027 = 8.9997$。

$6.5025 \leq 8.9997$，**Armijo 满足**。✓

**曲率条件**：$\phi'(\alpha) = \nabla f(\mathbf{x}_k + 0.15\mathbf{d}_k)^\top \mathbf{d}_k$。

新点梯度：$\nabla f(1.7, 0.85) = (2\times1.7, 10\times0.85)^\top = (3.4, 8.5)^\top$。

$\phi'(0.15) = (3.4, 8.5)\cdot(-2,-1)^\top = -6.8 - 8.5 = -15.3$。

曲率条件：$-15.3 \geq 0.9\times(-18) = -16.2$。$-15.3 \geq -16.2$，**满足**。✓

【答案】$\alpha = 0.15$ 满足完整 Wolfe 条件。

### 例 2b：强 Wolfe 条件手算

> **题目**：接例 2，对同一设置，验证 $\alpha = 0.15$ 是否满足**强** Wolfe 条件（将曲率条件改为 $|\phi'(\alpha)| \leq c_2|\phi'(0)|$）。

【解】

由例 2，$\phi'(0.15) = -15.3$，$c_2|\phi'(0)| = 0.9\times18 = 16.2$。

强 Wolfe：$|\phi'(0.15)| = 15.3 \leq 16.2$，**满足**。✓

注意：强 Wolfe 比 Wolfe 更严格（对曲率条件取绝对值），这里 $\phi'(0.15) = -15.3$（仍为负数，步长还在下坡段），Wolfe 条件允许，强 Wolfe 也允许。若 $\phi'(\alpha)$ 变为正数（步长已超过极小值），则不满足 Wolfe 条件，但可能满足强 Wolfe 曲率条件（若 $|\phi'(\alpha)|$ 足够小）。

【关键区别】：Wolfe 曲率条件 $\phi'(\alpha) \geq c_2\phi'(0)$ 允许步长到达极大值附近（$\phi'(\alpha)$ 正而大）；强 Wolfe 排除了这种情况，要求斜率绝对值小，确保步长在极小值附近。对 L-BFGS 而言，必须用强 Wolfe 条件。

### 例 3：精确线搜索 vs 非精确线搜索的迭代步数对比

> **题目**：对 $f(\mathbf{x}) = \frac{\kappa}{2}x_1^2 + \frac{1}{2}x_2^2$（$\kappa = 100$），从 $(1,1)^\top$ 出发用梯度下降。分析精确线搜索和固定学习率 $\alpha = 1/L$（$L = \kappa$）各需多少步达到 $\|\mathbf{x}_k - \mathbf{x}^*\| \leq 10^{-3}$。

【解】

精确最小值 $\mathbf{x}^* = \mathbf{0}$。

**固定学习率**：收敛因子 $\rho = 1 - 1/\kappa = 1 - 0.01 = 0.99$。

初始误差 $\|\mathbf{x}_0\| = \sqrt{2}$，需要 $0.99^k \sqrt{2} \leq 10^{-3}$，即 $k \geq \frac{\ln(\sqrt{2}/10^{-3})}{\ln(1/0.99)} \approx \frac{7.56}{0.01005} \approx 752$ 步。

**精确线搜索**（对二次函数，梯度下降+精确线搜索的收敛因子）：对正定二次函数，精确线搜索每步的最优步长为 $\alpha^* = \|\mathbf{g}_k\|^2/(\mathbf{g}_k^\top\mathbf{A}\mathbf{g}_k)$，收敛因子 $\leq \left(\frac{\kappa-1}{\kappa+1}\right)^2 = \left(\frac{99}{101}\right)^2 \approx 0.960$。约需 $\frac{\ln(\sqrt{2}\times10^3)}{\ln(1/0.960)} \approx \frac{7.56}{0.041} \approx 184$ 步。

【结论】精确线搜索比固定学习率快约 $4\times$，但每步需要额外计算精确步长（$\sim 1$ 次梯度）。对于二次函数，**共轭梯度法**仅需 $n=2$ 步精确收敛，效率远超两者。

### 例 4：黄金分割法精确线搜索步骤

> **题目**：用黄金分割法在 $[0, 2]$ 上最小化 $\phi(\alpha) = \alpha^2 - 1.6\alpha + 1$（精确极小值在 $\alpha^* = 0.8$）。运行前 3 步，计算每步的区间缩减。

【解】

黄金分割比 $\tau = (\sqrt{5}-1)/2 \approx 0.618$，保留比例 $1-\tau \approx 0.382$。

初始：$[a_0, b_0] = [0, 2]$，宽度 $= 2$。

**内点**：$c_0 = a_0 + (1-\tau)(b_0-a_0) = 0 + 0.382\times2 = 0.764$，$d_0 = a_0 + \tau(b_0-a_0) = 1.236$。

$\phi(0.764) = 0.584 - 1.222 + 1 = 0.362$，$\phi(1.236) = 1.528 - 1.978 + 1 = 0.550$。

$\phi(c_0) < \phi(d_0)$，极小值在 $[a_0, d_0]$，**缩小右界**：$b_1 = d_0 = 1.236$，$d_1 = c_0 = 0.764$，$c_1 = a_0 + (1-\tau)(b_1-a_0) = 0.382\times1.236 \approx 0.472$。

**第 1 步后**：$[0, 1.236]$，宽度 $\approx 1.236$（减少 $38.2\%$）。

$\phi(0.472) = 0.223 - 0.755 + 1 = 0.468$，$\phi(c_1) = 0.468 > \phi(d_1) = 0.362$，极小值在 $[c_1, b_1]$：$a_2 = c_1 = 0.472$。

**第 2 步后**：$[0.472, 1.236]$，宽度 $\approx 0.764$（又减少 $38.2\%$）。

（继续迭代，每步区间宽度乘以 $0.618$，约 $\ln(10^6)/\ln(1/0.618) \approx 30$ 步可达 $10^{-6}$ 精度）

【关键数字】：每步区间缩减 $38.2\%$（保留 $61.8\%$），无需梯度信息，适用于任意一维连续函数。相比二分法（每步缩减 $50\%$）效率稍低，但二分法需要导数；黄金分割只需函数值。
...
| 名称 | 数学表达 | 参数范围 | 作用 |
|------|---------|---------|------|
| 线搜索函数 | $\phi(\alpha) = f(\mathbf{x}_k + \alpha\mathbf{d}_k)$ | $\alpha > 0$ | 将多维问题化为一维 |
| 方向导数 | $\phi'(0) = \nabla f(\mathbf{x}_k)^\top \mathbf{d}_k < 0$ | — | 下降方向验证 |
| Armijo 条件 | $\phi(\alpha) \leq \phi(0) + c_1\alpha\phi'(0)$ | $c_1 \in (0,1)$ | 防步长过大 |
| Wolfe 曲率条件 | $\phi'(\alpha) \geq c_2\phi'(0)$ | $c_2 \in (c_1,1)$ | 防步长过小 |
| 强 Wolfe 曲率 | $\vert\phi'(\alpha)\vert \leq c_2\vert\phi'(0)\vert$ | $c_2 \in (c_1,1)$ | 远离极大值点 |
| 回溯缩减 | $\alpha \leftarrow \beta\alpha$，直到满足 Armijo | $\beta \in (0,1)$ | 有限步内终止 |
| 黄金分割比 | $\tau = (\sqrt{5}-1)/2 \approx 0.618$ | — | 精确线搜索区间缩减 |

### 解题套路

**套路 1：验证 Armijo 条件**
1. 计算 $\phi(0) = f(\mathbf{x}_k)$，$\phi'(0) = \nabla f(\mathbf{x}_k)^\top \mathbf{d}_k$（确认 $< 0$）
2. 计算 $\phi(\alpha) = f(\mathbf{x}_k + \alpha\mathbf{d}_k)$
3. 比较：$\phi(\alpha)$ 是否 $\leq \phi(0) + c_1\alpha\phi'(0)$？

**套路 2：回溯线搜索步数估计**
- 每步乘 $\beta$，经 $j$ 步后 $\alpha = \beta^j$
- 当 $\beta^j$ 足够小时，Taylor 展开保证 $\phi(\beta^j) \approx \phi(0) + \beta^j\phi'(0) < \phi(0) + c_1\beta^j\phi'(0)$（因 $c_1 < 1$，$\phi'(0) < 0$）
- 理论终止步数上界：$j \leq \log_\beta\!\left(\frac{(1-c_1)|\phi'(0)|}{L\|\mathbf{d}_k\|^2/2}\right)$（$L$ 为 Lipschitz 常数）

**套路 3：Wolfe 条件判断**
1. 先检查 Armijo（充分下降）
2. 再检查曲率：$\nabla f(\mathbf{x}_k + \alpha\mathbf{d}_k)^\top \mathbf{d}_k \geq c_2\phi'(0)$
3. 曲率条件几何含义：新点处方向导数"变得不太负"，说明沿方向 $\mathbf{d}_k$ 已足够下降

### 常见错误

1. **混淆 $c_1, c_2$ 的大小关系**：必须 $c_1 < c_2$，否则可接受区间为空集。典型取值 $c_1 = 10^{-4}$，$c_2 = 0.9$（梯度法）或 $c_2 = 0.1$（拟牛顿法需更强曲率条件）。
2. **忘记方向必须为下降方向**：若 $\phi'(0) \geq 0$，Armijo 条件右侧不减，步长 $\alpha = 0$ 即满足条件，算法不前进。**先验证 $\nabla f^\top \mathbf{d} < 0$**。
3. **回溯初始步长选错**：拟牛顿法应始终取 $\alpha_0 = 1$（Newton 步归一化），梯度法可根据上步信息选择初始 $\alpha_0$（Barzilai-Borwein 准则）。
4. **精确线搜索代价被低估**：黄金分割每步缩减比 $\approx 0.618$，达到 $10^{-6}$ 精度约需 $6\log_{0.618}(10^{-6}) \approx 80$ 步。非精确线搜索通常 $2$–$10$ 步即可终止，代价差一个量级。
...
```
给定：搜索方向 d_k（满足 ∇f^T d < 0），当前点 x_k，函数 f
目标：找到合适步长 α_k
...
└──────────────────────────────────────────┘
         ...
         └── 非精确线搜索（工程首选）
                │
                ├── 只需防步长太大 → Armijo 回溯
                │      α := βα（β∈(0,1)），直到满足 φ(α) ≤ φ(0)+c₁αφ'(0)
                │      优点：简单、有限步、无需梯度新值
                │      缺点：不防步长太小
                │
                ├── 防过大+防过小 → Wolfe 条件线搜索
                ...
                └── 强 Wolfe（拟牛顿法必备）
                       |φ'(α)| ≤ c₂|φ'(0)|
                       保证 y_k^T s_k > 0（BFGS 正定性）
                       适配：L-BFGS、BFGS
```

## 自测题（闭卷）

**T1**（基础）：设 $f: \mathbb{R} \to \mathbb{R}$ 是可微函数，$x_k = 1$，$d_k = -1$（梯度下降），$f(1) = 3$，$f'(1) = 2$。对步长 $\alpha = 0.5$ 和 $c_1 = 0.1$，写出 Armijo 条件的具体不等式，并说明 $\alpha = 0.5$ 使该条件成立的充分条件是什么。

**T2**（推导）：设 $\phi(\alpha) = (\alpha - 2)^2 + 1$（一维二次函数），$\phi(0) = 5$，$\phi'(0) = -4$。
(a) 求精确最小值点 $\alpha^* = \arg\min\phi(\alpha)$。
(b) 验证 $\alpha = 1.5$ 是否满足 Wolfe 条件（$c_1 = 10^{-4}$，$c_2 = 0.9$）。
(c) 验证 $\alpha = 0.01$ 是否满足 Wolfe 条件。

**T3**（计算）：回溯线搜索中取 $\beta = 0.5$，$c_1 = 0.1$，初始 $\alpha_0 = 4$。设 $\phi(0) = 10$，$\phi'(0) = -8$，且
$$\phi(\alpha) = \begin{cases} 10 - 7\alpha & 0 \leq \alpha \leq 1 \\ 3 + (\alpha-1)^2 & \alpha > 1 \end{cases}$$
逐步计算回溯过程，直到找到满足 Armijo 条件的步长，并报告所需回溯次数。

**T4**（概念）：解释为什么 Wolfe 条件对拟牛顿法（如 L-BFGS）特别重要。如果线搜索只满足 Armijo 条件，L-BFGS 的 $y_k = \nabla f_{k+1} - \nabla f_k$ 会出现什么问题？（提示：思考 $\mathbf{s}_k^\top\mathbf{y}_k$ 的符号。）

**T5**（AI 关联）：深度学习中的"学习率预热 + 余弦退火"策略与线搜索方法有何异同？从 Wolfe 条件的角度分析：为什么大批量训练中固定步长策略仍能有效，而小批量场景更需要自适应步长？
...
**经典教材**：
- Nocedal & Wright, *Numerical Optimization* (2nd ed., 2006)，第 3 章（线搜索算法）是最权威的参考，本章定理均来源于此。
- Boyd & Vandenberghe, *Convex Optimization* (2004)，第 9 章给出了强凸函数下回溯线搜索的精确收敛界。
- Bertsekas, *Nonlinear Programming* (3rd ed., 2016)，第 1 章详细分析了各类步长选取准则的收敛速率。

**关键算法实现**：
- SciPy `scipy.optimize.line_search_wolfe1/wolfe2`：基于 Wright & Nocedal (1994) 的标准实现，包含完整的 zoom 阶段。
- PyTorch `torch.optim.LBFGS`：内置强 Wolfe 线搜索，是 PyTorch 中最接近"标准"线搜索的优化器。
- 最小化库 `optax`（JAX）的 `scale_by_backtracking_line_search`：Armijo 回溯的 JIT 可编译版本。

**历史背景**：Armijo 条件由 Larry Armijo 于 1966 年提出；Wolfe 条件由 Philip Wolfe 于 1969/1971 年提出；强 Wolfe 条件在 Nocedal (1980) 引入 L-BFGS 时被明确强调为保证正定性的关键。回溯线搜索的概念则可追溯到更早的数值方法文献。

**与深度学习文献的联系**：Sutskever et al. (2013) 的动量 SGD 分析、Kingma & Ba (2014) 的 Adam 方法，均可从线搜索视角重新理解——Adam 的 $\hat{v}_t$ 分量隐式估计了二阶导数（类似精确线搜索步长 $1/L$），而 $\hat{m}_t$ 提供了梯度方向的一阶矩估计。Defazio et al. (2023) 的 "Learning-Rate-Free Learning" 进一步探讨了完全无需步长参数的自适应方法，是线搜索思想与深度学习的最新融合尝试。

## 答案提示

**T1**：Armijo 条件：$f(1 + 0.5\times(-1)) \leq f(1) + 0.1\times 0.5\times f'(1)\times(-1)$，即 $f(0.5) \leq 3 + 0.1\times 0.5\times(-2) = 3 - 0.1 = 2.9$。所以 $\alpha = 0.5$ 满足条件的充分条件是 $f(0.5) \leq 2.9$。

**T2**：(a) $\phi'(\alpha) = 2(\alpha-2) = 0 \Rightarrow \alpha^* = 2$，$\phi(2) = 1$。(b) $\alpha = 1.5$：Armijo：$\phi(1.5) = 0.25+1 = 1.25 \leq 5 + 10^{-4}\times 1.5\times(-4) = 4.9994$，✓；曲率：$\phi'(1.5) = 2(1.5-2) = -1 \geq 0.9\times(-4) = -3.6$，✓。Wolfe 条件满足。(c) $\alpha = 0.01$：Armijo：$\phi(0.01) = (0.01-2)^2+1 \approx 3.960+1 = 4.960 \leq 5 - 0.0004 = 4.9996$，✓；曲率：$\phi'(0.01) = 2(0.01-2) = -3.98 < -3.6$，曲率条件不满足（步长太小）。

**T3**：$\alpha_0 = 4$：$\phi(4) = 3 + 9 = 12$，Armijo 右侧 $= 10 + 0.1\times4\times(-8) = 6.8$，$12 > 6.8$，不满足，缩减。$\alpha_1 = 2$：$\phi(2) = 3 + 1 = 4$，右侧 $= 10 - 1.6 = 8.4$，$4 \leq 8.4$，满足！共缩减 **1 次**，步长 $\alpha = 2$。

**T4**：L-BFGS 更新公式需要 $\mathbf{s}_k^\top\mathbf{y}_k > 0$（保正定）。Wolfe 曲率条件 $\nabla f_{k+1}^\top\mathbf{d}_k \geq c_2\nabla f_k^\top\mathbf{d}_k$ 可推导出 $\mathbf{y}_k^\top\mathbf{s}_k = (\nabla f_{k+1} - \nabla f_k)^\top(\alpha_k\mathbf{d}_k) \geq (c_2-1)\alpha_k|\nabla f_k^\top\mathbf{d}_k| > 0$（当 $c_2 < 1$ 时）。若仅有 Armijo，$\mathbf{s}_k^\top\mathbf{y}_k$ 可能为负，导致 BFGS 矩阵失去正定性，算法崩溃。

**T5**：相同点：都是"找合适步长避免函数值上升"的策略。不同点：线搜索每步都精确决策（$O(\text{几次梯度计算})$）；余弦退火是全局预设策略，不依赖实时损失反馈。大批量场景中梯度估计方差小、每步下降方向一致，固定步长能保持稳定进展；小批量梯度噪声大，固定步长容易在噪声中"误判下降"，自适应步长（AdaGrad/Adam 等）通过累积二阶信息隐式实现了 Wolfe 类的步长控制。
...
| 段 | 来源 | 价值 |
|---|---|---|
| 一例速记 + 引入 + 思维路径还原 | 融合版前置 | 建立直觉，先有感性再看推导 |
| 学习目标 + 13.1–13.5 严格正文 | 原版 | 完整推导，Armijo/Wolfe/回溯/精确线搜索 |
| 本章小结 | 原版 | 公式速查表，闭卷前复盘 |
| 深度学习应用 + PyTorch 代码 | 原版 | 工程实战，学习率调度与 Armijo 对照 |
| 练习题 13.1–13.5 | 原版 | 系统巩固，从基础到 Rosenbrock |
| **抽象成方法** | 融合补充 | 公式速查表 + 5 步选步长流程，套路固化 |
| **方法变形** | 融合补充 | 4 类参数 / 场景变形，举一反三 |
| **思考路标** | 融合补充 | 12 条条件反射，考场 / 工程速查 |
| 易错点清单 | 融合补充 | 5 条高频陷阱，避雷指南 |
| 方法链接 | 融合补充 | 线搜索在梯度 / 牛顿 / RL 框架中的角色 |
| 典型应用例题 3 例 | 融合补充 | Armijo 手算 / Wolfe 验证 / 精确线搜索 |
| 自测题 5 题 + 答案 | 融合补充 | 闭卷验收，T1–T5 覆盖全章核心 |
| **融合版说明** | 融合补充 | 本表格，帮助读者定位每段来源与目的 |

**推荐学习节奏**：速记（5 min）→ 引入题（10 min）→ 正文（60 min）→ 小结 + 抽象成方法（10 min）→ 思考路标（快速扫描）→ 典型例题（20 min）→ 自测（闭卷 15 min）。
