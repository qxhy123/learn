...
请先停下来想一想：L1 和 L2 正则化的本质区别——次梯度条件决定了一个能产生精确零、另一个只能"接近零"。（"次梯度"指凸函数在不可导点处"所有支撑切线斜率"的集合，严格定义见下文 §12.1.0。）
...
### 12.1.0 预备：次梯度与次微分（本章核心工具）

本章处理 $\ell_1$ 范数这类**不可微**的凸函数，靠的工具是把导数推广到不可微点的**次梯度**。它在前面的引入和后面的软阈值推导中反复出现，这里先把它讲清。

**为什么需要它**：在光滑点，凸函数 $f$ 落在它唯一切线之上：$f(\mathbf{y})\ge f(\mathbf{x})+\nabla f(\mathbf{x})^\top(\mathbf{y}-\mathbf{x})$。但在 $|x|$ 的尖点 $x=0$ 处导数不存在——却存在**一整族**都"托住"函数的支撑直线。把这些支撑直线的斜率收集起来，就得到次梯度。

**定义（次梯度 / 次微分）**：设 $f:\mathbb{R}^n\to\mathbb{R}$ 为凸函数。若向量 $\mathbf{g}$ 满足

$$f(\mathbf{y}) \ge f(\mathbf{x}) + \mathbf{g}^\top(\mathbf{y}-\mathbf{x}),\qquad \forall\,\mathbf{y},$$

则称 $\mathbf{g}$ 是 $f$ 在 $\mathbf{x}$ 处的一个**次梯度**（subgradient）。所有次梯度构成的集合称为**次微分**（subdifferential），记作 $\partial f(\mathbf{x})$。其几何意义：$\partial f(\mathbf{x})$ 是"所有能从下方支撑 $f$ 的切线（超平面）斜率"的集合。

**两条关键性质**：

- **可微点退化为普通梯度**：若 $f$ 在 $\mathbf{x}$ 可微，则 $\partial f(\mathbf{x})=\{\nabla f(\mathbf{x})\}$（单点集）。次梯度是梯度的推广。
- **最优性条件**：$\mathbf{x}^*$ 是凸函数 $F$ 的全局极小点 $\iff \mathbf{0}\in\partial F(\mathbf{x}^*)$。（这是 $\nabla F(\mathbf{x}^*)=\mathbf{0}$ 在不可微情形的推广——"零是一个次梯度"。）

**典范例子 $f(x)=|x|$**：

$$\partial |x| = \begin{cases} \{+1\}, & x>0,\\[2pt] [-1,\,1], & x=0,\\[2pt] \{-1\}, & x<0. \end{cases}$$

在 $x>0$、$x<0$ 处就是普通导数 $\pm1$；唯独尖点 $x=0$ 处，从 $-1$ 到 $+1$ 的每条斜率都能从下方托住 $|x|$，故次微分是整个**区间** $[-1,1]$。

![绝对值函数 |x| 在尖点 x=0 处的次梯度：一束斜率介于 -1 与 1 之间的支撑直线](../figures/svg/opt-p4-12-3.svg)

正是这个"尖点处次微分是一段区间"的事实，使 $\ell_1$ 正则化能产生**精确的零**（见本章引入与 §12.1.4 软阈值推导）：只要数据项的梯度落在区间 $[-\lambda,\lambda]$ 内，$\mathbf{0}\in\partial F$ 就在 $x=0$ 处成立。
