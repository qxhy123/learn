...
> **难度**：★★★☆☆
> **前置知识**：多元微积分、线性代数（特征值）
> **建议先读**：[第二部分导论《到底什么是"最优化问题"？》](./03b-what-is-an-optimization-problem.md) —— 先理解"最优化问题 = 在可行域上最小化目标""无约束只是 $\mathcal{F}=\mathbb{R}^n$ 的特例"，再看本章的最优性条件会更有全局感。
> **本文件**：融合"原版严格推导 + 速记 / 套路 / 自测"。保留原版完整正文 + 前置一例速记 / 思维路径 + 后置方法总结与自测。

> **一例速记**：
> **一阶必要条件**：极小点处梯度为零 $\nabla f(\mathbf{x}^*)=\mathbf{0}$（驻点）。梯度非零 $\Rightarrow$ 还能下降 $\Rightarrow$ 不是极小。
> **二阶判别**：在驻点处看 Hessian $\mathbf{H}^*$：正定 $\succ 0$ → 严格局部极小；负定 $\prec 0$ → 极大；不定 → 鞍点。
> **凸函数特权**：驻点直接是全局极小（一阶充要）；严格凸函数极小唯一。
> **鞍点普遍性**：深度网络参数空间中，鞍点比局部极小多得多（概率 $\approx 1-2^{-n}$）；SGD 噪声有助于逃离。
> **AI 联系**：损失曲面分析 = 高维非凸函数的临界点分析；理解鞍点是设计好优化器的前提。

---

## 引入：一道"驻点却不是极值"的题

> **题目**：设 $f(x_1, x_2) = x_1^2 - x_2^2$。验证原点是驻点，并判断它是极小、极大还是鞍点。

请先停下来想一想：梯度为零只是极值的**必要条件**，驻点还可能是鞍点。沿 $x_1$ 轴函数上凸（碗形），沿 $x_2$ 轴函数下凸（倒碗），这正是鞍点的几何直觉。

---

## 思维路径还原（解题者的内心独白）

> "拿到这道题，第一步验证驻点：$\nabla f = (2x_1, -2x_2)^\top$，在原点等于 $(0,0)^\top$，确实是驻点。
>
> **第二步：看 Hessian**。$\mathbf{H} = \begin{pmatrix}2 & 0 \\ 0 & -2\end{pmatrix}$，特征值 $\{+2, -2\}$——正负都有，**不定矩阵**。
>
> **第三步：套判别表**。不定 $\Rightarrow$ 鞍点。物理直觉：坐到马鞍上，前后方向是上坡，左右方向是下坡，你坐的位置正是鞍点。
>
> **延伸思考**：如果改成 $f = x_1^2 + x_2^2$，Hessian $= 2\mathbf{I} \succ 0$，原点是严格局部极小。如果改成 $f = x_1^3$，Hessian $= 0$（退化），一阶二阶都不够，需要高阶分析。这三个例子把驻点的三种命运都覆盖了。"
...
若问题带约束 $\mathbf{x} \in \mathcal{S}$，则需在**可行域**（feasible set）$\mathcal{S}$ 内讨论极小性——即只在满足约束的点中比较 $f$ 值。本章主要讨论**无约束**情形，相当于 $\mathcal{S}=\mathbb{R}^n$（整个空间都可行）；一般约束优化将在第三部分展开。

> 可行域、最优化问题标准形、全局/局部最优解与"求解"的统一定义，见 [第二部分导论《到底什么是"最优化问题"？》](./03b-what-is-an-optimization-problem.md)。本章定义的极小点，就是该标准形在 $\mathcal{S}=\mathbb{R}^n$ 时的最优解。
...
### 图 4-1：一阶 vs 二阶最优性条件三种比较

![极小点、鞍点、极大点的函数曲线与 Hessian 特征对比](../figures/svg/opt-p2-04-1.svg)
...
1. **求梯度令为零**：解方程组 $\nabla f(\mathbf{x}) = \mathbf{0}$，得驻点集合
2. **在每个驻点算 Hessian**：写出 $\mathbf{H}^* = \nabla^2 f(\mathbf{x}^*)$
3. **看特征值符号**（或用顺序主子式 Sylvester 准则）：
...
| 负定 $\prec 0$ | 全负 | 严格局部极大 |
| 不定 | 正负皆有 | 鞍点 |
| 半正定 $\succeq 0$ | 含零特征值 | 无法判断，需高阶 |

### 二维 Sylvester 快速判别

对 $\mathbf{H} = \begin{pmatrix}a & b \\ b & d\end{pmatrix}$：
- $\Delta_1 = a > 0$ 且 $\Delta_2 = ad - b^2 > 0$ $\Rightarrow$ 正定（极小）
- $\Delta_1 = a < 0$ 且 $\Delta_2 > 0$ $\Rightarrow$ 负定（极大）
- $\Delta_2 < 0$ $\Rightarrow$ 不定（鞍点）
...
若已知 $f$ 是凸函数，则驻点直接是全局极小，不需要逐个算 Hessian。例：$f(\mathbf{x}) = \frac{1}{2}\mathbf{x}^\top \mathbf{A}\mathbf{x} - \mathbf{b}^\top\mathbf{x}$，$\mathbf{A}\succ 0$ 时驻点方程 $\mathbf{Ax}=\mathbf{b}$ 唯一解即全局极小。

### 变形 2：退化驻点（Hessian 半正定）

当 $\mathbf{H}^*$ 含零特征值时，Taylor 展开的二次项不能确定增减，需用三阶或四阶项判断。典型例：$f(x)=x^4$ 在 $x=0$，$f''=0$ 但确是极小（四阶项 $>0$）；$f(x)=x^3$ 在 $x=0$，四阶分析也不够，是拐点（非极值）。

### 变形 3：多极值点问题

一个函数可以有多个局部极小点。非凸函数 $f(x) = x^4 - 3x^2 + 2x$ 有两个局部极小，但只有一个全局极小。判断全局极小需要比较所有局部极小的函数值。

### 变形 4：约束边界处的极值

若问题定义在有界区域 $\mathcal{S}$ 上，极值也可能在边界取到，不一定是驻点。此时需要同时检验内部驻点和边界情况（边界优化将在 Part 3 约束优化中展开）。
...
1. 看到"极小点候选" → 先解 $\nabla f = \mathbf{0}$ 找驻点，驻点才是候选
2. 看到驻点 → 立刻算 Hessian，判定特征值符号
3. 看到 Hessian 正定 → 严格局部极小（充分条件）
4. 看到 Hessian 不定 → 鞍点，有正有负特征值
5. 看到"凸函数" → 驻点直接是全局极小，无需验证充分条件
6. 看到"严格凸" → 极小唯一；$\nabla f = \mathbf{0} \iff$ 全局极小（充要）
7. 看到"高维深度学习" → 鞍点远比局部极小多，SGD 噪声帮助逃离
8. 看到 Hessian 半正定（含零特征值）→ 无法二阶判断，需高阶分析或数值验证
9. 看到"一阶条件" → 必要但不充分；$f(x)=x^3$ 在 $0$ 点是反例
10. 看到"负曲率方向" → 对应 Hessian 最小特征值 $\lambda_{\min} < 0$ 的特征向量，沿此方向可逃离鞍点

---

## 易错点

1. **驻点不是极值点**：$\nabla f = \mathbf{0}$ 只是极值的必要条件，不是充分条件。$f(x) = x^3$ 在 $x=0$ 处梯度为零，但既不是极小也不是极大。
2. **"Hessian 正定"必须在驻点处检验**：如果 $\nabla f \neq \mathbf{0}$，Hessian 正定也无法断定极值——必须先确认是驻点再验证 Hessian。
3. **半正定 $\neq$ 极小**：$\mathbf{H} \succeq \mathbf{0}$ 是极小的必要条件，但含零特征值时不能断定是极小（可能是鞍点）。例：$f(x,y) = x^3 + y^2$ 在原点，$\mathbf{H} = \text{diag}(0, 2)$ 半正定，但原点是鞍点。
4. **局部极小 $\neq$ 全局极小**（非凸）：多个局部极小点中只有函数值最小的才是全局极小，必须比较所有局部极小的函数值才能下结论。
5. **鞍点与拐点的混淆**：一维拐点（$f''=0$，$f''' \neq 0$）对应高维的退化鞍点；而多元函数的鞍点（$\mathbf{H}$ 不定）是确定的——沿负曲率方向函数值严格下降。
...
> **题目**：$f(x_1, x_2) = x_1^3 + x_2^3 - 3x_1 x_2$。求所有驻点并判别类型。

【思路】解 $\nabla f = \mathbf{0}$，得驻点；各驻点分别算 Hessian，用 Sylvester 判别。

【解】
$\nabla f = \begin{pmatrix}3x_1^2 - 3x_2 \\ 3x_2^2 - 3x_1\end{pmatrix} = \mathbf{0}$，得 $x_1^2 = x_2$，$x_2^2 = x_1$。

代入：$x_1^4 = x_1 \Rightarrow x_1(x_1^3 - 1) = 0$，故 $x_1 = 0$ 或 $x_1 = 1$。

驻点：$(0, 0)$ 和 $(1, 1)$。

$\mathbf{H}(x_1, x_2) = \begin{pmatrix}6x_1 & -3 \\ -3 & 6x_2\end{pmatrix}$。

**在 $(0,0)$**：$\mathbf{H} = \begin{pmatrix}0 & -3 \\ -3 & 0\end{pmatrix}$，$\Delta_2 = 0 - 9 = -9 < 0$，不定 $\Rightarrow$ **鞍点**。

**在 $(1,1)$**：$\mathbf{H} = \begin{pmatrix}6 & -3 \\ -3 & 6\end{pmatrix}$，$\Delta_1 = 6 > 0$，$\Delta_2 = 36 - 9 = 27 > 0$，正定 $\Rightarrow$ **严格局部极小**，$f(1,1) = -1$。

【答案】$\boxed{(0,0) \text{ 鞍点};\; (1,1) \text{ 严格局部极小}, f = -1}$。

---

### 例 2：凸函数全局极小

> **题目**：$f(\mathbf{x}) = \frac{1}{2}\mathbf{x}^\top \mathbf{A}\mathbf{x} - \mathbf{b}^\top\mathbf{x}$，$\mathbf{A} = \begin{pmatrix}4 & 1 \\ 1 & 3\end{pmatrix}$，$\mathbf{b} = \begin{pmatrix}2 \\ 1\end{pmatrix}$。求全局极小点。

【思路】验证 $\mathbf{A} \succ 0$（凸函数），一阶条件充要，直接解 $\mathbf{Ax} = \mathbf{b}$。

【解】$\Delta_1 = 4 > 0$，$\Delta_2 = 12 - 1 = 11 > 0$，故 $\mathbf{A} \succ 0$，$f$ 严格凸。

$\nabla f = \mathbf{Ax} - \mathbf{b} = \mathbf{0} \Rightarrow \mathbf{x}^* = \mathbf{A}^{-1}\mathbf{b} = \frac{1}{11}\begin{pmatrix}3 & -1 \\ -1 & 4\end{pmatrix}\begin{pmatrix}2 \\ 1\end{pmatrix} = \frac{1}{11}\begin{pmatrix}5 \\ 2\end{pmatrix}$。

【答案】$\boxed{\mathbf{x}^* = (5/11,\; 2/11)^\top}$，全局唯一极小点。

---

### 例 3：深度学习损失曲面

> **题目**：设某神经网络参数空间维数为 $n = 10^6$。从随机矩阵理论角度估计：(1) 随机驻点是局部极小的概率；(2) 实践中如何处理大量鞍点。

【解】
(1) 正定概率 $\approx 2^{-n} = 2^{-10^6} \approx 0$（指数级趋零）。随机驻点几乎确定是鞍点。

(2) 三种实践策略：**使用 SGD**（随机噪声自然逃离鞍点）；**加动量**（积累方向性，跨越平坦区）；**自适应方法**（Adam 利用二阶矩信息调整步长）。另：深度网络局部极小值通常接近全局极小（Goodfellow 等 2015），不必强求找全局极小。

【答案】极小概率约 $2^{-10^6}$；鞍点主导；实践用 SGD / 动量 / Adam 自然处理。

---

## 自测题

**自测 1**　$f(x, y) = x^2 + 2xy + 3y^2 - 4x - 6y + 1$。求驻点并判别类型。

> 提示：$\nabla f = (2x+2y-4, 2x+6y-6) = \mathbf{0}$ 解得 $(3, -1/2)$……等等，重新算：$2x+2y=4, 2x+6y=6$，解得 $y=1/2, x=3/2$。$\mathbf{H} = \begin{pmatrix}2 & 2 \\ 2 & 6\end{pmatrix}$，$\Delta_2 = 12-4=8>0$，正定，严格局部极小（亦全局极小，因 $f$ 凸）。

**自测 2**　$f(x, y) = x^2 - 3y^2 + 2xy$。求驻点类型。

> 提示：$\nabla f = (2x+2y, -6y+2x) = \mathbf{0}$，解得 $(0,0)$。$\mathbf{H} = \begin{pmatrix}2 & 2 \\ 2 & -6\end{pmatrix}$，$\Delta_2 = -12-4 = -16 < 0$，不定 $\Rightarrow$ **鞍点**。

**自测 3**　证明：若 $f$ 是凸函数，$\mathbf{x}^*$ 是局部极小点，则 $\mathbf{x}^*$ 也是全局极小点。

> 提示：假设存在 $\mathbf{y}$ 使 $f(\mathbf{y}) < f(\mathbf{x}^*)$，取 $\mathbf{z}_\theta = (1-\theta)\mathbf{x}^* + \theta\mathbf{y}$，由凸性 $f(\mathbf{z}_\theta) < f(\mathbf{x}^*)$，当 $\theta\to 0^+$ 时 $\mathbf{z}_\theta \to \mathbf{x}^*$，矛盾于局部极小性。

**自测 4**　一维函数 $f(x) = x^4 - 4x^2$。找出所有驻点并分类（含二阶判别失效时的处理）。

> 提示：$f' = 4x^3 - 8x = 4x(x^2-2) = 0$，驻点 $x \in \{0, \sqrt{2}, -\sqrt{2}\}$。$f''(x) = 12x^2 - 8$。$f''(0) = -8 < 0$：极大。$f''(\pm\sqrt{2}) = 24-8=16 > 0$：严格局部极小。全局极小在 $\pm\sqrt{2}$ 处，$f(\pm\sqrt{2}) = 4-8=-4$。

**自测 5**　为什么 $f(x) = x^3$ 在 $x=0$ 处不是极值点，而 $f(x) = x^4$ 在 $x=0$ 处是极小点？两者的一阶、二阶条件分别如何？

> 提示：$f(x)=x^3$：$f'(0)=0$，$f''(0)=0$（退化），三阶项 $f''' = 6 \neq 0$ 说明是拐点（鞍点）。$f(x)=x^4$：$f'(0)=0$，$f''(0)=0$（退化），$f'''(0)=0$，四阶项 $f^{(4)}(0)=24>0$ 说明是严格局部极小（函数值在 $0$ 附近确实最小）。

---

**回头看一眼"一例速记"**：

> $\nabla f = \mathbf{0}$ 是极小必要条件（一阶）；Hessian 正定是充分条件（二阶）。
> 凸函数：一阶充要；严格凸：全局唯一。
> 鞍点：Hessian 不定；深度学习中鞍点主导，SGD 噪声自然逃离。
