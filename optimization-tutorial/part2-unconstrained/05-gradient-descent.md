...
> **难度**：★★★☆☆
> **前置知识**：多元微积分（梯度）、第4章最优性条件
> **本文件**：融合"原版严格推导 + 速记 / 套路 / 自测"。保留原版完整正文 + 前置一例速记 / 思维路径 + 后置方法总结与自测。

> **一例速记**：
> **迭代公式**：$\mathbf{x}_{k+1} = \mathbf{x}_k - \alpha_k \nabla f(\mathbf{x}_k)$；负梯度 = 最速下降方向（Cauchy-Schwarz 最大化下降量）。
> **步长选择**：太小则收敛慢；太大则震荡 / 发散；$\alpha \leq 1/L$ 保证单调下降（$L$ = 梯度 Lipschitz 常数）。
> **收敛率**：$L$-光滑凸函数 $O(1/k)$；$\mu$-强凸函数线性收敛 $O(\rho^k)$，$\rho = 1 - \mu/L$。
> **条件数**：$\kappa = L/\mu$；$\kappa$ 大 $\Rightarrow$ 等高线椭圆扁 $\Rightarrow$ 锯齿震荡 $\Rightarrow$ 慢收敛。
> **AI 联系**：SGD = 小批量随机梯度下降，是深度学习默认优化器；批量大小影响梯度方差与收敛噪声。

---

## 引入：一道"步长选哪个"的判断题

> **题目**：对 $f(x_1, x_2) = \frac{1}{2}(x_1^2 + 25x_2^2)$，梯度 Lipschitz 常数 $L = 25$。用固定步长 $\alpha = 0.1$ 从 $(1,1)^\top$ 出发做梯度下降，问：$x_2$ 分量会震荡还是收敛？

请先停下来想一想：$L = 25$，步长稳定条件是 $\alpha \leq 1/L = 0.04$，而 $0.1 > 0.04$，所以步长超过安全上界——$x_2$ 方向**必然震荡**（乘子为 $1 - 25\times 0.1 = -1.5$，绝对值 $>1$）。
...
>
> **关键参数**：Hessian $= \text{diag}(1, 25)$，$L = 25$（最大特征值），$\mu = 1$（最小特征值）。
>
> **稳定步长上界**：$\alpha \leq 1/L = 1/25 = 0.04$。题目给的 $\alpha = 0.1 > 0.04$，超界了。
>
> **追踪 $x_2$ 分量**：更新规则 $x_2^{(k+1)} = x_2^{(k)} - 0.1 \times 25 x_2^{(k)} = (1 - 2.5) x_2^{(k)} = -1.5 \times x_2^{(k)}$。
>
> 从 $x_2^{(0)} = 1$ 出发：$1, -1.5, 2.25, -3.375, \ldots$——**发散**（震荡且增大）。
>
> **延伸思考**：如果取 $\alpha = 0.04 = 1/L$，$x_2$ 乘子为 $1-25\times 0.04 = 0$，**一步收缩到零**——这正是精确步长的特别性质（对二次函数 $x_2$ 方向一步到位）。如果取 $\alpha = 0.02 < 1/L$，收敛但要多步。三种步长的命运差异是梯度下降最核心的直觉。"
...
> **先说清"收敛速率"是什么**：设 $\mathbf{e}_k = \mathbf{x}_k - \mathbf{x}^*$（或用最优性间隙 $f(\mathbf{x}_k)-f^*$）为第 $k$ 步的误差。收敛速率刻画 $\|\mathbf{e}_k\|\to 0$ 的**快慢**，由相邻两步误差之比来分类：
>
> | 收敛类型 | 定义（$k\to\infty$） | 直觉 |
> |---|---|---|
> | **次线性（sublinear）** | 误差按 $O(1/k)$、$O(1/\sqrt k)$ 等多项式速度趋零（比值 $\|\mathbf{e}_{k+1}\|/\|\mathbf{e}_k\|\to 1$） | 越走越慢，每步只缩一点点 |
> | **线性（linear，又称几何）** | $\|\mathbf{e}_{k+1}\|\le r\|\mathbf{e}_k\|$，固定比例 $0<r<1$，即 $\|\mathbf{e}_k\|=O(r^k)$ | 每步误差至少缩小固定倍率 |
> | **二次（quadratic）** | $\|\mathbf{e}_{k+1}\|\le C\|\mathbf{e}_k\|^2$ | 有效位数每步翻倍，极快（见第6章牛顿法） |
>
> 关键对照：**次线性 $O(1/k)$ 比线性 $O(r^k)$ 慢得多**——要达到精度 $\epsilon$，次线性需 $O(1/\epsilon)$ 步，线性只需 $O(\ln(1/\epsilon))$ 步。本节出现的"$O(1/k)$ 次线性""线性收敛"皆按此定义；完整的收敛阶比较（含超线性、二次）见 [第6章 §6.2.2](./06-newton-methods.md)。
...
### 图 5-1：梯度下降轨迹（椭圆等高线 + 之字形路径）

![椭圆等高线上梯度下降迭代点沿之字形路径收敛](../figures/svg/opt-p2-05-1.svg)

### 图 5-2：步长选择对比（太小 / 合适 / 太大）

![相同函数三条梯度下降轨迹对比：步长过小缓慢，步长合适平滑，步长过大震荡](../figures/svg/opt-p2-05-2.svg)
...
1. **初始化**：选起始点 $\mathbf{x}_0$，设步长策略（固定 $\alpha$ 或回溯线搜索），设终止准则 $\|\nabla f\| \leq \varepsilon$
2. **计算梯度**：$\mathbf{g}_k = \nabla f(\mathbf{x}_k)$（自动微分或解析推导）
3. **更新参数**：$\mathbf{x}_{k+1} = \mathbf{x}_k - \alpha_k \mathbf{g}_k$
4. **判断终止**：若 $\|\mathbf{g}_k\| \leq \varepsilon$ 则停止，否则回步骤 2
...
| 概念 | 公式 | 直觉 |
|---|---|---|
| 迭代公式 | $\mathbf{x}_{k+1} = \mathbf{x}_k - \alpha\nabla f(\mathbf{x}_k)$ | 沿负梯度下山 |
| 最速下降方向 | $-\nabla f(\mathbf{x}_k) / \|\nabla f(\mathbf{x}_k)\|$ | Cauchy-Schwarz 最优方向 |
| 步长安全上界 | $\alpha \leq 1/L$ | 不超过曲率倒数 |
| 一步下降量 | $\geq \frac{1}{2L}\|\nabla f\|^2$ | 梯度越大下降越快 |
| 凸函数收敛 | $f(\mathbf{x}_k) - f^* \leq \frac{L\|\mathbf{x}_0-\mathbf{x}^*\|^2}{2k}$ | $O(1/k)$ 次线性 |
| 强凸收敛 | $\|\mathbf{x}_k - \mathbf{x}^*\|^2 \leq (1-\mu/L)^k \cdot \|\mathbf{x}_0-\mathbf{x}^*\|^2$ | 线性几何收缩 |
| 条件数 | $\kappa = L/\mu$ | 越大越慢越震荡 |
...
非凸函数无法保证找到全局极小，只能收敛到驻点（$\nabla f \to \mathbf{0}$）。收敛率为 $O(1/\sqrt{k})$（梯度范数平方的平均）。深度学习中这已经足够——局部极小质量通常不差（见第4章）。

### 变形 2：投影梯度下降（约束情形预览）

有约束 $\mathbf{x} \in \mathcal{C}$ 时，更新后加投影：$\mathbf{x}_{k+1} = \text{proj}_{\mathcal{C}}(\mathbf{x}_k - \alpha\nabla f(\mathbf{x}_k))$。约束优化细节见 Part 3。

### 变形 3：坐标下降

每步只沿一个坐标方向下降：$x_j \leftarrow x_j - \alpha \frac{\partial f}{\partial x_j}$，其他坐标不动。适用于变量可分离的问题，是稀疏学习（LASSO 等）的标准方法。

### 变形 4：子梯度法（不可微函数）

对不可微凸函数（如 $\ell_1$ 范数），用**子梯度** $\mathbf{g} \in \partial f(\mathbf{x})$ 代替梯度。步长需要 $\sum \alpha_k = \infty$，$\sum \alpha_k^2 < \infty$（如 $\alpha_k = 1/\sqrt{k}$）。收敛率降至 $O(1/\sqrt{k})$。
...
1. 看到"梯度下降" → 立刻写迭代公式 $\mathbf{x}_{k+1} = \mathbf{x}_k - \alpha\nabla f$，想负梯度是最速下降方向
2. 看到"步长 $\alpha$" → 问三件事：是否 $\leq 1/L$？太大震荡还是太小？是固定还是自适应？
3. 看到"$L$-光滑" → 凸函数 $O(1/k)$ 收敛率，步长上界 $1/L$
4. 看到"强凸 $\mu$" → 线性收敛 $O(\rho^k)$，$\rho = 1-\mu/L$；达到 $\epsilon$ 精度需 $O(\kappa\ln(1/\epsilon))$ 步
5. 看到"条件数 $\kappa = L/\mu$ 大" → 等高线椭圆扁，锯齿震荡，考虑预条件或 Newton 法
6. 看到"深度学习训练" → SGD（小批量），步长调度（warmup+decay），动量（0.9）
7. 看到"Armijo 条件" → 回溯线搜索，$f(\mathbf{x}-\alpha\mathbf{g}) \leq f(\mathbf{x}) - c\alpha\|\mathbf{g}\|^2$，$c=10^{-4}$
8. 看到"非凸函数 GD" → 只能找驻点，$O(1/\sqrt{k})$ 梯度平方平均；不保证全局极小
9. 看到"梯度为零停止" → 已收敛到驻点，不一定是极小（可能是鞍点）——深度学习中这通常没问题
10. 看到 SGD 波动大 → 减小学习率 / 增大批量 / 加梯度裁剪（`clip_grad_norm_`）

---

## 易错点

1. **步长超过 $1/L$ 时函数值不保证下降**：$\alpha > 1/L$ 时，二次上界不再有效，梯度下降可能震荡甚至发散。$L$ 未知时务必用回溯线搜索或保守初始步长。
2. **条件数影响收敛，不是绝对值**：$L = 100$ 但 $\mu = 50$ 时，$\kappa = 2$ 仍然快收敛；$L = 2$ 且 $\mu = 0.001$ 时 $\kappa = 2000$，极慢。决定收敛速度的是**比值**而非绝对大小。
3. **收敛率 $O(1/k)$ 是最优性间隙，不是参数误差**：$f(\mathbf{x}_k) - f^* = O(1/k)$ 不意味着 $\|\mathbf{x}_k - \mathbf{x}^*\| = O(1/k)$；强凸函数才有参数收敛率。
4. **`optimizer.zero_grad()` 必须在每步调用**：PyTorch 默认梯度累积，忘记清零会让梯度变成多步的叠加，相当于学习率放大，导致不稳定。
5. **学习率调度与"基础学习率"混淆**：`scheduler` 修改的是当前学习率，不是初始学习率。先设好合适的 `lr`，再加调度；不要试图用调度"救活"不合适的基础学习率。
...
> **题目**：$f(x) = \frac{1}{2}x^2 + 2x + 3$，从 $x_0 = 5$ 出发，$\alpha = 0.5$。计算前 3 步，并求最优值。

【思路】梯度 $f'(x) = x + 2$；迭代 $x_{k+1} = x_k - 0.5(x_k + 2)$；最优点 $x^* = -2$（令 $f' = 0$）。

【解】
$x_1 = 5 - 0.5 \times 7 = 1.5$；$x_2 = 1.5 - 0.5 \times 3.5 = -0.25$；$x_3 = -0.25 - 0.5 \times 1.75 = -1.125$。

$f^* = f(-2) = 1$；误差 $|x_3 - x^*| = |-1.125 - (-2)| = 0.875$。

【答案】$\boxed{x_1 = 1.5,\ x_2 = -0.25,\ x_3 = -1.125}$；向 $x^* = -2$ 线性收敛。

---

### 例 2：条件数与震荡

> **题目**：$f(\mathbf{x}) = \frac{1}{2}(x_1^2 + 25x_2^2)$，$L = 25$，$\mu = 1$，$\kappa = 25$。取步长 $\alpha = 1/L = 0.04$，从 $(1,1)^\top$ 出发，计算一步后的点和函数值，并估计达到精度 $\epsilon = 0.01$ 所需步数。

【解】$\nabla f = (x_1, 25x_2)^\top$；$\mathbf{x}_1 = (1,1)^\top - 0.04(1,25)^\top = (0.96, 0)^\top$。

$f(\mathbf{x}_1) = \frac{1}{2}(0.96^2 + 0) = 0.461$，从初始 $f(\mathbf{x}_0) = 13$ 大幅下降。

所需步数：$O(\kappa \ln(1/\epsilon)) = 25 \times \ln(100) \approx 25 \times 4.6 \approx 115$ 步。

【答案】$\boxed{\mathbf{x}_1 = (0.96, 0)^\top}$；约需 115 步达到 $\epsilon = 0.01$ 精度。

---

### 例 3：Armijo 线搜索逻辑

> **题目**：在某点 $\mathbf{x}_k$，$f(\mathbf{x}_k) = 10$，$\|\nabla f(\mathbf{x}_k)\|^2 = 4$，$c = 10^{-4}$，初始步长 $\alpha_0 = 1$，$\beta = 0.5$。验证第一次试探（$\alpha = 1$）是否满足 Armijo 条件（假设 $f(\mathbf{x}_k - \alpha\nabla f) = 9.8$），若不满足则继续。

【解】
Armijo 条件：$f(\mathbf{x}_k - \alpha\nabla f) \leq f(\mathbf{x}_k) - c\alpha\|\nabla f\|^2 = 10 - 10^{-4} \times 1 \times 4 = 9.9996$。

$9.8 \leq 9.9996$：满足！接受步长 $\alpha = 1$。

若假设 $f(\mathbf{x}_k - \nabla f) = 10.5 > 9.9996$：不满足，令 $\alpha = 0.5$，再测试……

【答案】第一次试探通常满足 Armijo 条件（足够下降量已达到）；回溯的意义在于防止步长过大导致函数值升高。

---

## 自测题

**自测 1**　$f(x) = x^2 - 4x + 3$，$\alpha = 0.3$，$x_0 = 4$。计算前 2 步，并判断步长是否在安全范围（$L = f''(x) = 2$，$1/L = 0.5$）内。

> 提示：$f'(x) = 2x - 4$；$x_1 = 4 - 0.3 \times 4 = 2.8$；$x_2 = 2.8 - 0.3 \times 1.6 = 2.32$。$\alpha = 0.3 \leq 0.5 = 1/L$，在安全范围内，函数值单调下降（$x^* = 2$）。

**自测 2**　$f(\mathbf{x}) = 2x_1^2 + 8x_2^2$。求 $L$，$\mu$，$\kappa$，并说明为何 GD 在 $x_2$ 方向更快收缩到零。

> 提示：Hessian $= \text{diag}(4, 16)$，$L = 16$，$\mu = 4$，$\kappa = 4$。步长 $\alpha = 1/16$；$x_2$ 更新乘子 $= 1 - 16 \times \frac{1}{16} = 0$（一步到零！），$x_1$ 乘子 $= 1 - 4 \times \frac{1}{16} = 0.75$，需多步收敛。

**自测 3**　设 SGD 用无偏随机梯度 $\mathbf{g}_k = \nabla f(\mathbf{x}_k) + \boldsymbol{\epsilon}_k$，$\mathbb{E}[\boldsymbol{\epsilon}_k] = \mathbf{0}$。说明为何期望意义下 SGD 是向正确方向前进的，并指出噪声 $\boldsymbol{\epsilon}_k$ 对鞍点逃离的作用。

> 提示：$\mathbb{E}[\mathbf{g}_k] = \nabla f(\mathbf{x}_k) + \mathbf{0} = \nabla f(\mathbf{x}_k)$，期望等于真实梯度，所以期望意义下等价于确定性 GD。但在鞍点（真实梯度为零）处，噪声 $\boldsymbol{\epsilon}_k \neq \mathbf{0}$ 仍会推动参数移动；若恰好沿负曲率方向，函数值下降，自然逃离鞍点。

**自测 4**　证明：取步长 $\alpha = 1/L$，$L$-光滑函数的梯度下降满足每步函数值下降不等式 $f(\mathbf{x}_{k+1}) \leq f(\mathbf{x}_k) - \frac{1}{2L}\|\nabla f(\mathbf{x}_k)\|^2$。

> 提示：代入 $\mathbf{y} = \mathbf{x}_k - \frac{1}{L}\nabla f(\mathbf{x}_k)$ 到二次上界 $f(\mathbf{y}) \leq f(\mathbf{x}_k) + \langle\nabla f(\mathbf{x}_k), \mathbf{y}-\mathbf{x}_k\rangle + \frac{L}{2}\|\mathbf{y}-\mathbf{x}_k\|^2$。代入 $\mathbf{y}-\mathbf{x}_k = -\frac{1}{L}\nabla f$，化简即得 $\leq f(\mathbf{x}_k) - \frac{1}{L}\|\nabla f\|^2 + \frac{1}{2L}\|\nabla f\|^2 = f(\mathbf{x}_k) - \frac{1}{2L}\|\nabla f\|^2$。

**自测 5**　比较三种步长策略在以下场景中的优劣：(A) 已知精确 $L$；(B) $L$ 未知，函数光滑；(C) 深度学习大模型训练。

> 提示：(A) 固定步长 $1/L$ 最优——无额外计算，理论保证最好。(B) Armijo 回溯——自适应找合适步长，每步多几次函数值计算，实际中最常用于传统优化。(C) 固定学习率 + 调度（cosine decay / warmup）——因前向传播代价高，回溯线搜索不可行；AdaGrad / Adam 进一步自适应每个参数的步长。

---

**回头看一眼"一例速记"**：

> $\mathbf{x}_{k+1} = \mathbf{x}_k - \alpha\nabla f$；负梯度最速下降；$\alpha \leq 1/L$ 稳定。
> 凸函数 $O(1/k)$；强凸线性收敛；条件数 $\kappa$ 大则震荡。
> SGD = 小批量随机梯度，无偏但有噪声，是深度学习的基础。
...
| 学习目标 + 5.1–5.5 严格正文 | 原版 | 完整推导与证明 |
| 几何示意（图） | Part 2 配图 | 可视化轨迹与步长影响 |
