...
理解这些变换之前，先回到最根本的一条关系：对数函数与同底指数函数**互为反函数**，因此它们的图像关于直线 $y=x$ **对称**。下图以 $a=2$ 为例，把 $y=2^x$、$y=\log_2 x$ 和虚线 $y=x$ 画在一起，并标出几对互换坐标的对称点：$(0,1)\leftrightarrow(1,0)$、$(1,2)\leftrightarrow(2,1)$。每一对对称点都被 $y=x$ 垂直平分，这正是"横纵坐标交换"的几何含义。

![y=2^x 与 y=log_2 x 关于直线 y=x 对称：标出对称点 (0,1)对(1,0)、(1,2)对(2,1)，两函数互为反函数](../figures/svg/log-p2-05-1.svg)
