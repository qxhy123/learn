...
- **开篇"第一性原理拆解 + 学习大纲"段落（不少于 800 字）**。这一段不是"工程师为什么要学"的动员宣传，而是把本章的所有概念用第一性原理拆开重组：
  - **拆**：剥离所有可记忆的术语和工具名，逼出本章要解决的"不可化简的问题"是什么。例如 0a 章的不可化简问题是"CPU 是有限的物理器件，单条指令完成不了的事必须用并行/缓存/预测来逼近极限速度"，而不是"工程师要懂流水线"。
  - **推**：从这个不可化简的问题出发，推导出每个机制为什么必然存在。例如：单指令带宽不够 → 指令级并行 → 流水线 → 冒险 → 乱序 → 分支预测 → SIMD；内存远慢于 CPU → 局部性原理 → 多级 Cache → 一致性问题 → MESI → 伪共享。
  - **绘**：用一张 mermaid 思维导图（mindmap 或 flowchart）显式画出"问题 → 机制 → 子机制 → AI 场景影响"的因果链路。这张图是本章的"学习地图"。
  - **导**：以"读完本章后你应该能回答的 5-7 个第一性问题"列表收尾，作为读者后续章节学习的检验清单。
- 结尾 Worked example（不少于 600 字，含数字、命令、推理链）
- 4-8 个 mermaid 图（流程 / 状态机 / 序列 / 架构，**含开篇思维导图**）
