...
| HNSW（M=32） | ~35 GB（原始向量 + 图） | 0 | ~97% | 1-5 ms |
...
| **SSD 需求** | 0 | 0 | ~350 GB（精确向量 + 图） |
| **Recall@10** | ~97% | ~91% | ~95% |
| **P50 延迟** | ~2 ms | ~1.5 ms | ~8 ms |
| **P99 延迟** | ~8 ms | ~6 ms | ~22 ms |
| **QPS（32 核）** | ~800 | ~2500 | ~400 |
| **增量写入成本** | 中等（图修改） | 低（直接写入最近桶） | 高（需重建受影响区域） |
| **内存安全边界** | 256 GB 中占 15%，安全 | 256 GB 中占 1.4%，安全 | 256 GB 中占 1.3%，安全 |

### 决策推荐

本场景（P99 < 50ms，Recall@10 ≥ 92%，256 GB RAM，无 GPU）三种方案均满足性能要求。推荐 HNSW，原因：

1. 召回率最高（97% vs 91% / 95%），满足 92% 要求且有余量
2. P99 延迟 8ms，远低于 50ms 要求，有足够余量应对过滤召回损失
3. 256 GB RAM 足够，38 GB 内存占用（15%）安全
4. 增量更新中等成本，符合每天 10 万条新向量的需求
5. 无需 SSD 依赖（更高可靠性，避免 IO 抖动）

如果 RAM 降低到 64 GB 或向量库增长到 10 亿，切换 DiskANN（召回高于 IVFPQ 且 P99 仍 < 50ms）。
