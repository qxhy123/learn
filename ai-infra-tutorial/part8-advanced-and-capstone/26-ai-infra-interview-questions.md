...
- 良好：能围绕关键变量给出有顺序的判断路径
- 优秀：能用验证证据支撑取舍，并说明对GPU feeding、训练吞吐和数据成本的影响
...
- 是否能通过profiler、synthetic data、关闭 checkpoint、单卡/多卡对比验证判断
...
- 良好：能围绕关键变量给出有顺序的判断路径
- 优秀：能用验证证据支撑取舍，并说明对GPU feeding 和训练吞吐的影响
...
- 是否能通过profiler timeline、NCCL_DEBUG、bucket size A/B 验证判断
...
- 是否能通过TP size A/B、NCCL timeline、同机拓扑、tokens/s验证判断
...
- 是否能把 分布式训练平台 与100B 训练可完成性和可恢复性联系起来
