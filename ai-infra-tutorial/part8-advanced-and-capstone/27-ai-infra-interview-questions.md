# 第27章：AI Infra 模拟面试与评分校准手册

> 本章定位为第26章题库的互补手册：不再扩充题量，而是帮助候选人把答案组织成可面试表达，帮助面试官把题库组合成模拟面试、记录证据并校准评分。
...
把 Kubernetes、GPU、NCCL、vLLM、registry 这些名字拿掉，AI Infra 面试真正想确认的是：候选人是否能把模型系统看成一条受资源约束、会排队、会失败、需要治理的工程链路。第26章回答“可以问什么题”，本章回答“如何把这些题组织成一场有效面试，以及如何把回答转化为可复盘评分”。
...
本章当前仍按系统层次保留题块：27.1 建立基础认知，27.2 回到底层资源，27.3 和 27.4 覆盖训练与制品，27.5 和 27.6 覆盖推理与平台调度，27.7 覆盖治理，27.8 用综合 case 把前面内容串起来。阅读时应把这些题块当作模拟面试材料库，而不是第26章的重复题库。

## 2. 学习目标

完成本章后，你应该能够：

1. 把第26章题库组合成 45-90 分钟的模拟面试流程。
2. 用资源、链路、故障、治理四条线组织候选人答案，避免散点式背诵。
3. 区分训练、推理、平台、可靠性岗位的追问重点和评分权重。
4. 把一个回答拆成事实证据、推理链路、遗漏约束和后续追问。
5. 作为面试官，写出可复盘、可校准的评分理由。

## 3. 使用方式

- **候选人模拟**：从第26章选题，本章用于练习“先给结论、再分层、最后落到指标或案例”的表达节奏。
- **面试官组卷**：从岗位画像出发组合题目。推理岗多问 KV Cache、batching、SLO；训练岗多问并行、checkpoint、恢复；平台岗多问队列、配额、发布和治理。
- **评分记录**：每个回答记录候选人讲清的证据、遗漏的约束、追问后的修正能力，而不是只记“会/不会”。
...
## 27.1 AI Infra 基础认知与系统分层

### 27.1.1 AI Infra 面试到底在考什么
...
### 27.1.2 AI Infra 与传统后端基础设施的核心差异
...
### 27.1.9 描述你做过的项目时如何对齐 AI Infra 视角
...
### 27.1.10 AI Infra 工程师 vs 算法、SRE、DevOps 的边界
...
## 27.2 硬件、GPU、内存、网络与存储基础

### 27.2.1 选择 GPU 时应该看哪些指标
...
### 27.2.6 PCIe、NVLink 与 NVSwitch 的取舍
...
### 27.2.9 对象存储、并行文件系统与本地 NVMe 的选择
...
### 27.2.11 CPU 在 AI Infra 中为什么仍然重要
...
### 27.3.7 Pipeline Parallel 的气泡问题
...
### 27.3.11 Elastic Training 与恢复语义
...
## 27.4 数据、制品、Checkpoint 与 Registry
...
### 27.4.3 Checkpoint sharding 的基本语义
...
### 27.4.4 Checkpoint restore 的一致性
...
### 27.4.8 Adapter 与 Base Model 兼容
...
## 27.5 推理服务、KV Cache、Batching 与推理引擎

### 27.5.1 Prefill 与 Decode 的资源差异
...
### 27.5.4 Continuous Batching 的取舍
...
### 27.5.7 Speculative Decoding 的收益边界
...
### 27.5.13 Agent 推理与普通 Chat Serving
...
## 27.6 Kubernetes、调度、队列、配额与平台化

### 27.6.1 Kubernetes 中 GPU 资源的特殊性
...
## 27.7 可观测性、发布、安全、成本与多租户治理

### 27.7.1 AI Infra 可观测性的指标体系
...
## 27.8 综合系统设计与故障排查 Case

### 27.8.1 设计 70B 在线推理平台
...
- **Warm-up questions**：27.2.2、27.5.1、27.5.2，确认候选人是否能把 GPU 指标、prefill/decode 和 KV Cache 说成容量模型。
- **Deep-dive questions**：27.5.4、27.5.8、27.6.7、27.7.9，围绕 batching、engine 选型、autoscaling 和租户隔离连续追问。
...
- **Warm-up questions**：27.2.5、27.3.1、27.3.3，确认候选人是否理解拓扑、单机瓶颈和 DP 扩展边界。
- **Deep-dive questions**：27.3.5、27.3.8、27.3.10、27.4.4，压测并行策略、straggler 和恢复语义。
...
- **Warm-up questions**：27.1.3、27.1.8、27.4.6，确认候选人是否有平台分层、最小闭环和 registry 思维。
- **Deep-dive questions**：27.6.5、27.6.6、27.7.4、27.7.8，围绕队列、配额、发布和成本治理追问。
...
- **Warm-up questions**：27.1.7、27.2.12，确认候选人是否能从告警和错误日志做故障分类。
- **Deep-dive questions**：27.3.12、27.5.14、27.7.11，分别压训练 hang、推理压测和事故复盘。
...
- **Warm-up questions**：27.1.1、27.1.4、27.7.12，确认候选人是否能从系统判断、负载画像和预算沟通起步。
- **Deep-dive questions**：27.6.12、27.7.3、27.8.1、27.8.2，覆盖平台演进、SLO、推理系统和训练系统设计。
