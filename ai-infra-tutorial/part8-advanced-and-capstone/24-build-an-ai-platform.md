...
#### 24.2.2 Capstone 交付物与验收标准

本章的 capstone 不要求你把整个平台真正搭完，但要求你把“能落地的第一版平台”描述到可以评审、可以排期、可以验收的程度。交付物应围绕一条最小闭环展开：训练提交 -> 资源排队 -> 制品登记 -> 评测门禁 -> 灰度发布 -> 观测回滚 -> 复盘改进。

你可以用下面清单检查自己的方案是否已经从架构图走到可执行设计：

| 交付物 | 至少要写清楚什么 | 验收方式 |
|--------|------------------|----------|
| 平台 API 草案 | `TrainingJob`、`EvalRun`、`ModelVersion`、`Deployment` 的字段、状态、权限和幂等语义 | 任选一个模型版本，能从 API 请求追到训练、评测、发布和回滚记录 |
| 队列 / 配额策略 | 租户队列、GPU flavor、MIG / MPS、queue / quota、优先级、抢占条件、在线服务保留容量和超额申请规则 | 给出 3 个团队同时提交任务时的排队结果，解释为什么公平，并能回链到 `CapacityLedger` 的资源与配额口径 |
| 推理容量与 engine 契约 | TTFT / TPOT / KV cache、batching、warm pool、量化格式、engine shape contract 和峰值 headroom | 固定一组输入 / 输出分布压测，证明容量估算、KV 预算、量化取舍和 engine shape 约束都能复现 |
| 发布状态机 | `draft -> evaluated -> staging -> canary -> production -> rollback / archived` 的转移条件 | 每个状态都有进入条件、退出条件、失败处理和审计记录 |
| 发布单元与恢复目标 | `ReleaseUnit` 绑定模型、tokenizer、adapter、engine、镜像、router、index、cache、eval gate、rollback target 和 checkpoint restore 语义 | 灰度失败时能从 `ReleaseUnit` 找到可恢复对象，并演练一次 checkpoint restore 或服务 rollback |
| SLO 仪表盘 | 推理 p50 / p95 / p99、错误率、吞吐、GPU 利用率、队列等待、成本、模型质量采样 | 能用一张 dashboard 判断“慢、贵、错、排队长”分别是哪类问题 |
| incident runbook | 告警分级、止血动作、回滚步骤、沟通窗口、证据收集和复盘模板 | 给定 p99 突增或错误率升高，值班人能按 runbook 在 15 分钟内完成止血决策 |
| 容量表 | 当前 GPU / CPU / 存储 / 网络容量、GPU flavor / MIG / MPS、保留量、峰值需求、增长假设和扩容触发线 | 能回答未来 3 个月新增两个模型后是否会挤占在线 SLO，并更新 `CapacityLedger` |
| 证据对象 | `EvidenceBundle` 记录 symptom、scope、workload、version、evidence、hypothesis、action、retest、rollback；`CapacityLedger` 记录资源、队列、成本和 headroom | 任意一次优化、发布或事故都能找到证据包和容量账本变更，而不是只保留截图或口头结论 |
| 故障演练脚本 | 模型灰度失败、节点池不可用、registry 写入失败、队列积压、回滚失败的演练步骤 | 每个脚本包含注入方式、预期告警、人工操作、自动恢复和复盘问题 |
| 评分 rubric | 架构完整性、资源经营、发布治理、可观测性、故障处理、成本归因、边界取舍 | 评审人能按同一标准给出 1-5 分，并指出缺口来自哪里 |

评分 rubric 可以按 5 个等级使用：

| 分数 | 判断标准 |
|------|----------|
| 1 分 | 只有组件名和高层图，缺少 API、状态、指标和故障处理，无法执行 |
| 2 分 | 有基本链路，但资源、发布、观测或 runbook 中至少两项不可验收 |
| 3 分 | 最小闭环成立，关键对象、队列规则、发布状态机和 SLO 指标能解释清楚 |
| 4 分 | 能处理多租户冲突、灰度失败、容量增长和 incident 复盘，取舍有明确依据 |
| 5 分 | 方案可直接进入工程排期，边界清晰、风险可控、指标可验收，并能说明后续演进 |

写 capstone 时不要追求“大而全”。如果一个交付物还不能被测试、演练或评审，它就还只是想法；如果它能让另一个工程师按文档复现一次提交、发布、回滚和复盘，它才算进入平台设计的可交付状态。
