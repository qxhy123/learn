...
你已经会查"基本积分公式表"——$\int x^n\,dx$、$\int e^x\,dx$、$\int\sin x\,dx$……但 $xe^{x^2}$ 并不在表上：它是 $x$ 与 $e^{x^2}$ 的乘积，而积分**没有**像求导那样现成的"乘积法则"。

停下来想想：单独的 $\int e^{x^2}\,dx$ 其实没有初等原函数；可一旦前面多乘一个 $x$，情况就完全变了——这个 $x$ 恰好是"凑微分"的钥匙。这正是本章要教的第一件新工具：**换元（凑微分）法**；而当凑不出来时（如 $\int x\ln x\,dx$），再用第二件工具：**分部积分**。

下面的"思维路径还原"就完整演示这把钥匙怎么用。
...
### 11.1.5 不定积分运算规则体系（完整推导）

求导有"6 大法则"（和差、数乘、乘积、商、链式、反函数）；不定积分作为求导的**反运算**，也有一套对应的运算规则。但因为"反运算"的难度天然高于正运算，**积分规则的种类更多、技巧性更强**。本节系统给出全部主要规则的**不跳步**推导。
...
第 0 层：原函数定义 + N-L 互逆
   ↓
第 1 层（线性）：∫(αf+βg) = α∫f + β∫g
   ↓
第 2 层（换元）：第一类（凑微分）— 来自链式法则
                第二类（变量替换）— 来自反函数 + 第一类
   ↓
第 3 层（乘积反演）：分部积分 ∫u dv = uv - ∫v du — 来自乘积法则
   ↓
第 4 层（特殊类型）：有理函数部分分式
                   三角有理函数 Weierstrass 替换 t = tan(x/2)
                   含 √(ax²+bx+c) 的配方化标准型
```

每条规则都源于**一条对应的求导规则 + N-L 公式（或形式不变性）**。下面逐条不跳步推导。

---

#### 规则 1：线性性（数乘 + 和差的合并）

**定理**：设 $f, g$ 有原函数，$\alpha, \beta$ 为常数，则

$$\int [\alpha f(x) + \beta g(x)]\,dx = \alpha\int f(x)\,dx + \beta\int g(x)\,dx.$$

**完整推导**：

**第一步**（写出右端的"待证候选原函数"）：设 $F, G$ 分别是 $f, g$ 的原函数，则
$$\alpha\int f\,dx + \beta\int g\,dx = \alpha F(x) + \beta G(x) + C.$$

**第二步**（求导验证）：对 $\alpha F + \beta G$ 求导，用求导的线性性：
$$[\alpha F + \beta G]' = \alpha F' + \beta G' = \alpha f + \beta g.$$

**第三步**（结论）：故 $\alpha F + \beta G$ 是 $\alpha f + \beta g$ 的原函数，由原函数结构定理（任意两个原函数差一个常数），不定积分相等。$\square$

> **隐藏陷阱**：常数因子 $\alpha$ 可以提到积分号外，但**变量不行**！$\int x\cdot f(x)\,dx \neq x\int f(x)\,dx$。这一点常被初学者忽略——因为乘积法则不是线性的。

---

#### 规则 2：与求导 / 微分的互逆关系（严格表述）

**定理**：

(i) **先积分再求导**：$\displaystyle\frac{d}{dx}\left[\int f(x)\,dx\right] = f(x)$（连续性保证）。

(ii) **先求导再积分**：$\displaystyle\int F'(x)\,dx = F(x) + C$。

**推导**：

(i) 设 $F$ 是 $f$ 的一个原函数，则 $\int f\,dx = F(x) + C$。两边对 $x$ 求导：
$$\frac{d}{dx}[F(x) + C] = F'(x) + 0 = f(x).\quad\square$$

(ii) $F'$ 显然有原函数 $F$。由不定积分定义即得。$\square$

**结论**：积分与求导是**精确互逆**的——除了"差一个常数"这层模糊。这正是为什么不定积分必须带 $+C$：求导时常数项消失，反推回去就无从恢复。

---

#### 规则 3：第一类换元法（凑微分法）

**定理**：设 $f$ 有原函数 $F$（即 $F' = f$），$u = \varphi(x)$ 连续可导，则

$$\boxed{\int f(\varphi(x))\cdot \varphi'(x)\,dx = F(\varphi(x)) + C = \int f(u)\,du\bigg|_{u = \varphi(x)}.}$$

#### 不跳步推导

**第一步**（构造候选原函数）：考察 $F(\varphi(x))$，它是 $x$ 的函数。
...
**第四步**（反推不定积分）：上式表明 $F(\varphi(x))$ 是 $f(\varphi(x))\varphi'(x)$ 的一个原函数，故
$$\int f(\varphi(x))\varphi'(x)\,dx = F(\varphi(x)) + C.\quad\square$$

**理论本质**：**第一类换元法 = 链式法则的反演**。链式法则说"复合函数求导 = 外导乘内导"；反过来，看见"外函数 + 内函数导数相乘"的结构，就应想到反向回归到外函数的原函数。

#### 与微分形式不变性的关系

把推导的最后一步改写：
$$\int f(\varphi(x))\,\underbrace{\varphi'(x)\,dx}_{= du} = \int f(u)\,du.$$

——这正是**微分形式不变性**（8.7.3）的应用：$du = \varphi'(x)\,dx$ 让"凑微分"成立。所以"凑微分法"实际上是"用 $du$ 替换 $\varphi'(x)dx$"。

#### 常用凑微分识别清单（不跳步）

凑微分的关键是识别被积函数中**隐藏的 $d(\cdot)$ 结构**。下表展示"凑微分"的等价改写：

| 看到形式 | 等价改写 | 对应 $d(\cdot)$ |
|---|---|---|
| $\varphi'(x)\,dx$ | $d(\varphi(x))$ | 任意 |
| $x^{n-1}\,dx$ | $\dfrac{1}{n}d(x^n)$ | $d(x^n) = nx^{n-1}dx$ |
| $\dfrac{1}{x}\,dx$ | $d(\ln\|x\|)$ | $d\ln\|x\| = \dfrac{dx}{x}$ |
| $e^x\,dx$ | $d(e^x)$ | |
| $\cos x\,dx$ | $d(\sin x)$ | |
| $\sin x\,dx$ | $-d(\cos x)$ | |
| $\sec^2 x\,dx$ | $d(\tan x)$ | |
...
**定理**：设 $x = \varphi(t)$ **严格单调、连续可导且 $\varphi'(t)\neq 0$**，若 $f(\varphi(t))\varphi'(t)$ 有原函数 $G(t)$，则

$$\boxed{\int f(x)\,dx = G(\varphi^{-1}(x)) + C.}$$

#### 不跳步推导

**第一步**（已知条件）：$G'(t) = f(\varphi(t))\varphi'(t)$。

**第二步**（设反函数）：由 $\varphi$ 严格单调连续，反函数 $t = \varphi^{-1}(x)$ 存在、严格单调、连续可导。由反函数求导法则（8.3）：
$$\frac{dt}{dx} = \frac{1}{\varphi'(t)} = \frac{1}{\varphi'(\varphi^{-1}(x))}.$$

**第三步**（求 $G(\varphi^{-1}(x))$ 对 $x$ 的导数）：用链式法则：
$$\frac{d}{dx}G(\varphi^{-1}(x)) = G'(\varphi^{-1}(x))\cdot \frac{d\varphi^{-1}}{dx} = G'(t)\cdot \frac{1}{\varphi'(t)}.$$

**第四步**（代入 $G'(t) = f(\varphi(t))\varphi'(t) = f(x)\varphi'(t)$）：
$$\frac{d}{dx}G(\varphi^{-1}(x)) = f(x)\varphi'(t)\cdot \frac{1}{\varphi'(t)} = f(x).$$

**第五步**（反推）：故 $G(\varphi^{-1}(x))$ 是 $f(x)$ 的原函数，结论成立。$\square$

#### 关键条件分析

- **严格单调** → 反函数 $\varphi^{-1}$ 存在唯一 → 最后回代不会产生歧义；
- **$\varphi'(t)\neq 0$** → 反函数可导（反函数法则需要） → 第四步的约分合法；
- **连续可导** → 微分代换 $dx = \varphi'(t)\,dt$ 合法。

**典型适用**：根号、分式或复杂结构通过新变量"变直变简"。三角代换、根式代换、倒代换都属于此类。
...
$$\boxed{\int u(x)\,dv(x) = u(x)v(x) - \int v(x)\,du(x),}$$

或写成 $\displaystyle\int uv'\,dx = uv - \int u'v\,dx$。
...
**第四步**（用互逆关系 ii）：$\int (uv)'\,dx = uv + C$，并将 $C$ 吸收到右端的不定积分常数中：
$$\int uv'\,dx = uv - \int u'v\,dx.\quad\square$$

**微分形式**：用 $du = u'\,dx, dv = v'\,dx$ 改写得 $\int u\,dv = uv - \int v\,du$。

#### 选择 $u$ 的优先策略（LIATE / "反对幂指三"）

按下表中"反"在前的顺序选 $u$，余下作 $dv$。**理由**：把"难求导的（如 $\ln x, \arctan x$）"作 $u$，求导后变简单；把"易积分的（如 $e^x, \sin x$）"作 $dv$，积分后形式不变。

| 优先级 | 类型 | 例子 |
|---|---|---|
| 1 | **L**og / **反**三角 | $\ln x, \arcsin x$ |
| 2 | **I**nv 三角 / **对**数 | $\arctan x$ |
| 3 | **A**lg / **幂** | $x^n$ |
| 4 | **T**rig / **三**角 | $\sin x, \cos x$ |
| 5 | **E**xp / **指** | $e^x, a^x$ |

**循环型应用**：若两次分部后原积分 $I$ 重现，解方程即得。见 11.5.3 与例 11.14。

---

#### 规则 6：有理函数的部分分式分解

**问题**：求 $\displaystyle\int \frac{P(x)}{Q(x)}\,dx$，$P, Q$ 是多项式。

**完整算法**（不跳步）：

**第一步**（化为真分式）：若 $\deg P \ge \deg Q$，作多项式除法 $P = qQ + r$（$\deg r < \deg Q$）：
$$\frac{P}{Q} = q + \frac{r}{Q}.$$
其中 $q$ 是多项式（直接积分）。

**第二步**（分母因式分解）：将 $Q(x)$ 在实数域分解为
$$Q(x) = c\cdot \prod_i (x - a_i)^{m_i}\cdot \prod_j (x^2 + b_j x + c_j)^{n_j},$$
即一次因式的幂与不可约二次因式的幂之积。

**第三步**（部分分式分解，理论保证）：$\dfrac{r(x)}{Q(x)}$ 唯一地写为：

$$\frac{r(x)}{Q(x)} = \sum_i \sum_{k=1}^{m_i}\frac{A_{i,k}}{(x-a_i)^k} + \sum_j\sum_{l=1}^{n_j}\frac{B_{j,l}x + C_{j,l}}{(x^2+b_j x+c_j)^l}.$$

**第四步**（求系数 $A, B, C$）：通分回去对比系数，或用"覆盖法"（Heaviside 方法）等技巧。

**第五步**（逐项积分）：每个简单分式都是可积的基本类型：

- $\displaystyle\int \frac{A}{(x-a)^k}\,dx$：$k=1$ 给 $A\ln|x-a|$；$k\ge 2$ 给 $-\dfrac{A}{(k-1)(x-a)^{k-1}}$。
- $\displaystyle\int \frac{Bx+C}{x^2+bx+c}\,dx$：先把分子凑成分母的导数（$2x+b$），再配方处理剩余的 $\dfrac{1}{(x+b/2)^2 + (\cdot)^2}$ 给出 $\arctan$。

**为什么这种分解总是可能？** 这是代数学的**部分分式定理**——由代数闭包（$\mathbb{C}$）+ 共轭对相乘 + 多项式 Bézout 性质保证。

> **示例**：$\dfrac{1}{x^2-1} = \dfrac{1}{(x-1)(x+1)} = \dfrac{A}{x-1} + \dfrac{B}{x+1}$。通分对比：$1 = A(x+1) + B(x-1)$。取 $x=1$ 得 $A=1/2$；取 $x=-1$ 得 $B=-1/2$。故 $\int\dfrac{dx}{x^2-1} = \dfrac{1}{2}\ln\left|\dfrac{x-1}{x+1}\right| + C$。

---

#### 规则 7：三角有理函数的 Weierstrass 替换

**问题**：求 $\displaystyle\int R(\sin x, \cos x)\,dx$，$R$ 是有理函数。

**Weierstrass 万能替换**：令 $t = \tan\dfrac{x}{2}$，则
...
**第二步**（同除 $\cos^2(x/2)$ 化为 $\tan(x/2)$）：分子分母同除 $\cos^2(x/2)$：

$$\sin x = \frac{2\tan(x/2)}{\sec^2(x/2)} = \frac{2t}{1+t^2},\quad \cos x = \frac{1 - \tan^2(x/2)}{\sec^2(x/2)} = \frac{1-t^2}{1+t^2}.$$

（用了 $\sec^2 = 1+\tan^2$。）

**第三步**（$dx$ 与 $dt$ 的关系）：从 $t = \tan(x/2)$ 求导：
$$\frac{dt}{dx} = \frac{1}{2}\sec^2\frac{x}{2} = \frac{1+t^2}{2}\quad\Rightarrow\quad dx = \frac{2}{1+t^2}\,dt.$$

**第四步**：代入后被积函数变成 $t$ 的**有理函数**——由规则 6（部分分式）总可积分。$\square$

> **威力**：理论上任何 $\int R(\sin x, \cos x)\,dx$ 都可化为有理函数积分。但实际中过早用 Weierstrass 替换可能让表达式比直接凑微分复杂得多——所以它是"最后兜底"的工具。

---

#### 规则 8：含 $\sqrt{ax^2+bx+c}$ 的积分（配方 + 三角 / 双曲替换）

**步骤**：

**第一步**（配方）：$ax^2 + bx + c = a\left[\left(x + \dfrac{b}{2a}\right)^2 + \dfrac{4ac-b^2}{4a^2}\right]$。

**第二步**（线性代换）：令 $u = x + \dfrac{b}{2a}$，把根号化为 $\sqrt{a^2 - u^2}$、$\sqrt{a^2 + u^2}$ 或 $\sqrt{u^2 - a^2}$ 之一（取决于 $a$ 与判别式的正负）。

**第三步**（三角替换）：用 11.4.1 的标准三角代换处理。
...
第 0 步：在基本公式表中？  → 直接套
   否 ↓
第 1 步：拆成线性组合 = α∫… + β∫…  → 规则 1
   ↓
第 2 步：含 φ(x) 与 φ'(x) 结构？   → 规则 3 第一类换元（凑微分）
   否 ↓
第 3 步：含根号 / 复杂结构？        → 规则 4 第二类换元（三角/根式/倒代换）
   否 ↓
第 4 步：含两类函数相乘？          → 规则 5 分部积分（按 LIATE）
   循环型 → 解方程
   ↓
第 5 步：有理函数 P(x)/Q(x)？       → 规则 6 部分分式
   ↓
第 6 步：三角有理函数？             → 规则 7 Weierstrass 替换
   ↓
第 7 步：含 √(ax²+bx+c)？          → 规则 8 配方 + 三角替换
   ↓
都不行：可能是"不可初等积分"（如 ∫e^{-x²}dx）
        → 用级数展开（第 13 章）或数值积分（12.6）
```

> **总结口诀**：**线性 → 凑微分 → 变量替换 → 分部 → 有理 → 万能替换**。按这个顺序检查，可处理本科水平的全部初等不定积分。
