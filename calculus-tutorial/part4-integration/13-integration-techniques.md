...
对于形如 $\int R(x, \sqrt{ax^2 + bx + c}) \, dx$ 的积分（其中 $R$ 是有理函数），当三角代换不便时，可用**欧拉代换**。

**第一欧拉代换**：若 $a > 0$，设 $\sqrt{ax^2 + bx + c} = t - \sqrt{a} \cdot x$。

**第二欧拉代换**：若 $c > 0$，设 $\sqrt{ax^2 + bx + c} = tx + \sqrt{c}$。

**第三欧拉代换**：若 $ax^2 + bx + c = a(x - x_1)(x - x_2)$（有两个实根），设 $\sqrt{ax^2 + bx + c} = t(x - x_1)$。

> **例题 13.14** 用欧拉代换求 $\int \dfrac{1}{\sqrt{x^2 + 1}} \, dx$。
...
> **例题 13.15** 计算 $\int_{-\pi}^{\pi} \dfrac{x^2 \sin x}{1 + x^4} \, dx$。
...
> **例题 13.16** 计算 $\int_{-1}^{1} \dfrac{x^4}{1 + e^x} \, dx$。
...
> **例题 13.17** 计算 $\int_0^{\pi/2} \sin^6 x \, dx$。
...
> **例题 13.18** 计算 $\int_0^{\pi/2} \cos^5 x \, dx$。
...
> **例题 13.19** 计算 $\int_0^{\pi} \dfrac{x \sin x}{1 + \cos^2 x} \, dx$。
...
> **例题 13.20** 计算 $\int_0^{\pi/2} \dfrac{\sin x}{\sin x + \cos x} \, dx$。
