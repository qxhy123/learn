...
> **一例速记**：
> **$\varepsilon$-$N$ 三步**：① 设 $\varepsilon > 0$ ② 反向估计 $|a_n - L| < \varepsilon$ 解出 $N$ ③ 验证。
> **夹逼**：找 $\alpha_n \leq a_n \leq \beta_n$ 且 $\lim\alpha_n = \lim\beta_n = L$，则 $\lim a_n = L$。
> **单调有界**：$\{a_n\}$ 单调递增有上界（或单调递减有下界）$\Rightarrow$ 必收敛。
> **等比极限**：$|q| < 1 \Rightarrow q^n \to 0$；$|q| > 1 \Rightarrow |q^n| \to +\infty$（发散）。
> **重要数列**：$\lim (1 + 1/n)^n = e \approx 2.718$；$\lim \sqrt[n]{n} = 1$；$\lim \sqrt[n]{a} = 1$（$a>0$）。

---

## 引入：一道"看起来简单"的递推极限

> **题目**：设数列 $\{a_n\}$ 满足 $a_1 = 1$，$a_{n+1} = \dfrac{a_n + 2}{2}$（$n \geq 1$）。猜测极限并严格证明收敛。

请先停下来想一想：极限是多少？如何证明它**一定存在**（而不是先算"假设极限为 $L$"后直接用）？
...
>
> **第 1 步：猜极限。** 设若 $\lim a_n = L$，则两边取极限：$L = (L + 2)/2 \Rightarrow 2L = L + 2 \Rightarrow L = 2$。所以**猜测极限是 2**。
>
> **第 2 步：证单调。** 我想证 $\{a_n\}$ 单调递增。计算 $a_{n+1} - a_n$：
> $$a_{n+1} - a_n = \frac{a_n + 2}{2} - a_n = \frac{2 - a_n}{2}.$$
> 要让这个差 $\geq 0$，需要 $a_n \leq 2$。所以**单调性依赖上界**，得先证有上界。
>
> **第 3 步：证有上界（数学归纳法）。** 猜上界是 $2$。$a_1 = 1 < 2$ ✓。设 $a_n < 2$，则
> $$a_{n+1} = \frac{a_n + 2}{2} < \frac{2 + 2}{2} = 2.$$
> 归纳完毕，对所有 $n$ 有 $a_n < 2$。
>
> **第 4 步：回证单调。** 由 $a_n < 2$ 知 $a_{n+1} - a_n = (2 - a_n)/2 > 0$，故 $\{a_n\}$ 单调递增。
>
> **第 5 步：单调有界定理。** $\{a_n\}$ 单调递增且有上界，必收敛。记极限为 $L$，由第 1 步 $L = 2$。
>
> **标准陷阱提醒**：若跳过第 2-4 步，直接设 $L$ 并解方程——在极限不存在时这步是非法的。比如 $a_{n+1} = a_n^2 - 1$，若 $a_1$ 选不当，数列可能发散，但设 $L$ 的方程照样有解（$L = -1$ 或 $L = 2$），那两个解都是"假象"。"
...
![数列极限 ε-N 可视化](../figures/svg/calc-p2-04-1.svg)

![单调有界定理：$(1+1/n)^n$ 收敛于 $e$](../figures/svg/calc-p2-04-2.svg)

---

## 思考路标（条件反射）

- 看到"证明数列极限" → 三步 $\varepsilon$-$N$：设 $\varepsilon$ → 反向估计找 $N$ → 验证
- 看到 $\lim a_n$ 是否存在 → 先验单调有界
- 看到"单调递增有上界" → 必收敛
- 看到夹逼 → 找两个收敛于同一极限的边界数列
- 看到 $(1 + 1/n)^n$ → 想 $e$
- 看到 $a_n = $ 含 $\sqrt[n]{}$ → 用 $\sqrt[n]{n} \to 1$, $\sqrt[n]{a} \to 1$（$a > 0$）
- 看到 $\lim a_n / b_n$ → 检查 $b_n \neq 0$，再用四则运算
- 看到子列 $\{a_{n_k}\}$ → 收敛数列的子列必收敛于同一极限

## 易错点

1. **数列发散 ≠ $\lim = \infty$**：$\{(-1)^n\}$ 发散但 $\lim \neq \infty$（来回震荡）。
2. **极限唯一性**：若 $\lim a_n$ 存在，必唯一。两个子列收敛到不同值 → 原数列发散。
3. **$\lim (a_n + b_n) = \lim a_n + \lim b_n$ 仅当两侧都存在**。$\{n\}$ 与 $\{-n\}$ 都发散，但 $\{n + (-n)\} = \{0\}$ 收敛于 0。
4. **$\varepsilon$-$N$ 中 $N$ 与 $\varepsilon$ 有关**：通常写 $N(\varepsilon)$ 或直接说"取 $N = \lceil 1/\varepsilon \rceil$"等。
...
| 名称 | 公式 / 定理 | 关键细节 |
|---|---|---|
| **$\varepsilon$-$N$ 定义** | $\forall\varepsilon>0,\exists N,n>N\Rightarrow\vert a_n-L\vert <\varepsilon$ | $N$ 依赖 $\varepsilon$；验证时从 $\vert a_n-L\vert <\varepsilon$ **反向**解 $n$ |
| **夹逼定理** | $\alpha_n\le a_n\le\beta_n$ 且 $\lim\alpha_n=\lim\beta_n=L\Rightarrow\lim a_n=L$ | 两侧必须收敛到**同一极限** |
| **单调有界定理** | 单调递增有上界（或单调递减有下界）$\Rightarrow$ 收敛 | 只保证极限**存在**，需再设 $L$ 解方程 |
| **等比极限** | $\vert q\vert <1\Rightarrow q^n\to 0$；$q>1\Rightarrow q^n\to+\infty$ | 底数绝对值决定收敛/发散 |
| **重要数列** | $\lim(1+1/n)^n=e$；$\lim\sqrt[n]{n}=1$；$\lim\sqrt[n]{a}=1$（$a>0$） | $n^k/q^n\to 0$（$\vert q\vert >1$）：指数胜多项式 |

### 求数列极限标准 4 步流程

**第 1 步：观察类型。**
- 有通项公式 → 直接四则运算 / 夹逼 / 等价替换
- 只有递推公式 → 单调有界定理（必须先证存在）

**第 2 步：验有界性（递推型）。**
- 猜上界 / 下界，数学归纳法证明

**第 3 步：证单调性（递推型）。**
- 计算 $a_{n+1} - a_n$（或 $a_{n+1}/a_n$），利用步骤 2 的有界性做符号判断

**第 4 步：设 $\lim a_n = L$，代入递推求 $L$。**
- 选正确的根（由有界性确定范围）
...
### 变形 1：$0/0$ 型多项式数列（最高次法）

分子分母同除以最高次 $n^k$，所有低次项 $\to 0$。

**模板**：$\dfrac{a_m n^m + \cdots}{b_k n^k + \cdots} \to \begin{cases} a_m/b_k & m=k \\ 0 & m<k \\ \pm\infty & m>k \end{cases}$

### 变形 2：$\infty/\infty$ 型指数数列（除最大底）

多个不同底指数并排，同除**最大底的 $n$ 次方**，其余项 $\to 0$。

**模板**：$\dfrac{a^n + b^n}{b^n + c^n}$（$a < b < c$）$\to 0$（分子分母同除 $c^n$）。

### 变形 3：$1^\infty$ 型

将底改写为 $1 + \text{小量}$，再用 $e$ 极限：
$(1 + \alpha_n)^{\beta_n} \to e^{\lim \alpha_n \beta_n}$（若 $\alpha_n\to 0$，$\beta_n\to\infty$）。

### 变形 4：夹逼——"放缩到求和"

$n$ 项求和型 $S_n = \sum_{k=1}^n f(n,k)$：将 $f(n,k)$ 的最大值和最小值提取，乘以 $n$，构造上下界，令两侧取极限。

**核心口诀**：分子分母看最高次（多项式）；底数看最大（指数）；不等式凑 $e$（$1^\infty$）；求和放缩夹（夹逼）。
...
> **题目**：求 $\lim_{n\to\infty}\dfrac{3n^3 - 2n + 1}{n^3 + 5n^2}$。

【思路】分子分母最高次均为 $n^3$，同除 $n^3$。

【解】
$$\lim_{n\to\infty}\frac{3n^3-2n+1}{n^3+5n^2} = \lim_{n\to\infty}\frac{3 - 2/n^2 + 1/n^3}{1 + 5/n} = \frac{3-0+0}{1+0} = 3$$
...
> **题目**：求 $\lim_{n\to\infty}\left(\dfrac{1}{n^2+1} + \dfrac{1}{n^2+2} + \cdots + \dfrac{1}{n^2+n}\right)$。

【思路】设 $S_n$ 为该和式。用最大项和最小项放缩，得到两个能求极限的边界。

【解】注意 $n^2+1 \le n^2+k \le n^2+n$（$k=1,\ldots,n$），故
$$\frac{n}{n^2+n} \le S_n \le \frac{n}{n^2+1}.$$
即 $\dfrac{1}{n+1} \le S_n \le \dfrac{n}{n^2+1}$。

两边极限均为 $0$（$n/(n^2+1)\to 0$）。由夹逼定理，
$$\lim_{n\to\infty} S_n = 0.$$

【答案】$\boxed{0}$

【注】$\displaystyle\sum_{k=1}^n \dfrac{1}{n^2+k}$ 是"夹逼"模板题：$n$ 项中每项介于最大和最小之间，乘以 $n$ 后取极限。

### 例 3：递推数列——单调有界

> **题目**：设 $a_1 = \sqrt{2}$，$a_{n+1} = \sqrt{2 + a_n}$（$n\ge 1$）。证明 $\{a_n\}$ 收敛，并求极限。
...
【答案】$\boxed{\lim_{n\to\infty} a_n = 2}$

【注】必须按顺序：**先证有界 → 再证单调 → 最后设 $L$**。跳过前两步直接设 $L$ 是错误的。

---

## 自测题

**自测 1**　求 $\lim_{n\to\infty}\dfrac{2^n + 3^n}{3^n + 4^n}$。

> 💡 提示：分子分母同除 $4^n$，利用 $\vert q\vert <1\Rightarrow q^n\to 0$，答案为 $0$。

**自测 2**　求 $\lim_{n\to\infty} n\left(\sqrt{n+1}-\sqrt{n}\right)$。

> 💡 提示：有理化 $\sqrt{n+1}-\sqrt{n} = \dfrac{1}{\sqrt{n+1}+\sqrt{n}}$，故 $n\cdot\dfrac{1}{\sqrt{n+1}+\sqrt{n}}\to\dfrac{1}{2}$。答案 $\dfrac{1}{2}$。

**自测 3**　用夹逼定理求 $\lim_{n\to\infty}\sqrt[n]{a_1^n+a_2^n+\cdots+a_m^n}$，其中 $a_i>0$。

> 💡 提示：设 $M=\max_i a_i$，则 $M\le\sqrt[n]{\sum a_i^n}\le M\cdot m^{1/n}$。$m^{1/n}\to 1$，故答案为 $M=\max_i a_i$。

**自测 4**　设 $a_1=1$，$a_{n+1}=\dfrac{a_n+3}{4}$。证明收敛并求极限。

> 💡 提示：令 $b_n=a_n-1$，则 $b_{n+1}=b_n/4$，故 $b_n\to 0$，即 $a_n\to 1$。或：先证 $a_n<3$，再证单调递增，设 $L$，解得 $L=1$。

**自测 5**　证明：若 $a_n>0$ 且 $a_{n+1}/a_n\to q<1$，则 $\lim a_n=0$。

> 💡 提示：取 $r\in(q,1)$，则 $\exists N$，$n>N$ 时 $a_{n+1}<r\cdot a_n$，故 $0<a_n<a_N\cdot r^{n-N}\to 0$（等比数列夹逼）。
