...
代入 $y'' - 2y' + y$，三次项与二次项中含 $A,B$ 的部分恰好相消，得
...
> **例题 24.10** 用常数变易法求方程 $y'' + y = \dfrac{1}{\cos x}$ 的一个特解。
...
> **例题 24.11** 一弹簧振子，质量 $m = 1$ kg，弹性系数 $k = 4$ N/m，无阻尼。初始时物体在平衡位置下方 $0.1$ m 处静止释放。求运动规律。
...
> **例题 24.12** 一RLC串联电路，$L = 1$ H，$R = 2$ Ω，$C = 0.5$ F，外加电压 $E = 10$ V（直流）。初始时 $q(0) = 0$，$i(0) = 0$。求电荷 $q(t)$。
...
7. **物理意义（振动/阻尼）**：$\alpha<0$ → 衰减；$\alpha=0$ → 等幅振荡；$\alpha>0$ → 发散增长。弹簧系统 $mx''+cx'+kx=F(t)$ 中，$\alpha=-\dfrac{c}{2m}$，$k/m=\omega^2+\alpha^2$。
