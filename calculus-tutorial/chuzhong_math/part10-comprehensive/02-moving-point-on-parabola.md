...
![四组平行四边形：配对甲（红）、乙（紫）、丙两组（绿）](../figures/svg/ex-p10-02-3.svg)

**思路**：四个点构成平行四边形，关键看**哪两点是对角线**。三种配对：
- 配对甲：$AC$ 与 $PQ$ 是对角线 → $AC$ 中点 = $PQ$ 中点。
- 配对乙：$AP$ 与 $CQ$ 是对角线 → $AP$ 中点 = $CQ$ 中点。
- 配对丙：$AQ$ 与 $CP$ 是对角线 → $AQ$ 中点 = $CP$ 中点。
...
- **配对丙**：$AQ$ 中点 $\left(\frac{-1 + t}{2}, 0\right) = CP$ 中点 $\left(\frac{x}{2}, \frac{-x^2 + 2x + 3 + 3}{2}\right)$。由 $y$：$-x^2 + 2x + 6 = 0$，$x^2 - 2x - 6 = 0$，$x = 1 \pm \sqrt{7}$。两个解都给出有效 $P$，对应 $t = x - 1 = \pm\sqrt{7}$，故 $P = (1 + \sqrt{7}, -2)$、$Q = (\sqrt{7}, 0)$ 或 $P = (1 - \sqrt{7}, -2)$、$Q = (-\sqrt{7}, 0)$。
