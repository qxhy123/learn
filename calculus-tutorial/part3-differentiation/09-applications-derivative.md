...
> **一例速记**：
> **单调性 + 极值**：求 $f'(x)$，解 $f'=0$ 找驻点，看 $f'$ 符号变化（左正右负 → 极大）。
> **凹凸 + 拐点**：求 $f''$，$f''>0$ 凸（开口向上），$f''$ 变号处为拐点。
> **罗必达**：仅适用 $\frac{0}{0}$ 或 $\frac{\infty}{\infty}$ 不定型；其它不定型需先变形（如 $0\cdot\infty$ 转化为商）。
...
请先停下来想一想：见**含参 $f$ 求单调**，标准动作是先求 $f'$、解 $f'=0$ → **分类讨论 $a$ 的取值**。

**关键观察**：$a$ 的不同取值（正 / 零 / 负）会让 $f'$ 变成不同的二次函数（开口方向不同，根的存在性不同），所以必须**分情况**。下面把内心独白完整还原。

---

## 思维路径还原（解题者的内心独白）

> "看到含参 $f$ 求单调性，立刻准备**分类讨论 $a$**。
>
> **求 $f'$**：$f'(x) = 3ax^2 - 6ax = 3ax(x-2)$。
>
> **分类**：$f'$ 的零点是 $x=0$ 和 $x=2$（与 $a$ 无关）。但 $f'$ 的**符号**依赖 $a$：
>
> - **情形 1：$a=0$**。则 $f(x) = 1$ 是常函数，**全 $\mathbb{R}$ 既不增也不减**。
> - **情形 2：$a>0$**。$f'=3a\cdot x(x-2)$。$x(x-2)$ 在 $x<0$ 时正、$0<x<2$ 时负、$x>2$ 时正。乘 $3a>0$ 保号 → **$f$ 在 $(-\infty,0)$ 增、$(0,2)$ 减、$(2,+\infty)$ 增**。极大值 $f(0)=1$，极小值 $f(2)=1-4a$。
> - **情形 3：$a<0$**。$3a<0$ 改变符号 → **$f$ 在 $(-\infty,0)$ 减、$(0,2)$ 增、$(2,+\infty)$ 减**。极小值 $f(0)=1$，极大值 $f(2)=1-4a$（注意 $a<0$ 时 $1-4a>1$）。
>
...
> ⚠️ **常见陷阱**
> 洛必达法则的一个高频误用是：看到“复杂分式”就直接求导。必须先确认原式确实是 $\dfrac{0}{0}$ 或 $\dfrac{\infty}{\infty}$ 型，并且在求导后仍有可分析的极限；若 $\lim \dfrac{f'}{g'}$ 不存在，只能说明“这条路走不通”，不能反推出原极限不存在。
...
$$
\eta(t)=\eta_{\min} + \frac12(\eta_{\max}-\eta_{\min})
...
![函数与导数符号的对应：单调性](../figures/svg/calc-p3-09-1.svg)

![极值 + 拐点：凹凸性几何意义](../figures/svg/calc-p3-09-2.svg)

![罗必达法则的几何直观](../figures/svg/calc-p3-09-3.svg)
...
- 看到"求最值（闭区间）" → 极值候选点 ∪ 端点 → 比较
- 看到"凹凸性" → 用 $f''$：$f''>0$ 凸（开口向上）
- 看到 $0/0$ 或 $\infty/\infty$ → 罗必达
- 看到含 $\ln, e^x$ 的极限 → 罗必达常奏效
- 看到"求最优解 / 最大利润 / 最小面积" → 建模 + 求导 → 验范围
- 看到 $f$ 含参 → 必分类讨论

## 易错点

1. **罗必达适用条件**：必须是 $0/0$ 或 $\infty/\infty$ 不定型，其它不定型（$0\cdot\infty, \infty-\infty, 1^\infty$ 等）需先变形。
...
| **极值（一阶）** | $f'$ 在 $x_0$ 左正右负 → 极大；左负右正 → 极小 | 符号变化判极值 |
| **极值（二阶）** | $f'(x_0)=0$，$f''(x_0)<0$ → 极大；$>0$ → 极小 | 计算 $f''$ 更快 |
| **拐点** | $f''$ 变号处（$f''=0$ 不一定是拐点） | 凹凸性转换点 |
| **洛必达** | $\frac{f'(x)}{g'(x)}$ 仅在 $0/0$ 或 $\infty/\infty$ 时可用 | 不定型极限 |
...
$y=u^v$（$u\to 1, v\to\infty$），标准化写法：$y=e^{v\ln u}$，因为 $u=1+\alpha$（$\alpha\to 0$），$v\ln u\approx v\alpha$，再用洛必达。或用公式 $\lim u^v=e^{\lim v(u-1)}$（等价无穷小 $\ln(1+\alpha)\sim\alpha$）。

### 变形 4：罗必达不适用时的替代

若 $\lim f'/g'$ 不存在，**不能**反推原极限不存在。换用等价无穷小、夹逼定理或变量替换。典型：$\lim_{x\to\infty}(x+\sin x)/x=1+\lim(\sin x)/x$，但 $\lim\cos x$ 不存在——洛必达失效，直接用夹逼。
...
$f''(x)=6x-6$，$f''(-1)=-12<0$ → 极大，$f(-1)=10$；$f''(3)=12>0$ → 极小，$f(3)=-22$。

$\boxed{f \text{ 在 } (-\infty,-1) \text{ 和 } (3,+\infty) \text{ 递增；极大值 } f(-1)=10\text{，极小值 }f(3)=-22}$

【注】驻点是否为极值必须经过"符号变化验证"或"二阶导验证"，不能只写出驻点就下结论。

### 例 2：洛必达 + 不定型变形

> **题目**：求 $\lim_{x\to 0^+}\left(x^x - 1 - x\ln x\right)/x^2$。

【思路】直接代入 $x=0$ 时 $x^x\to 1$、$x\ln x\to 0$，分子 $\to 0$，分母 $\to 0$，是 $0/0$ 型。

【解】利用 $x^x = e^{x\ln x}$，令 $t=x\ln x\to 0$（$x\to 0^+$），$e^t\approx 1+t+t^2/2$。

更直接：展开 $x^x=e^{x\ln x}=1+x\ln x+\frac{(x\ln x)^2}{2}+o((x\ln x)^2)$，

分子 $= x^x - 1 - x\ln x = \frac{(x\ln x)^2}{2}+o((x\ln x)^2)$，

$$\lim_{x\to 0^+}\frac{x^x-1-x\ln x}{x^2}=\lim_{x\to 0^+}\frac{(x\ln x)^2}{2x^2}=\lim_{x\to 0^+}\frac{(\ln x)^2}{2}=+\infty$$

$\boxed{+\infty}$（极限不存在，但有意义：分子趋于 $0^+$ 但比 $x^2$ 慢得多）

【注】若改为 $\lim_{x\to 0^+}(x^x-1)/x$，则分子 $\sim x\ln x+\frac{(x\ln x)^2}{2}$，分母 $x$，极限 $=\ln x\to-\infty$。
...
$\boxed{\text{凹区间 }(-\infty,2),\ \text{凸区间 }(2,+\infty),\ \text{拐点 }(2, 2e^{-2})}$
...
**自测 1**　求 $f(x)=x+\dfrac{4}{x}$（$x>0$）的最小值。

> 💡 提示：$f'=1-4/x^2=0\Rightarrow x=2$，$f(2)=4$。$f''>0$ 确认极小。最小值为 $\boxed{4}$（$x\to 0^+$ 和 $x\to+\infty$ 时 $f\to+\infty$，故全局最小）。

**自测 2**　用洛必达求 $\lim_{x\to 0}\dfrac{e^x-e^{-x}-2x}{x-\sin x}$。

> 💡 提示：连用两次洛必达：$(e^x+e^{-x}-2)/(1-\cos x) \to (e^x-e^{-x})/\sin x \to (e^x+e^{-x})/\cos x\big|_{x=0}=2$。

**自测 3**　求 $\lim_{x\to 1^-}(1-x)^{1/\ln(1-x)}$。

> 💡 提示：$1^\infty$ 型（$x\to 1^-$ 时 $1-x\to 0^+$，$1/\ln(1-x)\to -\infty/(-\infty)$）。设 $y$，$\ln y=\frac{\ln(1-x)}{\ln(1-x)}=1$，故 $y=e$。$\boxed{e}$。

**自测 4**　$f(x)=x^4-8x^2+3$。求所有拐点。

> 💡 提示：$f''=12x^2-16=0\Rightarrow x=\pm 2/\sqrt{3}$。验 $f''$ 变号（左负右正），两点均为拐点：$(\pm 2/\sqrt{3},\ f(\pm 2/\sqrt{3}))$。

**自测 5**　证明当 $x>0$ 时 $\ln(1+x)>\dfrac{x}{1+x}$。
...
> 单调性 + 极值：$f'\to$ 驻点 → 符号变化（左正右负 = 极大）。
> 凹凸 + 拐点：$f''>0$ 凸，$f''$ 变号处为拐点。
> 罗必达：仅 $0/0$ 或 $\infty/\infty$；其它先变形。
