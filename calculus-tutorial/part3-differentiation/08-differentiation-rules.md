...
> **例题 8.7** 设 $x^2 + y^2 = 1$，求 $\dfrac{dy}{dx}$。
...
> **例题 8.10** 求 $y = \dfrac{x \sqrt{1-x^2}}{(1+x^2)^2}$（$|x| < 1$）的导数。
...
> **例题 8.11** 椭圆的参数方程为 $x = a\cos t$，$y = b\sin t$，求 $\dfrac{dy}{dx}$。

**解**：

$$\frac{dy}{dx} = \frac{(b\sin t)'}{(a\cos t)'} = \frac{b\cos t}{-a\sin t} = -\frac{b}{a}\cot t$$

> **例题 8.12** 摆线的参数方程为 $x = a(t - \sin t)$，$y = a(1 - \cos t)$，求 $\dfrac{dy}{dx}$。
...
> **例题 8.13** 对于摆线 $x = a(t - \sin t)$，$y = a(1 - \cos t)$，求 $\dfrac{d^2y}{dx^2}$。

**解**：由例题 8.12，$\dfrac{dy}{dx} = \cot\dfrac{t}{2}$。
...
> **例题 8.14** 求 $y = e^x$ 的 $n$ 阶导数。

**解**：$y' = e^x$，$y'' = e^x$，...，归纳得

$$(e^x)^{(n)} = e^x$$

> **例题 8.15** 求 $y = \sin x$ 的 $n$ 阶导数。
...
> **例题 8.16** 求 $y = x^2 e^x$ 的 $n$ 阶导数（$n \geq 2$）。
...
3. **隐函数求导**：方程两边对 $x$ 求导，$y$ 视为 $x$ 的函数，解出 $y'$

4. **对数求导法**：先取对数，再求导，适用于幂指函数和复杂乘除式

5. **参数方程求导**：$\dfrac{dy}{dx} = \dfrac{dy/dt}{dx/dt}$，二阶导数需要再除以 $\dfrac{dx}{dt}$

6. **高阶导数**：逐次求导，常用莱布尼茨公式处理乘积的高阶导数
