...
**第 4 题**：数列 $\{a_n\}$ 满足 $a_1 = 4$，$a_{n+1} = \dfrac{a_n}{2} + 1$。证明 $\{a_n\}$ 收敛并求极限。

提示：先证有下界 $a_n > 2$（归纳：$a_1 = 4 > 2$；若 $a_n > 2$，则 $a_{n+1} = \dfrac{a_n}{2}+1 > \dfrac{2}{2}+1 = 2$）；再证单调递减（$a_{n+1} - a_n = 1 - \dfrac{a_n}{2} < 0$，因为 $a_n > 2$）。由单调有界收敛定理极限存在。设极限 $L$，$L = \dfrac{L}{2} + 1$，解得 $L = 2$。
