...
> **一例速记**：
> **判敛流程**：先检查 $a_n\to 0$（否则直接发散）→ 正项级数用比值/根值/比较法 → 交错级数用 Leibniz 判别法 → 最后判断是绝对收敛还是条件收敛。
> **含阶乘首选比值法**：$\rho = \lim|a_{n+1}/a_n|$，$\rho<1$ 收敛，$\rho>1$ 发散，$\rho=1$ 失效。

---

## 引入：比值法的决策树

> **题目**：判断 $\displaystyle\sum_{n=1}^{\infty} \frac{n!}{n^n}$ 是否收敛。

请先停下来想一想：看到含**阶乘**的正项级数，标准动作是**比值法**。

**关键观察**：$n!$ 与 $n^n$ 竞争，前者增长更慢（Stirling 近似 $n!\sim\sqrt{2\pi n}(n/e)^n$ → 比 $n^n$ 多一个 $1/e^n$ 的衰减因子）。下面把内心独白完整还原。

---

## 思维路径还原（解题者的内心独白）

> "见到 $\sum n!/n^n$，先看通项 $a_n = n!/n^n > 0$，是**正项级数**。
>
> **第一步：验必要条件**。$a_n\to 0$？粗略估计 $n!/n^n \le 1/2^{n-1}\to 0$，必要条件满足。
>
> **第二步：选方法**。含 $n!$ 首选比值法。
>
...
> **第三步：求极限**。
>
> $$\rho = \lim_{n\to\infty}\left(\frac{n}{n+1}\right)^n = \lim_{n\to\infty}\frac{1}{\left(1+\frac{1}{n}\right)^n} = \frac{1}{e} \approx 0.368 < 1$$
>
> **第四步：下结论**。$\rho = 1/e < 1$，由比值判别法，级数**收敛**。
>
...
![级数判敛决策树](../figures/svg/calc-p5-15-1.svg)

**图 15-2**：p-级数 $\sum 1/n^p$ 通项衰减速度比较

![p-级数收敛性](../figures/svg/calc-p5-15-2.svg)

**图 15-3**：交错调和级数部分和震荡收敛至 $\ln 2$

![交错级数部分和](../figures/svg/calc-p5-15-3.svg)

---

## 思考路标（条件反射）

- 看到 $\sum a_n$ → 先检查 $a_n\to 0$（必要条件，否则发散）
- 看到正项级数 → 比值 / 根值 / 比较法
- 看到 $\sum 1/n^p$ → $p>1$ 收，$p\leq 1$ 散（p-级数）
- 看到 $\sum q^n$ → $|q|<1$ 收，否则散（几何）
- 看到交错级数 → Leibniz：$|a_n|\downarrow 0$ → 收敛
- 看到 $\rho=\lim|a_{n+1}/a_n|$ → 比值法：$\rho<1$ 收，$\rho>1$ 散
- 看到含阶乘 → 比值法常奏效
- 看到绝对收敛 → 必条件收敛；反之不成立

## 易错点

1. **$a_n\to 0$ 是收敛的必要不充分**：$\sum 1/n$ 满足但发散（调和级数）。
...
| 步骤 | 动作 | 备注 |
|---|---|---|
| 1 | 检查 $a_n \to 0$ | 否则直接**发散** |
| 2 | 判断是否为**正项**级数 | 是 → 走比值 / 根值 / 比较路线 |
| 3 | 含阶乘或指数幂 → **比值法** | $\rho = \lim \vert a_{n+1}/a_n\vert $ |
| 4 | 含 $n$ 次幂 → **根值法** | $\rho = \lim \sqrt[n]{\vert a_n\vert }$ |
| 5 | $\rho = 1$ / 含对数 → **积分判别法** 或**比较法** | 比对 p-级数 / 几何级数 |

对**交错级数** $\sum(-1)^{n-1}a_n$（$a_n>0$）：先验 Leibniz 条件（$a_n \downarrow 0$）→ 收敛；再判 $\sum a_n$ 敛散 → 区分绝对 / 条件收敛。
...
| 几何级数 $\sum q^n$ | $\vert q\vert <1$ 收，$\vert q\vert \geq 1$ 散 | 比值 / 根值法目标 |
...
通项形如 $a_n = f(n) \cdot g(n)$：先看哪个因子"主导"衰减速度，选参照级数做**极限形式比较法**。

**例**：$\sum \dfrac{\ln n}{n^2}$。与 $\sum 1/n^{3/2}$ 比较：$\dfrac{\ln n / n^2}{1/n^{3/2}} = \dfrac{\ln n}{\sqrt{n}} \to 0$，故比 $\sum 1/n^{3/2}$ 更小，绝对收敛。
...
通项变号但不交错时，先取绝对值，若 $\sum \vert a_n\vert $ 收敛则原级数绝对收敛（无需验 Leibniz）。
...
> **题目**：判断 $\displaystyle\sum_{n=1}^{\infty} \frac{\ln n}{n^2}$ 的收敛性。
...
$\sum 1/n^{3/2}$ 是 $p = 3/2 > 1$ 的 p-级数，收敛。由比较判别法，原级数**收敛**。

【答案】$\boxed{\text{收敛（正项级数，比较法）}}$。

### 例 2：交错级数 → 绝对 vs 条件

> **题目**：判断 $\displaystyle\sum_{n=1}^{\infty} \frac{(-1)^{n-1}}{\sqrt{n}}$ 是绝对收敛、条件收敛还是发散。

【思路】先 Leibniz → 收敛；再查 $\sum 1/\sqrt{n}$ → $p=1/2 \leq 1$，发散。

【解】$a_n = 1/\sqrt{n}$ 单调递减且 $\to 0$，由 Leibniz 判别法，交错级数**收敛**。
...
> **题目**：判断 $\displaystyle\sum_{n=0}^{\infty} \frac{(-2)^n}{n!}$ 的收敛性。

【思路】含阶乘 → 比值法；通项含负号先取绝对值。

【解】取绝对值 $\vert a_n\vert  = 2^n / n!$，

$$\frac{\vert a_{n+1}\vert }{\vert a_n\vert } = \frac{2^{n+1}}{(n+1)!} \cdot \frac{n!}{2^n} = \frac{2}{n+1} \to 0 < 1$$

故 $\sum \vert a_n\vert $ 收敛，原级数**绝对收敛**（且和为 $e^{-2}$）。
...
**自测 1**　判断 $\displaystyle\sum_{n=1}^{\infty} \frac{2^n \cdot n!}{n^n}$ 的收敛性。

> 提示：比值法，$\rho = \lim (n/(n+1))^n \cdot 2 = 2/e < 1$，**收敛**。

**自测 2**　判断 $\displaystyle\sum_{n=1}^{\infty} \left(\frac{3n+1}{4n-1}\right)^n$ 的收敛性。

> 提示：根值法，$\rho = \lim \sqrt[n]{a_n} = 3/4 < 1$，**收敛**。

**自测 3**　判断 $\displaystyle\sum_{n=2}^{\infty} \frac{1}{n(\ln n)^3}$ 的收敛性。

> 提示：积分判别法，$p = 3 > 1$，类比 $\sum 1/(n(\ln n)^p)$，**收敛**。

**自测 4**　$\displaystyle\sum_{n=1}^{\infty} \frac{(-1)^n}{n^2 + 1}$ 是绝对收敛、条件收敛还是发散？

> 提示：$\sum 1/(n^2+1) < \sum 1/n^2$ 收敛，故**绝对收敛**。
...
> 判敛流程：先必要条件 $a_n\to 0$ → 正项级数（比值/根值/比较）→ 交错级数（Leibniz）→ 绝对/条件收敛。
> 含阶乘首选比值法；通项含 $n$ 次幂首选根值法；$\rho=1$ 换积分判别或比较法。
