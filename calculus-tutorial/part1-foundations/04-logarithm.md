...
> **一例速记**：
> **核心互逆**：$a^{\log_a x} = x$（$x>0$），$\log_a(a^x) = x$；$\ln x = \log_e x$，$\lg x = \log_{10} x$。
> **三大恒等式**：$\log(xy) = \log x + \log y$；$\log(x/y) = \log x - \log y$；$\log x^r = r\log x$。
> **换底公式**：$\log_a x = \dfrac{\ln x}{\ln a}$；任何底 $a^x = e^{x\ln a}$。
> **单调性**：$a > 1$ 时 $a^x$ 和 $\log_a x$ 均递增；$0 < a < 1$ 时均递减。
> **增长比较**：$\ln x \ll x^\alpha \ll a^x$（$x \to +\infty$，$\alpha > 0$，$a > 1$）。
...
> **结论**：$x = 3$。
>
> **通用模式**：对数方程 = （写定义域）$\to$ （合并到单个 $\log_a$）$\to$ （两边取 $a$ 的幂消去 $\log$）$\to$ （回代验证）。Log-Sum-Exp 数值技巧也来自同样的恒等式：$\log\sum e^{z_i} = m + \log\sum e^{z_i - m}$，把最大值 $m$ 分离出来，让指数项都不超过 $1$，从而数值稳定。"
...
![指数函数与对数函数关于 y=x 对称](../figures/svg/calc-p1-04-1.svg)
...
$\log\sum_i e^{z_i} = m + \log\sum_i e^{z_i - m}$（$m = \max z_i$）。数值上让所有指数都 $\leq 0$，避免上溢和下溢，是 softmax 数值稳定实现的核心。
...
【答案】$\boxed{x = 2}$（两问答案相同，纯属巧合，验证了流程正确）。
...
即 $x^2 - x - 4 < 0$，解得 $\dfrac{1-\sqrt{17}}{2} < x < \dfrac{1+\sqrt{17}}{2}$。

与定义域取交集（$x < -1$ 或 $x > 2$），得

$$\frac{1-\sqrt{17}}{2} < x < -1 \quad \text{或} \quad 2 < x < \frac{1+\sqrt{17}}{2}.$$

【答案】$\boxed{x \in \left(\frac{1-\sqrt{17}}{2},\ -1\right) \cup \left(2,\ \frac{1+\sqrt{17}}{2}\right)}$。
...
(1) $\log_4 8 = \dfrac{\log_2 8}{\log_2 4} = \dfrac{3}{2}$；$\log_9 27 = \dfrac{\log_3 27}{\log_3 9} = \dfrac{3}{2}$；和为 $3$。

(2) 令 $x = 1000$，比较 $(\ln x)^3$ vs $x^{1/2}$。由 $\ln x \ll x^{1/6}$（$x \to \infty$），对 $x = 1000$ 较大时 $\ln 1000 \approx 6.9 \ll 1000^{1/6} \approx 3.16$……实际上 $(\ln 1000)^3 \approx 6.9^3 \approx 329$ 而 $1000^{1/2} \approx 31.6$，所以 $(\ln 1000)^3 \gg 1000^{1/2}$（注意"$\ll$"是渐近结论，$x = 1000$ 还不够大）。
...
**自测 1**　化简：$\log_2 32 + \log_2\dfrac{1}{8} - \log_4 16$。
...
**自测 3**　已知 $\lg 2 \approx 0.301$，求 $\log_2 100$ 的近似值。

> 💡 提示：换底 $\log_2 100 = \lg 100 / \lg 2 = 2 / 0.301 \approx 6.64$。

**自测 4**　证明 $\log_a x \cdot \log_b y = \log_a y \cdot \log_b x$（$a, b, x, y > 0$，$a, b \neq 1$）。

> 💡 提示：两边都换底为 $\ln$：左边 $= \dfrac{\ln x}{\ln a} \cdot \dfrac{\ln y}{\ln b}$，右边 $= \dfrac{\ln y}{\ln a} \cdot \dfrac{\ln x}{\ln b}$，分子均为 $\ln x \ln y$，分母均为 $\ln a \ln b$，相等。

**自测 5**　PyTorch 中计算 $\log\!\sum_i e^{z_i}$ 时，为什么要先减去 $m = \max z_i$，而不是直接计算？
