# LAB-MODEL-INDEX-01: 观察同一文件的 staged 与 unstaged 差异

## 目标

观察同一文件的 staged 与 unstaged 差异。本场景对应 `01 Mental Model`，用于把章节中的 lab id 落地为可重复执行的本地 Git 练习。
...
cd workspaces/model-index/index-lab
```

## 执行

1. 观察同一个文件同时存在 staged 与 unstaged 修改。
2. 用 `git restore --staged card.txt` 取消暂存后再次观察。
3. 重新 `git add card.txt`，说明 index 快照如何变化。
