# LAB-ORIENT-STATUS-01: 初始化练习仓库并运行状态观察面板

## 目标

初始化练习仓库并运行状态观察面板。本场景对应 `00 Orientation`，用于把章节中的 lab id 落地为可重复执行的本地 Git 练习。
...
cd workspaces/orient-status/status-lab
```

## 执行

1. 运行默认观察面板。
2. 解释 working tree、index、HEAD 各自当前看到什么。
3. 用 `git diff` 和 `git diff --cached` 对照状态输出。
...
```bash
cd git-tutorial/labs
rm -rf workspaces/orient-status
```
