# LAB-DAILY-CLEAN-COMMIT-01: 从修改到干净提交的闭环

## 目标

从修改到干净提交的闭环。本场景对应 `02 Daily Workflow`，用于把章节中的 lab id 落地为可重复执行的本地 Git 练习。
...
cd workspaces/daily-clean-commit/clean-commit-lab
```
...
1. 先观察未提交修改。
2. 只把一个意图加入暂存区并提交。
3. 确认 `git status -sb` 回到干净，或解释剩余修改为什么暂不提交。
...
```bash
cd git-tutorial/labs
rm -rf workspaces/daily-clean-commit
```
