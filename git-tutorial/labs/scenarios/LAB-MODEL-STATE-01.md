# LAB-MODEL-STATE-01: Model State 01

## 目标

把章节中的 `LAB-MODEL-STATE-01` 引用落地为一个可执行练习入口。
...
- 不确定前先创建备份分支：`git branch backup/lab-model-state-01`。
- 对未共享历史优先在本地修正；对已共享历史优先使用新增提交修复。
- 如实验仓库混乱，回到 `labs` 目录重新运行 `./bin/git-lab.sh LAB-MODEL-STATE-01 --force`。

## 清理

```bash
rm -rf git-tutorial/labs/workspaces/model_state_01
```
