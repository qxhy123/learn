...
本地 hooks、别名和配置层级。本场景对应 `07 Scale and Governance`，用于把章节中的 lab id 落地为可重复执行的本地 Git 练习。
...
1. 查看 `core.hooksPath` 来源。
2. 尝试提交 `secret.pem`，观察 hook 拦截。
3. 移除敏感文件后重新提交，并说明哪些规则必须进 CI。
